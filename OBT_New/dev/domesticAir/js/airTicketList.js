		...
		success: function(data) {
			...
			if (res.FareList.length != 0) {
				$(".recommendTrain").html(
					...
					'</div>\
                    <div style="line-height:60px;width:100px;text-align: center;">' + res.UseTime +
					'</div>\
                    ...
                    <div style="font-size:14px;color:#4d4d4d;line-height:60px;margin-left:95px;">' +
					get_lan('lowest') + '</div>\
                    \
                ')
...
function ticketList() {
	...
	// } else if (searchDomInfo.type == "roundTrip") {
		...
		if (!isReturn) {
			...
			if(searchDomInfo.type=='multiple'){
				if(obtLanguage=="CN"){
					var roundStr=sobj.arrivalCityText+"-"+$('#domArrivalCity').val()+"&nbsp;&nbsp;&nbsp;"+sobj.returndate+"&nbsp;&nbsp;&nbsp;"+getWeek(sobj.returndate)
				}else{
					var roundStr=sobj.arrivalCity+"-"+$('#domArrivalCity').attr('code')+"&nbsp;&nbsp;&nbsp;"+sobj.returndate+"&nbsp;&nbsp;&nbsp;"+getWeek(sobj.returndate)
				}
			}else{
				if(obtLanguage=="CN"){
...
function ticketListInfo(res) {
	...
	res.map(function(item, index) {
		...
		var showBaggage = 'hide', baggage = '';
		var baggageItem = item.Cabins[0].BaggageList[0]
		if( parseInt(baggageItem.LuggageTypeCode) === 1 && (baggageItem.BaggageWeightInfo || baggageItem.BaggagePieces) ) {
			showBaggage = ''
		...
		$(".ticketList").append('\
			...
			'</div>\
                <div class="ticketMeal">\
				 ...
				 <div class="luggage '+showBaggage+'">'+baggage+'</div>\
			</div>\
			...
			'" LimitFare="'+item.LimitFare+'"><span style="text-decoration: underline;">' + item.Cabins[
				0].FareAmountShow + '</span><span style="font-size:14px;color:#4d4d4d;">'+ProfileInfo.OfficeCurrency+'</span></div>\
					...
					success: function(data) {
						...
						if (res.length != 0) {
							$(that).parent().next().html(
								...
								'</th>\
                                <th style="width:180px;" class="Baggage">'+get_lan('ticketSpread').Baggage+'</th>\
							...
							res.map(function(item) {
								...
								// 行李额弹窗
								var baggage ='', baggageList = item.BaggageList, baggageDetail = ''
								...
								var baggagePopUpDom = baggageDetail ? 
									'<div class="baggage__pop hide"><div class="baggage__wrapper">\
										<div class="baggage__pop__text">'+get_lan('ticketSpread').BaggagePopText+'</div>\
								...
								if (alterPrice != "" && parseFloat(item.FareAmount) >= parseFloat(alterPrice)) {
									$(that).parent().next().children(".spreadTable").append(
										...
										item.FareAmountShow + ProfileInfo.OfficeCurrency +'</td>\
										<td class="baggage__weight">' + baggagePopUpDom +
											'<span class="'+baggageBtn+'">' + baggage + '</span>' + 
										'</td>\
							...
							// 点击行李额显示详细
							$('.baggage__btn').mouseenter(function(e){
								$('.baggage__pop').addClass('hide')
								var baggagePop = $(this).siblings('.baggage__pop')
								var height = baggagePop.height(), width = baggagePop.width()
								var top = (0-parseInt(height)) + 'px', left = parseInt(( 0 - parseInt(width) + 66 ) / 2) + 'px'
								baggagePop.removeClass('hide').css('top',top).css('left',left)
								$('.baggage__triangle')
									.css('top',parseInt(parseInt(height)-5) +'px')
									.css('left', (parseInt(width) + parseInt(( 0 - parseInt(width) + 78 ) / 2)) + 'px')
								stopBubble(e)
							})
							$('.baggage__btn').mouseleave(function(){
								$('.baggage__pop').addClass('hide')
							})
...
function orderTicket(that) {
	...
	var orgAirport = '', arrAirport = '', orgAirportBack = '', arrAirportBack = ''
	var orgCity = $(that).attr("orgcity") ? $(that).attr("orgcity") : $(that).attr("airportdepartecode")
	var dstcity = $(that).attr("dstcity") ? $(that).attr("dstcity") : $(that).attr("airportarrivecode")
	...
	if (searchDomInfo.type == "oneWay" || ((searchDomInfo.type == "roundTrip" || searchDomInfo.type == "multiple") && !isReturn)) {
		var newPrice = price;
		orgAirport = orgCity
		arrAirport = dstcity
		// 缓存起落机场
		var airportInfoForCheapestAPI={
			orgAirport:orgCity,
			arrAirport:dstcity
		}
		window.sessionStorage.setItem('airportInfoForCheapestAPI',JSON.stringify(airportInfoForCheapestAPI))
	} else if ((searchDomInfo.type == "roundTrip" || searchDomInfo.type == "multiple") && isReturn == 1) {
		try {
			var domTicketInfo = JSON.parse($.session.get('domTicketInfo'));
			console.log(domTicketInfo);
			var newPrice = domTicketInfo.price + ',' + price;
			orgAirportBack = orgCity
			arrAirportBack = dstcity
			orgAirport = JSON.parse(window.sessionStorage.getItem('airportInfoForCheapestAPI')).orgAirport
			arrAirport = JSON.parse(window.sessionStorage.getItem('airportInfoForCheapestAPI')).arrAirport
		} catch (error) {
			console.log(error)
		}
	}
	...
	var JSONObjForCheapestSegmentNew = {
		request: {
			paramKey         : paramKey,
			isReIssue        : "",
			id               : id,
			...
			segmentSearchType: sType,
			selectCabinCode  : swCabinCode,
			selectFlightNO   : swflightnumber,
			orgAirport       : orgAirport,
			arrAirport       : arrAirport,
			orgAirportBack   : orgAirportBack,
			arrAirportBack   : arrAirportBack
		}
	}
	$('body').mLoading("show");
	$.ajax({
		...
		data: {
			...
			jsonStr: JSON.stringify(JSONObjForCheapestSegmentNew)
		},
		success: function(data) {
			var res = JSON.parse(data);
			console.log(res);
			...
			if (parseInt(lowestPolicyFareAmount) < parseInt(price) || SWPolicy) {
				...
				$(".reasonConfirm").unbind("click").click(function() {
					if (!$('.reasonRadio:checked').val() && $('.reasonRadio:checked').attr("code") != "OT") {
						...
						var reasonCode = $('.reasonRadio:checked').attr("code") == "OT" ? $('.reasonRadio:checked').attr("code") + tools.innerHtml($(".reasonTextarea").val()) : $('.reasonRadio:checked').attr("code");
						var reasonText = $('.reasonRadio:checked').attr("code") == "OT" ? tools.innerHtml($(".reasonTextarea").val()) : $(
