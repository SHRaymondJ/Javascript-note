...
var cn = {
    ...
    "passengerPop": {
        ...
        "nameRemind": "姓名填写有误",
        "documentRemind": "证件信息填写有误",
        "countryRemind": "国籍填写有误",
        "validityRemind": "有效期填写有误",
        'company': '公司:',
    },
    "newCustomerPop": {
        "nick": "昵称:",
    ...
    "success": "预订成功！",
    "luggageReminder": "目前各航空公司的免费托运行李额政策各不相同，部分航空公司取消了免费行李额，乘客需要在航空公司或者机场支付托运行李费用。具体情况请以航空公司官网为准，或者咨询服务小组，谢谢！",
    "covidReminder": "因各地新冠疫情防控政策可能随时调整，请您在出行前务必参照当地最新疫情防控政策，合理安排出行。",
    "totalAmount": "总金额:",
    ...
    "VIPReminder":"当前航班预订已超过5位VIP，请选择其他航班",
    "alterBtn":"确认改签",
    "alterChangeFee":"改签费:",
    "alterReminder":"改签后订单无法恢复，是否确认提交改签申请?",
    "importantTips":"重要提示",
    "baggagePrices":"最新超重行李价格",
}
var en = {
    "remarkPop": {
    ...
    "passengerPop": {
        ...
        "documentRemind": "Incorrect filling of document information",
        "countryRemind": "Wrong nationality",
        "nameRemind": "Wrong name",
        "validityRemind": "The validity period is filled in incorrectly",
        'company': 'Company:',
    },
    "newCustomerPop": {
        "nick": "Nick:",
    ...
    "luggageReminder": "In view of the different baggage policy of different airlines, some airlines cancelled the free baggage allowance, passengers have to pay for check-in luggage at the airline or airport. Please refer to airlines webs for detailed baggage policy, or contact service team for more information, thanks.",
    "covidReminder": "As the local COVID-19 prevention and control policies may be adjusted at any time, please refer to the latest local COVID-19 prevention policies and control regulations before departure and make reasonable travel arrangements.",
    "totalAmount": "Total:",
    ...
    "VIPReminder":"Over 5 VIPs have booked current flight, please choose another flight.",
    "alterBtn":"Confirm change",
    "alterChangeFee":"Change fee:",
    "alterReminder":"The booking cannot be recovered after it is changed, please confirm to apply change?",
    "importantTips":" important tips",
    "baggagePrices":" latest excess baggage prices",
}
...
function getPOReminder(){
    var POReminder = ''

    if ( obtLanguage == 'CN' && ProfileInfo.ReminderCnForPObook ){
        POReminder = '<div class="luggageReminder">'+ ProfileInfo.ReminderCnForPObook + '</div>'
    }else if ( ProfileInfo.ReminderEnForPObook ){
        POReminder = '<div class="luggageReminder">'+ ProfileInfo.ReminderEnForPObook + '</div>'
    }
    return POReminder
}
//内容展示
function showContent() {
    ...
    $("#main").html('\
        <div class="autoCenter">\
            ...
            <div class="passengerBody">\
              ...
              <div class="passengerBarLi" style="width:250px;padding-left:45px;">'+ get_lan('passengerPop').ticketName + '</div>\
			...
			<div class="luggageReminder hide bcdReminder baggageReminder">'+ get_lan("luggageReminder") + '</div>'
			+ POReminder +
    ...
    if(ProfileInfo.onlineStyle == 'APAC'){
        $(".progressBar").html('\
            ...
            <div class="progressLine active"></div><span class="activeProgress">'+ get_lan('progressBar').book + '</span>\
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var fareAmountRemind = ""   // 南航提醒
            var airCompanyNotes = ProfileInfo.AirCompanyNotes

            if(ProfileInfo.onlineStyle != "APPLE") {
                airCompanyNotes = airCompanyNotes.filter(function(note){
                    for(var i = 0; i< res.length; i++) {
                        if(note.AirCompanyCode == res[i].AirLineCode){
                            return true
                        }
                    }
                    return false
                })
                if(airCompanyNotes.length > 0) {
                    $(".someAirlineRemind").removeClass("hide");
                    var newReminder = ''
                    airCompanyNotes.map(function(note) {
                        var companyName = obtLanguage === 'CN' ? note.AirCompanyNameCn : (note.AirCompanyNameEn + '\'')
                        var protocalType = note.AirType === '4' ? get_lan('importantTips') : get_lan('baggagePrices')
                        newReminder += '<a href="' + note.AirRemark + '" class="airlineRemind2" target="_blank">' + companyName + protocalType + '</a>、'
                    })
                    newReminder = newReminder.slice(0,newReminder.length-1)
                    $('.someAirlineRemind').append(newReminder)
                }
            }
            res.map(function (item, index) {
                ...
                // 南航提醒
                if (item.Cabins[0].FareAmountRemind) {
                    fareAmountRemind = item.Cabins[0].FareAmountRemind
                }
                var reselection = ProfileInfo.onlineStyle === 'APPLE' ? get_lan('orderDetail').reselection : '[' + get_lan('orderDetail').reselection + ']'
                var orderDetailObj = {
                    resourceType        : item.Cabins[0].ResourceType,
                    ticketAmount        : item.Cabins[0].TotalFare,
                    dateStart           : item.DateStart,
                    title               : item.AirLine,
                    code                : item.AirLineCode,
                    airline             : item.AirLine,
                    flightNo            : item.FlightNo,
                    isShowOperation     : showOperationAirline,
                    operationAirlineText: get_lan('orderDetail').true + ' ' + item.OperationAirline,
                    newOrder            : reselection,
                    timeStart           : item.TimeStart,
                    timeArrive          : item.TimeArrive,
                    duration            : item.Duration,
                    departe             : item.AirportDeparte,
                    arrive              : item.AirportArrive,
                    mealText            : get_lan('orderDetail').meal + ': ',
                    mealValue           : item.Meal,
                    cabinText           : get_lan('orderDetail').cabin + ': ',
                    cabinValue          : item.Cabins[0].Cabin,
                    planeTypeText       : get_lan('orderDetail').planeType + ': ',
                    planeTypeValue      : item.PlaneType,
                    showAmount          : showAmount,
                    fareAmountText      : get_lan('orderDetail').ticketPrice,
                    fareAmountValue     : item.Cabins[0].FareAmount,
                    totalTaxText        : get_lan('orderDetail').tax,
                    totalTaxValue       : item.Cabins[0].TotalTax + ProfileInfo.OfficeCurrency,
                    serviceClass        : serviceFeeDomClass.class + showServiceFare,
                    serviceText         : serviceFeeDomClass.description,
                    serviceValue        : serviceFeeDomClass.fee + ProfileInfo.OfficeCurrency,
                    showSavePrice       : hideSaveFare,
                    currency            : ProfileInfo.OfficeCurrency,
                    luggage             : get_lan('orderDetail').luggage
                }
                
                var o = orderDetailObj

                if ( ProfileInfo.onlineStyle === 'APPLE' ) {
                    $(".orderDetail").append('\
                        <div class="orderDetailBody" ResourceType="'+ o.resourceType + '" ticketAmount="' + o.ticketAmount + '">\
                            <div class="orderDateStart">'+ o.dateStart + ' ' + getWeek(o.dateStart) + '</div>\
                            <div class="orderAirLine" title="'+ o.title + '" code="' + o.code + '">' + o.airline + '</div>\
                            <div class="orderFlightNo">'+ o.flightNo + '</div>\
                            <div class="orderOperationAirline '+ o.isShowOperation + '">' + o.operationAirlineText + '</div>\
                            <div class="orderNewOrder">'+ o.newOrder + '</div>\
                            <div class="orderTimeStart">'+ o.timeStart + '</div>\
                            <div class="orderArrowIcon"></div>\
                            <div class="orderTimeArrive">'+ o.timeArrive + '</div>\
                            <div class="orderDuration">'+ o.duration + '</div>\
                            <div class="orderAirportDeparte">'+ o.departe + '</div>\
                            <div class="orderAirportArrive">'+ o.arrive + '</div>\
                            <div class="orderMeal">'+ o.mealText + o.mealValue + '</div>\
                            <div class="orderCabin">'+ o.cabinText + o.cabinValue + '</div>\
                            <div class="orderPlaneType">'+ o.planeTypeText + o.planeTypeValue + '</div>\
                            <div class="orderFareAmount flexRow '+ o.showAmount + '"><div style="width:120px;">' + o.fareAmountText + '</div><span class="mainFontColor" style="font-size:20px;">' + o.fareAmountValue + o.currency + '</span></div>\
                            <div class="orderTotalTax flexRow '+ o.showAmount + '"><div style="width:120px;">' + o.totalTaxText + '</div><span class="mainFontColor">' + o.totalTaxValue + '</span></div>\
                            <div class="'+ o.serviceClass + '"><div style="width:120px;">' + o.serviceText + '</div><span class="mainFontColor">' + o.serviceValue + '</span></div>\
                            <div class="orderLine"></div>\
                            <div class="orderBottomPrice"></div>\
                            <div class="ordeSavePrice '+ o.showSavePrice + '"></div>\
                            <div class="orderReason ellipsis"></div>\
                        </div>\
                    ')
                } else {
                    // 行李额弹窗
                    var baggage ='', baggageList = item.Cabins[0].BaggageList, baggageDetail = ''
                    if(baggageList.length > 0){
                        for(var b = 0; b < baggageList.length; b++) {
                            var baggageItem = baggageList[b]	// 防止后台更换数据结构
                            if ( parseInt( baggageItem.LuggageTypeCode ) == 1 && 
                                ( baggageItem.BaggageWeightInfo || baggageItem.BaggagePieces ) ) {
                                baggage = baggageItem.BaggageWeightInfo || baggageItem.BaggagePieces
                            }
                            
                            var des = obtLanguage === 'CN' ? baggageItem.DescriptionCN : baggageItem.DescriptionEN, 
                                LuggageType = baggageItem.LuggageType
                            if ( des && LuggageType ) {
                                baggageDetail += '<div class="baggage__pop__detail">' + baggageItem.LuggageType + ':' + des + '</div>'
                            }
                        }
                    }
                    var showBaggage = baggage === '' ? 'hide' : ''
                    if (!baggage) {
                        $('.baggageReminder').show()
                    }
                    var baggagePopUpDom = baggageDetail ? 
                        '<div class="baggage__pop hide"><div class="baggage__wrapper">\
                            <div class="baggage__pop__text">'+get_lan('orderDetail').BaggagePopText+'</div>\
                            <div class="baggage__pop__Details">'
                                + baggageDetail +
                            '</div></div>\
                            <div class="baggage__triangle"></div>\
                        </div>' : ''
                    // 有详细的话，行李额为点击样式
                    var baggageBtn = baggageDetail ? 'baggage__btn' : ''
                    // 苹果的样式 orderMeal, orderCabin, orderPlaneType 没有用到
                    $(".orderDetail").append('\
                        <div class="orderDetailBody" ResourceType="'+ o.resourceType + '" ticketAmount="' + o.ticketAmount + '">\
                            <div class="orderDetailBox__first d-f">\
                                <div class="orderDetail__main d-f fd-c">\
                                    <div class="orderDetail__flightInfo d-f jc-sb">\
                                        <div class="orderDetail__flightInfo__memo d-f fd-c jc-sa">\
                                            <div class="orderDetail__flightInfo__row d-f">\
                                                <div class="orderDateStart">'+ o.dateStart + ' ' + getWeek(o.dateStart) + '</div>\
                                                <div class="orderNewOrder">'+ o.newOrder + '</div>\
                                            </div>\
                                            <div class="orderDetail__flightInfo__row d-f">\
                                        ...
                                        <div class="orderDetail__flightInfo__main d-f ai-c">\
                                            <div class="orderDetail__flightInfo__start d-f fd-c ai-fe">\
                                                <div class="orderTimeStart">'+ o.timeStart + '</div>\
                                                <div class="orderAirportDeparte">'+ o.departe + '</div>\
                                            </div>\
                                            <div class="orderDetail__flightInfo__middle d-f fd-c">\
                                                <div class="orderDuration">'+ o.duration + '</div>\
                                                <div class="orderArrowIcon"></div>\
                                            </div>\
                                            <div class="orderDetail__flightInfo__arrive d-f fd-c ai-fs">\
                                                <div class="orderTimeArrive">'+ o.timeArrive + '</div>\
                                                <div class="orderAirportArrive">'+ o.arrive + '</div>\
                                            </div>\
                                        </div>\
                                    </div>\
                                    <div class="orderDetail__lowestTicket d-f">\
                                        <div class="orderBottomPrice"></div>\
                                        <div class="ordeSavePrice '+ o.showSavePrice + '"></div>\
                                        <div class="orderReason ellipsis"></div>\
                                    </div>\
                                </div>\
                                <div class="orderDetail__memo d-f ai-fe">\
                                    <div class="orderDetail__memo__textColumn d-f fd-c ai-fe">\
                                        <div class="orderDetail__memo__text">' + o.mealText + '</div>\
                                        <div class="orderDetail__memo__text">' + o.cabinText + '</div>\
                                        <div class="orderDetail__memo__text">' + o.planeTypeText + '</div>\
                                        <div class="orderDetail__memo__text baggage '+showBaggage+'">' + o.luggage + '</div>\
                                    </div>\
                                    <div class="orderDetail__memo__valueColumn d-f fd-c ai-fs">\
                                        <div class="orderDetail__memo__value">' + o.mealValue + '</div>\
                                        <div class="orderDetail__memo__value">' + o.cabinValue + '</div>\
                                        <div class="orderDetail__memo__value">' + o.planeTypeValue + '</div>\
                                        <div class="baggage '+showBaggage+'" style="position:relative;">' + baggagePopUpDom + 
                                            '<div class="orderDetail__memo__value '+baggageBtn+'">' + baggage + '</div>\
                                        </div>\
                                    </div>\
                                </div>\
                            </div>\
                            <div class="orderDetailBox__last d-f ai-fe">\
                                <div class="orderDetail__price__textColumn d-f fd-c ai-fs">\
                                    <span class="orderDetail__price__text">' + o.fareAmountText + '</span>\
                                    <span class="orderDetail__price__text">' + o.totalTaxText + '</span>\
                                    <span class="orderDetail__price__text">' + o.serviceText + '</span>\
                                </div>\
                                <div class="orderDetail__price__valueColumn d-f fd-c ai-fe">\
                                    <span class="orderDetail__price__value orderDetail__fareAmount"><b>' + o.fareAmountValue + '</b> ' + o.currency + '</span>\
                                    <span class="orderDetail__price__value">' + o.totalTaxValue + '</span>\
                                    <span class="orderDetail__price__value">' + o.serviceValue + '</span>\
                                </div>\
                            </div>\
                        </div>\
                    ')
                    // 点击行李额显示详细
                    $('.baggage__btn').click(function(e){
                        $('.baggage__pop').addClass('hide')
                        var baggagePop = $(this).siblings('.baggage__pop')
                        var height = baggagePop.height(), width = baggagePop.width()
                        var top = (0-parseInt(height)) + 'px', left = parseInt(( 0 - parseInt(width) ) / 2) + 'px'
                        baggagePop.removeClass('hide').css('top',top).css('left',left)
                        $('.baggage__triangle')
                            .css('top',parseInt(height) +'px')
                            .css('left', (parseInt(width) + parseInt(( 0 - parseInt(width) ) / 2)) + 'px')
                        stopBubble(e)
                    })
                    $(document).click(function(e){
                        if(!$(e.target).hasClass('baggage__btn')){
                            $('.baggage__pop').addClass('hide')
                        }
                    })
            ...
            // 南航提醒
            if (fareAmountRemind) {
                $('.covidReminder').before('<div class="luggageReminder">' + fareAmountRemind + '</div>')
            }
            //选择的航班和舱位信息
            ...
            $(".orderDetail").attr("DeparteAirport", res[0].AirportDeparte);
            $(".orderDetail").attr("ArriveAirport", res[0].AirportArrive);
            ...
            var citys = '';
            res.map(function (item) {
                citys += item.DeparteCityCN + ',' + item.ArriveCityCN + ',';    // nina要求改成中文
                    ...
                    $.ajax(
                        {
                            ...
                            success: function (data) {
                                ...
                                if (searchDomInfo.alterTicketInfo && !$.session.get('goOnBookOrderNo')) {
                                    ...
                                    var companyID = searchDomInfo.alterTicketInfo.split(',')[2];
                                    $(".remarkFooter").html('\
                                            ...
                                            success: function (data) {
                                                ...
                                                if (res.Message) {
                                                    ...
                                                    res.Customers.map(function (item, index) {
                                                        ...
                                                        var changePassengerInfo = ProfileInfo.onlineStyle === 'APPLE' ? get_lan('passengerInfo').changePassengerInfoApple : get_lan('passengerInfo').changePassengerInfo
                                                ...
                                                <div class="passengerLiDiv" style="width:250px;"><span class="PassengerNameText">'+ passengerName + '</span><span class="changePassengerInfo" index="' + index + '" customerId="' + item.CustomerID + '" style="margin-left:5px;cursor:pointer;">' + changePassengerInfo + '</span></div>\
                                ...
                                function __initIfTravelRequest(passengerID) {
                                    ...
                                    if (domTicketInfo.type == "oneWay") {
                                        var queryKey = passengerID + ',1,' + $(".orderDetail").attr("DeparteAirport") + ',' + $(".orderDetail").attr("ArriveAirport") + ',' + $(".orderDetail").attr("DateStart") + ',';
                                    }
                                    else if ((domTicketInfo.type == "roundTrip" || domTicketInfo.type == "multiple")) {
                                        var queryKey = passengerID + ',1,' + $(".orderDetail").attr("DeparteAirport") + ',' + $(".orderDetail").attr("ArriveAirport") + ',' + $(".orderDetail").attr("DateStart") + ',' + $(".orderDetail").attr("DateReturn");
                                    }
                                    if (searchDomInfo.type == "oneWay") {
                                        city = ""
                                    }
...
function selectPassengers() {
    ...
    function clickPassengerLi() {
        $(".selectPassengerLi").unbind("click").click(function () {
            ...
            $(".selectPassengerList").css("display", "none");
            if (domTicketInfo.type == "oneWay") {
                var queryKey = $(this).attr("searchId") + ',1,' + $(".orderDetail").attr("DeparteAirport") + ',' + $(".orderDetail").attr("ArriveAirport") + ',' + $(".orderDetail").attr("DateStart") + ',';
            }
            else if (domTicketInfo.type == "roundTrip" || domTicketInfo.type == "multiple") {
                var queryKey = $(this).attr("searchId") + ',1,' + $(".orderDetail").attr("DeparteAirport") + ',' + $(".orderDetail").attr("ArriveAirport") + ',' + $(".orderDetail").attr("DateStart") + ',' + $(".orderDetail").attr("DateReturn");
            }
...
function remark(remarks, CustomerID, CompanyID, isFirst, swRemark) {
    ...
    function switchRemarkType(dom) {
        if ($(dom).find("option:selected").attr("key")) {
            ...
            // 把选中的项目内有子规则的都按照默认的重新渲染一遍，再渲染sub内的，再把sub对应的赋值到老容器
            //渲染默认值
            if (subRuleTemp.Index != 0) {
                remarks[selectIndex].RelatedRemarkList.map(function (subRemark, index) {
                    remarks.map(function (remark) {
                        if (remark.Index == subRemark.SubRemarkIndex) {
                            subRuleTemp = {
                                Index: remark.Index,
                                Input: remark.Input,
                                CanModify: remark.CanModify
                            }
                        }
                    })
                    renderSingleRemark(subRuleTemp);
                })
            }
            remarks[selectIndex].RelatedRemarkList.map(function (rItem) {
                renderSubRemarks(rItem, remarks, selectKey);
            })
    ...
    function renderSubRemarks(rItem, remarks, selectKey) {
        if (rItem.ChooseMainValue == selectKey) {
            //选中有子规则的值的情况下
            //渲染sub
            var subRule = {
                Index: rItem.SubRemarkIndex,
                Input: "",
                CanModify: "false"
            }
            rItem.SubRemarkRuleList.map(function (sItem) {
                switch (sItem.SubRemarkRule) {
                    case 1: subRule.Input = sItem.SubRemarkValue; break;
                    case 2: subRule.CanModify = sItem.SubRemarkValue; break;
                }
            })
            renderSingleRemark(subRule);
            //找到sub对应的remark值，赋值到老容器
            remarks.map(function (remark) {
                if (remark.Index == subRule.Index) {
                    subRuleTemp = {
                        Index: remark.Index,
                        Input: remark.Input,
                        CanModify: remark.CanModify
    ...
    function renderSingleRemark(rule) {
        var ruleIndex = rule.Index;
        var ruleInput = rule.Input;
        var ruleCanModify = rule.CanModify;
        $("#remarkInput" + ruleIndex + "").val("");
        $("#remarkInput" + ruleIndex + "").removeAttr("key");
        // console.log(sItem)
        var colorRed = ruleInput.indexOf("4") != -1 || ruleInput == "" ? "" : "colorRed";
        if (colorRed == "") {
            $("#liTittle" + ruleIndex + "").removeClass("colorRed");
            $("#remarkInput" + ruleIndex + "").attr("require", "");
        } else if (colorRed == "colorRed") {
            $("#liTittle" + ruleIndex + "").addClass("colorRed");
            $("#remarkInput" + ruleIndex + "").attr("require", "colorRed");
        }

        if (ruleCanModify == "true") {
            // $("#remarkInput"+ruleIndex+"").attr("placeholder",get_lan("remarkPop").search);
            $("#remarkInput" + ruleIndex + "").removeAttr("disabled");
            // $("#remarkSelect" + ruleIndex + "").show();
            // $("#remarkInput"+ruleIndex+"").searchRemark();
            // 12.13新增
            $("#remarkInput" + ruleIndex + "").prev().removeAttr("disabled");

            var remarkObj = {}
            remarks.map(function (remarkList) {
                if (remarkList.Index == ruleIndex) {
            ...
            if (remarkObj.InList) {
                ...
                $("#remarkInput" + ruleIndex + "").attr("placeholder", "");
            }
        ...
        } else if (ruleCanModify == "false") {
            $("#remarkInput" + ruleIndex + "").attr("placeholder", "");
            $("#remarkInput" + ruleIndex + "").attr("disabled", "disabled");
            // $("#remarkSelect" + ruleIndex + "").hide();
            // 12.13新增
            $("#remarkInput" + ruleIndex + "").prev().attr("disabled", "disabled");
        }
    }
    // 重新渲染 右侧select
    // function renderRight(renderIndex,rule,swRemark){
    function renderRight(renderIndex, rule) {
        var item = ''
        ...
        $('.liContent').map(function (liItem) {
            ...
            if (liIndex == item.Index) {
                ...
                var colorRed = rule.Input.indexOf("4") != -1 || rule.Input == "" ? "" : "colorRed";
                if (item.InList) {
                    if (!item.ListCanSearch) {
                        $(this).html('\
				                    <select class="remarkSelect '+ newSelect + '" index="' + index + '" id="remarkSelect' + item.Index + '">\
...
function passengerPop() {
    $(".PassengerPop").html('\
        ...
        <div class="popChooseName">'
            + get_lan('passengerPop').chooseName + 
        //   '<span class="bottomColor">' + get_lan('passengerPop').nameRemind + '</span>' +
        '</div>\
...
function newCustomerPop(CompanyId) {
    ...
    $(".newCustomerPopBtn").unbind("click").click(function () {
        ...
        else if ($(".newCustomerInputSurNameCn").val() == "" || $(".newCustomerInputGivenNameCn").val() == "" || $(".newCustomerInputSurNameEn").val() == "" || $(".newCustomerInputGivenNameEn").val() == "") {
            alert(get_lan('passengerPop').nameRemind);
        } else if ($(".newCustomerInputDocuments").val() == "") {
            alert(get_lan('passengerPop').documentRemind);
        } else if (!$("#countryInput").attr("code")) {
            alert(get_lan('passengerPop').countryRemind);
        } else if ($('.newPopDocumentsSelect option:selected').val() != 1 && $(".newCustomerInputTime ").val() == "") {
            alert(get_lan('passengerPop').validityRemind);
        }
        else {
            ...
            $.ajax(
                {
                    ...
                    data: {
                        ...
                        jsonStr: '{"request":{"language":"' + obtLanguage + '","id":' + netUserId + ',"nameCn":"' + NameCN + '","nameEn":"' + NameEN + '","nickname":"' + Nick + '","gender":"' + sex + '","nation":"' + nationality + '","birthday":"' + birthday + '","NameS":"' + NameCN + '","email":"' + email + '","phoneNumber":"' + phone + '","groupIDs":"0","companyId":"' + companyId + '"}}'
                    },
                    success: function (data) {
                        var res = JSON.parse(data);
                        console.log(res);
    ...
    $.ajax(
        {
            ...
            success: function (data) {
                ...
                if(res.length === 1 ) {
                    // scheme： 代订其他公司的，改签单人，订单公司改成被代订人的
                    $('.orderDetail').attr('companyId', res[0].CompanyID)
                }
                res.map(function (item, index) {
            ...
            success: function (data) {
                ...
                res.map(function (item, index) {
                    ...
                    <div class="passengerLi flexRow" customerId="'+ item.ID + '" companyId="' + item.OrderCompanyId + '">\
                        <div class="passengerLiDiv" style="width:250px;text-align:left;padding-left:45px;box-sizing:border-box;"><span class="PassengerNameText">'+ passengerName + '</span><span class="changePassengerInfo" index="' + index + '" customerId="' + item.ID + '" style="margin-left:5px;cursor:pointer;">' + changePassengerInfo + '</span></div>\
                ...
                if (searchDomInfo.alterTicketInfo) {
                    ...
                    $(".delIcon").remove();
                    if(ProfileInfo.onlineStyle !== "APPLE"){
                        $('.bookTicketBtn').text(get_lan('alterBtn'));
                        $('.orderFareAmount div').text(get_lan('alterChangeFee'));
                    }
                }
                if ($.session.get("TAnumber")) {
                    $(".delIcon").remove();
...
function totalAmount() {
    ...
    initChart(total + insurancePrice);
...
function clickBookBtn() {
    $(".bookTicketBtn").unbind("click").click(function () {
        if (searchDomInfo.alterTicketInfo && ProfileInfo.onlineStyle !== "APPLE") {
            if(!confirm(get_lan('alterReminder'))){
                return false;
            }
        }
        if(ProfileInfo.SameFlighVIPReminder && 
        ["CA1547","CA1861","HO1251"].indexOf($('.orderFlightNo').text()) >= 0){
            alert(get_lan('VIPReminder'));
            return false;
        }
        if ($(".passengerLi").length == 0) {
            ...
            if ($.session.get('TAnumber') && !$.session.get('goOnBookOrderNo')) {
