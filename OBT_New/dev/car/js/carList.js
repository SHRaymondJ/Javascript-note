...
function carListInfo(res){
    ...
    res.map(function(item,index){
        ...
        var showPolicy = item.PolicyType == 3?"":"hide";
        $(".carList").append('\
            <div class="carLi">\
                ...
                <div class="outOfPolicy '+showPolicy+'">'+get_lan("carBody").outOfPolicy+'</div>\
	...
	$(".expenseDetails").unbind("click").click(function(){
		...
		tools.ajax(option,function(data){
			...
			if(res.FeeInfoList.length>0){
                var secondCurrency = res.FeeInfoList[0].FeeList.length > 1 
                    ? '<td>Amount<br><span>('+res.FeeInfoList[0].FeeList[1].CurrencyCode+')</span></td>'
                    : ''
				hideExtra='show'
				//额外费用
				ExtraStr='<table class="detaileTable" border="0" cellspacing="0" cellpadding="0"><tr>\
						<td>Amount<br><span>('+res.FeeInfoList[0].FeeList[0].CurrencyCode+')</span></td>'+secondCurrency+'<td>Description</td>\
					</tr>'
					
				res.FeeInfoList.map(function(item){
                    var secondAmount = item.FeeList.length > 1
                        ? '<td>'+item.FeeList[1].Amount+'</td>'
                        : ''
					ExtraStr=ExtraStr+'<tr>\
						<td>'+item.FeeList[0].Amount+'</td>'+secondAmount+'<td>'+item.Des+'</td>\
					</tr>'
				})
