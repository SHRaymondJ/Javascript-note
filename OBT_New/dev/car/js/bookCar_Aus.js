...
var en = {
    ...
    "Notice":"Notice",
    "carReason":"Choose Reason:",
    "pleaseSelect":"Please Select",
    "carReasonRemind":"Please select reason",
    "bookCar":"Book",
	...
	$("#main").html('\
        <div class="autoCenter">\
            ...
            <div class="creditCardInfo hide">\
              <div class="creditCardCheckBody">\
                <input type="checkbox" class="checkCreditCard"><span style="font-size:14px;">'+get_lan('creditCardInfo').guarantee+'</span>\
              </div>\
              <div class="creditCardTittle hide">'+get_lan('creditCardInfo').creditCardTittle+'</div>\
              <div class="creditCardBody flexWrap hide">\
                <div class="creditCardName flexRow" style="width:768px;">\
            ...
            <div class="chooseReasonBody hide">\
                <span class="carReasonText">' +get_lan('carReason') +'</span>\
                <select class="carReasonSelect">\
                  <option value="">'+get_lan("pleaseSelect")+'</option>\
                </select>\
            </div>\
            ...
            <div class="fareBody">\
                ...
                <div class="totalFare">'+get_lan('carBody').stroeMoney+'<span class="totalFareText" style="font-size:24px;font-weight:600;color:#FF6720;"></span></div>\
            </div>\
          </div>\
    ')
            ...
            res.CarModelList.map(function(item,index){
                ...
                $(".orderDetail").append('\
                    <div class="orderDetailBody">\
                        ...
                        <div class="pickUp">'+get_lan("carBody").pickUp+'</div>\
                        <div class="pickOff">'+get_lan("carBody").pickOff+'</div>\
                        ...
                        <div class="pickUpTime">'+searchCarInfo.date.split(' ')[1]+'</div>\
                        <div class="pickOffTime">'+searchCarInfo.returndate.split(' ')[1]+'</div>\
                        <div class="pickUpDate">'+searchCarInfo.date.split(' ')[0].split('-')[1]+'-'+searchCarInfo.date.split(' ')[0].split('-')[2]+' '+getWeek(searchCarInfo.date.split(' ')[0])+'</div>\
                        <div class="pickOffDate">'+searchCarInfo.returndate.split(' ')[0].split('-')[1]+'-'+searchCarInfo.returndate.split(' ')[0].split('-')[2]+' '+getWeek(searchCarInfo.returndate.split(' ')[0])+'</div>\
                        <div class="pickUpCity">'+searchCarInfo.departureCityText+'</div>\
                        <div class="pickOffCity">'+searchCarInfo.arrivalCityText+'</div>\
                    </div>\
                ')
                $(".fareBodyDetail").append('\
                    <div class="orderFareAmount flexRow"><div style="width:200px;">'+ get_lan("carBody").price + '</div><span style="color:#111C4E;font-weight:600;">' + item.DailyRate+item.CurrencyCode+ '</span></div>\
                ')
...
function needCreditCard(){
        ...
        $(".checkCreditCard").change(function(){
            if($(this).is(":checked")){
                $(".creditCardBody,.creditCardTittle").removeClass("hide");
            }else{
                $(".creditCardBody,.creditCardTittle").addClass("hide");
            }
        })
        $.ajax(
          {
    ...
    $.ajax(
      {
        ...
        success : function(data) {
						...
						<div style="height:130px;width:100%" class="flexRow">\
							<div style="height:130px;width:676px;box-sizing:border-box;font-size:14px;" class="flexWrap">\
								<div class="passengerLiName" style="width:300px;height:60px;">\
									...
									<div style="color:#041E5B;height:30px;line-height:30px;"><span class="PassengerNameText">' +passengerName + '</span><span class="changePassengerInfo" index="' + index +'" customerId="' + item.ID + '">' + get_lan('passengerInfo').changePassengerInfo +'</span></div>\
								</div>\
            ...
            $.ajax(
                ...
                dataType : 'json',
                data:{
                    url: $.session.get('obtCompany') + "/QueryService.svc/CheckCarCanBookPost",
                    jsonStr: '{"request":{"Uid":'+netUserId+',"Rph":"'+ selectCarInfo.Rph +'","Language":"' + obtLanguage + '"}}'
                },
                success : function(data) {
                    var res = JSON.parse(data);
                    console.log(res);
                    if(res.ReasonCodes.length>0){
                        res.ReasonCodes.map(function(item){
                            if(item.NoShow){
                                $(".chooseReasonBody").attr("reasonCode",item.Code);
                            }else{
                                $(".chooseReasonBody").removeClass("hide");
                                $(".carReasonSelect").append('\
                                <option value="'+item.Code+'">'+item.Des+'</option>\
                                ')
                            }
                        })
                    }else{
                        $(".chooseReasonBody").attr("reasonCode","");
                    }
                },
                error : function() {
                    // alert('fail');
...
function clickReserveBtn(){
    $(".bookCarBtn").unbind("click").click(function(){
        ...
        $('body').mLoading("show");
        if($(".chooseReasonBody").hasClass("hide")){
            var ReasonCode = $(".chooseReasonBody").attr("reasonCode");
        }else{
            var ReasonCode = $(".carReasonSelect").val();
            if(ReasonCode==""){
                alert(get_lan("carReasonRemind"));
                $('body').mLoading("hide");
                return false;
            }
        }
        var bookOption = {
            url:$.session.get('ajaxUrl'),
            data: {
                url: $.session.get('obtCompany') + "/OrderService.svc/BookCarNewPost",
                jsonStr: '{"request":{"ReasonCode":"'+ReasonCode+'","Language":"' +obtLanguage + '","QueryKey":"' +selectCarInfo.Rph + '","Uid":' +netUserId + ',"CreditCard":"' +creditCard + '"}}'
            },
        }
	        ...
	        success : function(data) {
				...
				res.DayHourList.map(function(item){
					if(item.HourCurrencyCode!='CNY'){
						...
						aboutMoney=get_lan("carBody").about + ' ' + item.TotalRate+item.HourCurrencyCode;
					}
				})
                $(".totalFareText").text(storeMoney);
				// $(".totalFareBody").html('\
				//     <span style="font-size:14px;color:#666;margin-right:10px;">'+get_lan("carBody").stroeMoney+':</span><span style="">'+storeMoney+'</span>\
				// 	<div style="height: 20px;line-height: 20px;text-align: right;"><span style="font-size:14px;color:#666666;line-height:20px">'+aboutMoney+'</span></div>\
				// ')
			}
	})
}
