...
// $.session.set('TAnumber',"TPWG-A2106-0003");
...
function searchBody() {
	...
	if (ProfileInfo.onlineStyle === 'BCD') {
		var openMapHotel = '<div class="openMapHotelBox">\
	...
	'</div>\
		...
		'</div>\
                    <div class="intlDateBody flexRow">\
                      ...
                      <select type="text" id="intlDepartureSelect" onchange="GrayIntelDepartPlusMinus()">\
                        <option value="all" class="intlAllDay">' +
		get_lan("searchBody").allDay +
		'</option>\
                        <option value="0">0:00</option>\
		...
		'</div>\
                    <div class="intlDateBody intlReturnDateBody flexRow">\
                      ...
                      <select type="text" id="intlReturnSelect" onchange="GrayIntelreturnPlusMinus()">\
                        <option value="all" class="intlAllDay">' +
		get_lan("searchBody").allDay +
		'</option>\
                        <option value="0">0:00</option>\
		...
		'</div>\
                <input type="text" id="hotelPrice" value="0-5000'+ProfileInfo.OfficeCurrency+'" minPrice="0" maxPrice="5000" autocomplete="off">\
	 ...
	 <div class="remindCover" style="position: fixed;top: 0;left: 0;bottom: 0;right: 0;background: rgba(0, 0, 0, 0.7);z-index: 9999;">\
	       <div class="remindPop" style="width: 1000px;height: 270px;background-color: #fff;z-index: 101;position: fixed;top: 50%;left: 50%;transform: translate(-50%,-50%);border-radius: 10px;padding: 10px;font-family: Sans-serif,Arial,Verdana;">\
	         ...
	         <div style="box-sizing: border-box;padding:10px;font-size: 15px;line-height:24px;">\
			   <p>Please review the <a href="https://apple.ent.box.com/folder/129602926863?s=e1cld3k5drfc9yc4duwkxolehwuw9p4d">China Hotel Check-in Policy</a>  to confirm hotel restrictions before traveling.</p>\
			   <br><p>For details on the latest travel restrictions and entry requirements for cities, please refer to <a href="https://fmdbtravel.corp.apple.com/resumption-tracker">Travel and Meetings Resumption Tracker</a>.</p>\
	         </div>\
	       </div>\
	     </div>\
	     '
	...
	for (var i = 0; i < $(".domMultipleDepartureDate").length; i++) {
		if (i == $(".domMultipleDepartureDate").length - 2) {
			...
			if (TAnumber != undefined && TAnumber != "") {
				...
				var minTime2
				// var travelObj =JSON.parse($.session.get('travelObj'));
				var TALimitDateObj = JSON.parse($.session.get('TALimitDateObj'));
				console.log(TALimitDateObj);
				TAminDate = TALimitDateObj.airStartDate==""?TALimitDateObj.allStartDate:TALimitDateObj.airStartDate;
				TAmaxDate = TALimitDateObj.airEndDate==""?TALimitDateObj.allEndDate:TALimitDateObj.airEndDate;
				if(TAminDate==0){
			...
			function searchTrain() {
				if ($(".searchTrainBtn").attr("state") == "oneWay") {
					...
					var searchTrainInfo = {
						...
						'departureCN': $('#trainDepartureCity').attr("cn"),
						'arrivalCN': $('#trainArrivalCity').attr("cn"),
						'date': $('#trainDepartureDate').val(),
						'queryKey': $('#trainDepartureCity').attr('cn') + ',' + $('#trainArrivalCity').attr('cn') + ',' + $(
								'#trainDepartureDate')
					...
					var searchTrainInfo = {
						...
						'queryKey': $('#trainDepartureCity').attr('cn') + ',' + $('#trainArrivalCity').attr('cn') + ',' + $('#trainDepartureDate')
							.val() + DepartureSelectValue+',' + $("#trainCabin").val(),
						'queryKeyReturn': $('#trainArrivalCity').attr('cn') + ',' + $('#trainDepartureCity').attr('cn') + ',' + $(
									...
									success: function (data) {
										...
										res.map(function(otherItem){
											...
											$(".orderDetailsTable").eq(index).append(
												...
												'</td>\
												  <td></td>\
												  <td>' + otherItem.ProductName +
												'</td>\
												  <td>' +otherItem.FarePaid+otherItem.Currency+
												'</td>\
												</tr>\
											')
										})
				...
				if (data == '' || data == "[]") {
					...
					resArr.map(function(item){
						switch (item.serviceType) {
							case "0":
								allStartList.push(new Date(item.StartTime).getTime());
								allEndList.push(new Date(item.EndTime).getTime());
								break;
							case "1":
								airStartList.push(new Date(item.StartTime).getTime());
								airEndList.push(new Date(item.EndTime).getTime());
								break;
							case "2":
								hotelStartList.push(new Date(item.StartTime).getTime());
								hotelEndList.push(new Date(item.EndTime).getTime());
								break;
							case "3":
								carStartList.push(new Date(item.StartTime).getTime());
								carEndList.push(new Date(item.EndTime).getTime());
								break;
							case "4":
								trainStartList.push(new Date(item.StartTime).getTime());
								trainEndList.push(new Date(item.EndTime).getTime());
								break;
						}
					})
					var allStartDate = formatDate(getMaximin(allStartList,"min"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(allStartList,"min"));
					var allEndDate = formatDate(getMaximin(allEndList,"max"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(allEndList,"max"));
					var airStartDate = formatDate(getMaximin(airStartList,"min"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(airStartList,"min"));
					var airEndDate = formatDate(getMaximin(airEndList,"max"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(airEndList,"max"));
					var hotelStartDate = formatDate(getMaximin(hotelStartList,"min"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(hotelStartList,"min"));
					var hotelEndDate = formatDate(getMaximin(hotelEndList,"max"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(hotelEndList,"max"));
					var carStartDate = formatDate(getMaximin(carStartList,"min"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(carStartList,"min"));
					var carEndDate = formatDate(getMaximin(carEndList,"max"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(carEndList,"max"));
					var trainStartDate = formatDate(getMaximin(trainStartList,"min"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(trainStartList,"min"));
					var trainEndDate = formatDate(getMaximin(trainEndList,"max"))=="NaN-NaN-NaN"?"":formatDate(getMaximin(trainEndList,"max"));
					var TALimitDateObj = {
					...
					console.log(TALimitDateObj);
					$.session.set('TALimitDateObj',JSON.stringify(TALimitDateObj));
					domMultipleDepartureDate();
					...
					function setDefaultCity(res){
						...
						// $.session.set('travelObj',JSON.stringify(travelObj));
						MultipleDepartureDate()
						...
						if (travelObj.type && (serviceType == 0 || serviceType == 1) && !domAirFlag) {
							...
							// setTimeout(function(){
							// 	$('#domDepartureDate').val(Dateformat(travelObj.startTime,0))
							// 	$('#domReturnDate').val(Dateformat(travelObj.startTime,1))
							// 	limitTime("#domDepartureDate",travelObj.startTime,travelObj.endTime)
							// 	limitTime("#domReturnDate",travelObj.startTime,travelObj.endTime)
							// },100)
						} else {
							// 国际
							if ((serviceType == 0 || serviceType == 1) && !intlAirFlag && !travelObj.type) {
								...
								setTimeout(function() {
									// $('#intlDepartureDate').val(Dateformat(travelObj.startTime,0))
									// $('#intlReturnDate').val(Dateformat(travelObj.startTime,1))
									// limitTime("#intlDepartureDate",travelObj.startTime,travelObj.endTime)
									// limitTime("#intlReturnDate",travelObj.startTime,travelObj.endTime)
								}, 100)
							}
						}
						// 酒店
						if ((serviceType == 0 || serviceType == 2)&& !hotelFlag) {
							...
							$('#hotelCity').attr('cityCode', travelObj.leaveCityCode)
							$('#hotelCity').val($.session.get('obtLanguage') == "CN" ? travelObj.leaveCityCN : travelObj.leaveCityEN)
							setTimeout(function() {
								// $('#hotelDepartureDate').val(Dateformat(travelObj.startTime,0))
								// $('#hotelReturnDate').val(Dateformat(travelObj.startTime,1))
								// limitTime("#hotelDepartureDate",travelObj.startTime,travelObj.endTime)
								// limitTime("#hotelReturnDate",travelObj.startTime,travelObj.endTime)
							}, 100)
							// 是否是国际
							if(!travelObj.type){
						...
						if (travelObj.type && (serviceType == 0 || serviceType == 4) && !trainFlag) {
							...
							setTimeout(function() {
								// $('#trainDepartureDate').val(Dateformat(travelObj.startTime,0))
								// $('#trainReturnDate').val(Dateformat(travelObj.startTime,1))
								// limitTime("#trainDepartureDate",travelObj.startTime,travelObj.endTime)
								// limitTime("#trainReturnDate",travelObj.startTime,travelObj.endTime)
							}, 100)
						}
						...
						setTimeout(function() {
							/*2021-7-7*/
							var allStartDate = TALimitDateObj.allStartDate;
							var allEndDate = TALimitDateObj.allEndDate;
							var airStartDate = TALimitDateObj.airStartDate;
							var airEndDate = TALimitDateObj.airEndDate;
							var hotelStartDate = TALimitDateObj.hotelStartDate;
							var hotelEndDate = TALimitDateObj.hotelEndDate;
							var carStartDate = TALimitDateObj.carStartDate;
							var carEndDate = TALimitDateObj.carEndDate;
							var trainStartDate = TALimitDateObj.trainStartDate;
							var trainEndDate = TALimitDateObj.trainEndDate;
							/*国内票*/
							if(airStartDate!=""){
								$('#domDepartureDate').val(Dateformat(airStartDate, 0))
								$('#domReturnDate').val(Dateformat(airStartDate, 1))
								limitTime("#domDepartureDate", airStartDate, airEndDate,"#domReturnDate")
							}else if(allStartDate!=""){
								$('#domDepartureDate').val(Dateformat(allStartDate, 0))
								$('#domReturnDate').val(Dateformat(allStartDate, 1))
								limitTime("#domDepartureDate", allStartDate, allEndDate,"#domReturnDate")
							}else{
								$('#domDepartureDate').val('').attr("disabled","disabled");
								$('#domReturnDate').val('').attr("disabled","disabled");
							}
							/*国际票*/
							if(airStartDate!=""){
								$('#intlDepartureDate').val(Dateformat(airStartDate, 0))
								$('#intlReturnDate').val(Dateformat(airStartDate, 1))
								limitTime("#intlDepartureDate", airStartDate, airEndDate,"#intlReturnDate")
							}else if(allStartDate!=""){
								$('#intlDepartureDate').val(Dateformat(allStartDate, 0))
								$('#intlReturnDate').val(Dateformat(allStartDate, 1))
								limitTime("#intlDepartureDate", allStartDate, allEndDate,"#intlReturnDate")
							}else{
								$('#intlDepartureDate').val('').attr("disabled","disabled");
								$('#intlReturnDate').val('').attr("disabled","disabled");
							}
							/*酒店*/
							if(hotelStartDate!=""){
								$('#hotelDepartureDate').val(Dateformat(hotelStartDate, 0))
								$('#hotelReturnDate').val(Dateformat(hotelStartDate, 1))
								limitTime("#hotelDepartureDate", hotelStartDate, hotelEndDate,"#hotelReturnDate")
								limitTime("#hotelReturnDate", hotelStartDate, hotelEndDate)
							}else if(allStartDate!=""){
								$('#hotelDepartureDate').val(Dateformat(allStartDate, 0))
								$('#hotelReturnDate').val(Dateformat(allStartDate, 1))
								limitTime("#hotelDepartureDate", allStartDate, allEndDate,"#hotelReturnDate")
								limitTime("#hotelReturnDate", allStartDate, allEndDate)
							}else{
								$('#hotelDepartureDate').val('').attr("disabled","disabled");
								$('#hotelReturnDate').val('').attr("disabled","disabled");
							}
							/*火车*/
							if(trainStartDate!=""){
								$('#trainDepartureDate').val(Dateformat(trainStartDate, 0))
								$('#trainReturnDate').val(Dateformat(trainStartDate, 1))
								limitTime("#trainDepartureDate", trainStartDate, trainEndDate,"#trainReturnDate")
							}else if(allStartDate!=""){
								$('#trainDepartureDate').val(Dateformat(allStartDate, 0))
								$('#trainReturnDate').val(Dateformat(allStartDate, 1))
								limitTime("#trainDepartureDate", allStartDate, trainEndDate,"#trainReturnDate")
							}else{
								$('#trainDepartureDate').val('').attr("disabled","disabled");
								$('#trainReturnDate').val('').attr("disabled","disabled");
							}
							/*end*/
							// $('#domDepartureDate').val(Dateformat(travelObj.startTime, 0))
							// $('#domReturnDate').val(Dateformat(travelObj.startTime, 1))
							// limitTime("#domDepartureDate", travelObj.startTime, travelObj.endTime,"#domReturnDate")
						
							// $('#intlDepartureDate').val(Dateformat(travelObj.startTime, 0))
							// $('#intlReturnDate').val(Dateformat(travelObj.startTime, 1))
							// limitTime("#intlDepartureDate", travelObj.startTime, travelObj.endTime,"#intlReturnDate")
						
							// $('#hotelDepartureDate').val(Dateformat(travelObj.startTime, 0))
							// $('#hotelReturnDate').val(Dateformat(travelObj.startTime, 1))
							// limitTime("#hotelDepartureDate", travelObj.startTime, travelObj.endTime,"#hotelReturnDate")
							// // limitTime("#hotelReturnDate", travelObj.startTime, travelObj.endTime)
						
							// $('#trainDepartureDate').val(Dateformat(travelObj.startTime, 0))
							// $('#trainReturnDate').val(Dateformat(travelObj.startTime, 1))
							// limitTime("#trainDepartureDate", travelObj.startTime, travelObj.endTime,"#trainReturnDate")
						}, 100)
