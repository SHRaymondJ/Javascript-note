...
var SEARCH = tools.queryString().search;
			...
			<div class="travelRequestPop">\
				...
				<div class="travelRequestListBox">\
					<div class="travelList">\
						' + travelApplication + '\
						<span class="TAselectedCustomer" companyid="" customerid="" employeename=""></span>\
					</div>\
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (res.message || res.customerTRs.length == 0) {
					$(".travelApplicationList").append('<div class="applicationLi">' + applicationRemind + '</div>')
				} else if (res.customerTRs.length != 0) {
					res.customerTRs.map(function (item) {
						$(".travelApplicationList").append('\
							<div class="applicationLi flexRow bigger">\
								...
								<div style="margin-left:20px;">' + item.CityInfos[0].OrgCity + '~' + item.CityInfos[0].DstCity + '</div>\
	...
	TAchooseCustomer('', function (data) {
		...
		if (res.length > 1) {
			...
			$(".TAchoosePassenger").html('\
				<div class="TAchoosePassenger-wrapper hide">\
					' + customer + '\
...
function showContent() {
	...
	if (ProfileInfo.onlineStyle == "APPLE") {
		var TitleStr = '<div class="pendTab mainFontColor hide">' + get_lan('table').pendingApproval + '</div><div class="ApproveLengthIcon hide"></div>'
	} else {
		var TitleStr = '<div class="pendTab mainFontColor hide">' + get_lan('table').pendingApproval + '<div class="ApproveLengthIcon hide"></div></div>'
	}
	...
	if (ProfileInfo.HideQRCode) {
		...
		var appDownloadBoxHtml =
			'<div class="QRgroup l" id="app_download_box">\
	...
	$("#main").html('\
        <article>\
            ...
            <div class="orderTittle flexRow autoCenter">\
                <div class="myOrderTab orderTittleActive">' + get_lan('table').myOrders + '</div>\
				' + TitleStr + '\
	...
	if (ProfileInfo.HasTravelRequest && ProfileInfo.IndexTravelRequest) {
		...
		if (ProfileInfo.onlineStyle == "APPLE") {
			$(".orderTittle").append('\
				<div class="taTab mainFontColor" style="margin: 0 56px 0 53px;cursor:pointer;color:#041e5b;text-decoration: underline;">' + taTab + '</div>\
			')
		} else {
			$(".orderTittle").append('\
				<div class="srpitLine"></div>\
				<div class="taTab mainFontColor" style="margin: 0 56px 0 40px;cursor:pointer;color:#041e5b;">' + taTab + '</div>\
			')
		}
	}
	if (ProfileInfo.HasTravelRequest && !ProfileInfo.IndexTravelRequest) {
...
function searchBody() {
	...
	$('.searchBody').html('\
        <ul class="tabBar flexRow">\
            <li class="tab airDom specificFontColor">' + get_lan('searchBody').airDom + '</li>\
            <li class="tab airIntl specificFontColor" style="width: 125px;">' + get_lan('searchBody').airIntl + '</li>\
            <li class="tab Hotel specificFontColor" id="indexHotelTab">' + get_lan('searchBody').hotel + '</li>\
            <li class="tab appleHotel specificFontColor hide">' + get_lan('searchBody').hotel + '</li>\
            <li class="tab Train specificFontColor">' + get_lan('searchBody').train + '</li>\
            <li class="tab Car specificFontColor ' + showCarRent + '">' + get_lan('searchBody').car + '</li>\
        </ul>\
        <div class="searchPage airDomBody">\
            <div class="domTabBar mainFontColor">\
                <span style="position:relative">\
					...
					<label for="domOneWay" style="margin-left:15px;cursor: pointer;">' + get_lan('searchBody').oneWay + '</label>\
                </span>\
                <span style="position:relative">\
                  	<input type="radio" id="domRoundTrip" name="domTrip" style="margin-left:56px">\
					<label for="domRoundTrip" style="margin-left:71px;cursor: pointer;">' + get_lan('searchBody').roundTrip + '</label>\
                </span>\
				<span style="position:relative">\
				    <input type="radio" id="domMultiple" name="domTrip" style="margin-left:56px">\
					<label for="domMultiple" style="margin-left:71px;cursor: pointer;">' + get_lan('searchBody').Multiple + '</label>\
				</span>\
            </div>\
			<div class="domnotMultiple">\
				...
				<div class="dateStyle flexRow">\
					...
					<div class="domDateBody flexRow">\
						...
						<select type="text" id="domDepartureSelect" onchange="GrayDepartPlusMinus()">'+ getHoursOptions('domAllDay') + '</select>\
					</div>\
					<select type="text" id="DepartPlusMinus"  class="plusMinus">'+ hoursDiffOptions + '</select>\
					...
					<div class="domDateBody domReturnDateBody flexRow">\
						...
						<select type="text" id="domReturnSelect" onchange="GrayreturnPlusMinus()">'+ getHoursOptions('domAllDay') + '</select>\
					</div>\
					<select type="text" style="color:#9B9B9B;" id="returnPlusMinus" class="plusMinus">'+ hoursDiffOptions + '</select>\
				</div>\
				<div class="selectTimeStyle flexRow hide">\
					<div class="dateTittle">' + get_lan('searchBody').departureTime + '</div>\
					<div class="domDateBody flexRow">\
						<select type="text" id="domDepartureSelect">'+ getHoursOptions() + '</select>\
					</div>\
					<div class="dateTittle domDateTittle" style="color:#9B9B9B;">' + get_lan('searchBody').returnTime + '</div>\
					<div class="domDateBody domReturnDateBody flexRow">\
						<select type="text" id="domReturnSelect">'+ getHoursOptions() + '</select>\
					</div>\
				</div>\
				...
				<div class="cabinStyle flexRow domCabinInfo">\
					...
					<input type="checkbox" class="domCodeShareCheckBox">\
			...
			<div class="domMultiple" style="display:none">\
				<div class="domAirMultipleBody" style="display:none">\
					<div class="domAirMultipleLi flexRow">\
						...
						<div class="MultipleLiInputBody flexRow">\
							...
							<select type="text" class="domMultipleSelect">'+ getHoursOptions('intlAllDay') + '</select>\
						</div>\
					</div>\
					<div class="domAirMultipleLi flexRow">\
					...
					<div class="flexRow cabinStyle domMultiCabinInfo" style="margin-top:5px;margin-bottom:40px">\
						...
						<select type="text" id="domCabin">' + optionStr + '</select>\
			...
			<div class="searchDomBtn btnBackColor" state="oneWay">' + get_lan('searchBody').search + '</div>\
        </div>\
        <div class="searchPage airIntlBody">\
            <div class="intlTabBar mainFontColor">\
                <span style="position:relative" class="oneWayIntl">\
					<input type="radio" id="intlOneWay" name="intlTrip" checked="checked">\
					<label for="intlOneWay" style="margin-left:15px;cursor: pointer;">' + get_lan('searchBody').oneWay + '</label>\
                </span>\
                <span style="position:relative">\
					<input type="radio" id="intlRoundTrip" name="intlTrip" style="margin-left:56px">\
					<label for="intlRoundTrip" style="margin-left:71px;cursor: pointer;">' + get_lan('searchBody').roundTrip + '</label>\
                </span>\
                <span style="position:relative">\
					<input type="radio" id="intlMultipleTrip" name="intlTrip" style="margin-left:56px">\
					<label for="intlMultipleTrip" style="margin-left:71px;cursor: pointer;">' + get_lan('searchBody').Multiple + '</label>\
                </span>\
            </div>\
            <div class="intlAirBody">\
                ...
                <div class="dateStyle flexRow">\
					...
					<select type="text" id="DepartPlusMinusintel"  class="plusMinus">'+ hoursDiffOptions + '</select>\
					...
					<select type="text" id="returnPlusMinusintel"  class="plusMinus">'+ hoursDiffOptions + '</select>\
				</div>\
                ...
                <div class="cabinStyle flexRow">\
                    <div class="cabinTittle">' + get_lan('searchBody').cabin + '</div>\
						<select type="text" id="intlCabin">' + optionStr + '</select>\
						<div class="intlDirectCheckBoxBody flexRow">\
							<div class="cabinTittle" style="text-align:right;padding-right: 10px;width:102px;">\
							<input type="checkbox" class="intlDirectCheckBox">\
						</div>\
            ...
            <div class="intlAirMultipleBody hide">\
				<div class="intlAirMultipleLi flexRow">\
              	...
              	<div class="addAirIntlBody specificFontColor">\
				  	<span style="font-weight:bold;margin-right:15px;font-size:16px;">+</span>\
				...
				<div class="pointsRemindBody hide intlPointsRemind"></div>\
              	<div class="flexRow" style="margin-top:5px;">\
					<div class="MultipleLiIcon" style="background:#fff;"></div>\
					<div class="MultipleLiText">' + get_lan('searchBody').cabin + '</div>\
					<select type="text" class="MultipleLiInput" id="MultipleCabin">' + optionStr + '</select>\
              	</div>\
            </div>\
            <div class="searchIntlBtn btnBackColor" state="oneWay">' + get_lan('searchBody').search + '</div>\
        </div>\
        <div class="searchPage hotelBody">\
            <div class="hotelTabBar mainFontColor">\
                <span style="position:relative">\
					<input type="radio" id="domHotel"  name="hotel" checked="checked">\
					<label for="domHotel" style="margin-left:15px;cursor: pointer;">' + get_lan('searchBody').domHotel + '</label>\
                </span>\
                <span style="position:relative">\
                  	<input type="radio" id="intlHotel"  name="hotel" style="margin-left:56px">\
					<label for="intlHotel" style="margin-left:71px;cursor: pointer;">' + get_lan('searchBody').intlHotel + '</label>\
                </span>\
            </div>\
            ...
            <div class="hotelKey flexRow">\
                ...
                <div class="flexRow" style="width:80%;">\
                	<input type="text" id="keyWordInput" autocomplete="off" placeholder="' + get_lan('searchBody').hotelKeyInput + '">\
                </div>\
                <div class="keyWordBody autoScrollY"></div>\
            </div>\
			<div class="flexRow searchHotelBox">\
				' + openMapHotel + '\
        ...
        <div class="searchPage appleHotelBody">\
            <div class="hotelTabBar">\
                <span style="position:relative">\
					<input type="radio" id="domHotel" name="applehotel" checked="checked">\
					<label for="domHotel" style="margin-left:15px;cursor: pointer;">' + get_lan('searchBody').domHotel + '</label>\
                </span>\
                <span style="position:relative">\
					<input type="radio" id="intlHotel" name="applehotel" style="margin-left:56px">\
					<label for="intlHotel" style="margin-left:71px;cursor: pointer;">' + get_lan('searchBody').intlHotel + '</label>\
                </span>\
            </div>\
            <div class="cityStyle flexRow" style="margin-top:5px">\
            ...
            <div class="searchAppleHotelBtn btnBackColor" state="domHotel" >' + get_lan('searchBody').search + '</div>\
        </div>\
        <div class="searchPage trainBody">\
            <div class="trainTabBar mainFontColor">\
                <span style="position:relative">\
					<input type="radio" id="trainOneWay" name="trainTrip" checked="checked">\
					<label for="trainOneWay" style="margin-left:15px;cursor: pointer;">' + get_lan('searchBody').oneWay + '</label>\
                </span>\
                <span style="position:relative">\
                  	<input type="radio" id="trainRoundTrip" name="trainTrip" style="margin-left:56px">\
					<label for="trainRoundTrip" style="margin-left:71px;cursor: pointer;">' + get_lan('searchBody').roundTrip + '</label>\
                </span>\
            </div>\
            <div class="cityStyle flexRow">\
                <div class="cityTittle">' + get_lan('searchBody').from + '</div>\
            ...
            <div class="dateStyle flexRow">\
				...
				<div class="trainDateBody trainReturnDateBody flexRow">\
					...
					<select type="text" class="trainReturnSelect" disabled="disabled">'+ getHoursOptions('trainAllDay') + '</select>\
				</div>\
            </div>\
            <div class="cabinStyle flexRow">\
                <div class="cabinTittle">' + get_lan('searchBody').trainNo + '</div>\
                <input type="text" id="trainCabin" placeholder="' + get_lan('searchBody').trainNoText + '" autocomplete="off">\
            </div>\
            <div class="searchTrainBtn btnBackColor" state="oneWay">' + get_lan('searchBody').search + '</div>\
        </div>\
        <div class="searchPage carBody">\
          	<div class="cityStyle flexRow" style="margin-top:20px;">\
              	<div class="cityTittle">' + get_lan('searchBody').carFrom + '</div>\
              	<input autocomplete="off" type="text" id="carDeparture" placeholder="' + get_lan('searchBody').carFrom + '">\
          	</div>\
          	<div class="cityStyle flexRow">\
              	<div class="cityTittle">' + get_lan('searchBody').carTO + '</div>\
              	<input autocomplete="off" type="text" id="carArrival" placeholder="' + get_lan('searchBody').carTO + '">\
          	</div>\
          	<div class="cityStyle flexRow">\
              	<div class="cityTittle">' + get_lan('searchBody').carFromDate + '</div>\
             	 <input autocomplete="off" type="text" id="carFromDate" readonly>\
              	<select id="carFromHour"></select>\
          	...
          	<div class="cityStyle flexRow">\
              	<div class="cityTittle">' + get_lan('searchBody').carToDate + '</div>\
              	<input autocomplete="off" type="text" id="carToDate" readonly>\
              	<select id="carToHour"></select>\
				<select id="carToMin">\
					<option value="00">00</option>\
					<option value="30">30</option>\
				</select>\
          	...
          	<div class="cityStyle flexRow carLine">\
              	<div class="cityTittle" style="'+ enStyle + '">' + get_lan('searchBody').carCompany + '</div>\
              	<select id="carCompany"></select>\
          	</div>\
          	<div class="searchCarBtn btnBackColor">' + get_lan('searchBody').search + '</div>\
        </div>\
        <div class="searchPage visaBody"></div>\
    ')
	...
	$("#hotelAddress").on('focus', function () {
		if (obtLanguage != "CN" && $('.hotelCityChange').attr('citycode')) {
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (!res.isDomesticAir && res.isInterAir) {
				...
				$('.hotelTabBar').hide()
				$('.hotelBody .cityStyle').css('margin-top', '20px')
			}
		},
		error: function () {
			// alert('fail');
...
function chooseHotel() {
	...
	$(".searchHotelBtn").unbind("click").click(function () {
		...
		if ($(this).attr("state") == "domHotel") {
			...
			var hotelCityCode = $('#hotelIntlCity').attr("cityCode");
	...
	function continueSearchHotel(hotelCityCode, hotelCityText, hotelState) {
		...
		var queryKey = hotelCityCode + ',' + hotelAreaTypeID + ',' + hotelname + ',' + address + ',' + $(
			"#hotelDepartureDate").val() + ',' + $("#hotelReturnDate").val() + ',' + stars + ',' + $("#hotelPrice").attr(
				"minPrice") + ',' + $("#hotelPrice").attr("maxPrice") + ",1,1,1,2000," + hotel_lonLat;
		var searchHotelInfo = {
			'queryKey': queryKey,
		...
		$("#tableBody").html('\
		    <div id="orderTable">\
				<div class="tr flexRow">\
					...
					<div style="min-width: 340px;max-width: 340px;">' + get_lan('table').route + '</div>\
						...
						if (ProfileInfo.onlineStyle == "APPLE") {
							$("#orderTable").append('\
							    <div class="flexRow" style="border-bottom:1px solid #d8d8d8;">\
							       <div class="ellipsis" style="width: 130px;border-right:1px solid #d8d8d8;box-sizing:border-box;padding-left:20px;">\
										<span class="orderNoClick specificFontColor ' + tableCell + '" style="text-decoration:underline;cursor:pointer;">' + item.OrderNo + '</span>\
									</div>\
							       	<div class="ellipsis" style="width: 160px;border-right:1px solid #d8d8d8;box-sizing:border-box;padding-left: 10px;">\
							...
							$("#orderTable").append('\
							    <div class="flexRow" style="border-bottom:1px solid #d8d8d8;">\
							       	<div class="" style="min-width: 130px;max-width: 130px;border-right:1px solid #d8d8d8;box-sizing:border-box;padding-left:20px;display: inline-block;">\
								   		...
								   		<div class="ellipsis orderNoClick specificFontColor ' + tableCell + '" style="text-decoration:underline;cursor:pointer;">' + item.OrderNo + '</div>\
								   	</div>\
...
function pendingApproval() {
	if (ProfileInfo.onlineStyle == "APPLE") {
		...
		$("#tableBody").html('\
		    <div id="orderTable">\
				<div class="tr flexRow">\
					<div style="min-width: 143px;max-width: 143px;padding-left:20px;">' + get_lan('table').orderNumber + '</div>\
					<div style="min-width: 82px;">' + get_lan('table').applyDate + '</div>\
					<div style="min-width: 31px;"></div>\
					<div style="min-width: 182px;">' + get_lan('table').roundTime + '</div>\
					<div style="min-width: 75px;">' + get_lan('table').shift + '</div>\
					<div style="min-width: 201px;">' + get_lan('table').route + '</div>\
					<div style="min-width: 100px;">' + get_lan('table').price + '</div>\
					<div style="min-width: 150px;text-align: center;">' + get_lan('table').operation + '</div>\
				</div>\
		    </div>')
	}
	$('#tableBody').mLoading("show");
						...
						approveList.map(function (item, index) {
							...
							$("#orderTable").append('\
						        <div class="flexRow" style="border-bottom:1px solid #d8d8d8;">\
						           	...
						           	<div class="ellipsis" style="width: 100px;border-right:1px solid #d8d8d8;box-sizing:border-box;">\
									...
									<table class="orderDetailsTable" border="0" style="width:580px;">\
						           	...
						           	<div class="flexRow" style="width: 150px;">\
						              	...
						              	<div class="denyBtn ' + tableCell + '" style="background: #d02239;" ApplicationNo="' + item.ApplicationNo + '">' + get_lan('table').deny + '</div>\
						           	</div>\
						        </div>')
							/*机票*/
						        ...
						        <div class="flexRow" style="border-bottom:1px solid #d8d8d8;">\
						           	<div class="" style="max-width: 130px;min-width: 130px;border-right:1px solid #d8d8d8;box-sizing:border-box;padding-left:20px;">\
										<div class="ellipsis ' + tableCell + '">' + item.Passenger + '</div>\
									   	<div class=" ellipsis approveNoClick mainFontColor ' + tableCell + '" style="text-decoration:underline;cursor:pointer;">' + item.OrderNo + '</div>\
									</div>\
						           	...
						           	<div class="ellipsis flexRow" style="align-items: center;justify-content: center;min-width: 100px;max-width: 100px;border-right:1px solid #d8d8d8;box-sizing:border-box;">\
									   	<div class="">' + item.BookTime.split(' ')[0] + '</div>\
									</div>\
						           	<table class="orderDetailsTable" border="0" style="width:580px;">\
						             	<tr>\
											<th style="width:30px;"></th>\
						           	...
						           	<div class="flexRow" style="width: 150px;align-items: center;">\
						              	...
						              	<div class="denyBtn" style="background: #d02239;" ApplicationNo="' + item.ApplicationNo + '">' + get_lan('table').deny + '</div>\
						           	</div>\
						        </div>')
							/*机票*/
							item.Segment.map(function (sItem) {
				...
				if (index < 6) {
					$(".newsList").append('\
						<div class="newsLi" index="' + index + '">\
							...
							<div class="line"></div> \
							<span class="r">' + res.companyNews[index].Time + '</span>\
						</div>')
				}
			})
			$(".articlePop").html('\
