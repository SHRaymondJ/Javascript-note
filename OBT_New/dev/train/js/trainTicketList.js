...
var travelType = searchTrainInfo.alterTicketInfo ? '1' : ''
...
function showContent(){
	$("#main").html('\
	    <div class="autoCenter">\
	        ...
	        <div class="ticketBody">\
	            ...
	            <div class="trainChooseBody flexColumn"></div>\
	...
	$(".searchBody").html('\
	    ...
	    <div class="trainDepartureCitySearch"><div class="departureAirIcon"></div><input type="text" id="trainDepartureCity" value="'+searchTrainInfo.departureCityText+'" code="'+searchTrainInfo.departureCity+'" citycode="'+searchTrainInfo.domqueryKey.split(",")[0]+'" cn="'+searchTrainInfo.departureCN+'"></div>\
	    <div class="trainArrivalCitySearch"><div class="arrivalAirIcon"></div><input type="text" id="trainArrivalCity" value="'+searchTrainInfo.arrivalCityText+'" code="'+searchTrainInfo.arrivalCity+'" citycode="'+searchTrainInfo.domqueryKey.split(",")[1]+'" cn="'+searchTrainInfo.arrivalCN+'"></div>\
	...
	$(".trainChooseBody").html('\
		<div class="trainChooseBody--oneLine flexRow">\
			<div class="trainChooseTitle">'+get_lan('trainChooseBody').model+'</div>\
			<div class="noLimitBtn trainChooseItem tabBarColor" belongTo="trainModel">'+get_lan('trainChooseBody').noLimit+'</div>\
			<div class="flexRow trainChooseItem"><div><input type="checkBox" traintype="G,C" id="trainModelGC" class="trainTypeInput"></div><label for="trainModelGC">'+get_lan('trainChooseBody').highSpeed+'</label></div>\
			<div class="flexRow trainChooseItem"><div><input type="checkBox" traintype="D" id="trainModelD" class="trainTypeInput"></div><label for="trainModelD">'+get_lan('trainChooseBody').motorCar+'</label></div>\
			<div class="flexRow trainChooseItem"><div><input type="checkBox" traintype="K,T,Z,L,Y,1" id="trainModelKTZLY1" class="trainTypeInput"></div><label for="trainModelKTZLY1">'+get_lan('trainChooseBody').Ordinary+'</label></div>\
		</div>\
		<div class="trainChooseBody--oneLine flexRow departStation">\
			<div class="trainChooseTitle">'+get_lan('siftBody').all1+'</div>\
			<div class="noLimitBtn trainChooseItem tabBarColor" belongTo="departStation">'+get_lan('trainChooseBody').all+'</div>\
		</div>\
		<div class="trainChooseBody--oneLine flexRow arrivalStation">\
			<div class="trainChooseTitle">'+get_lan('siftBody').all2+'</div>\
			<div class="noLimitBtn trainChooseItem tabBarColor" belongTo="arrivalStation">'+get_lan('trainChooseBody').all+'</div>\
		</div>\
		')
	//排序模块
	$(".siftBody").html('\
		<div style="width:100px;line-height:52px;margin-left:12px;">'+get_lan('siftBody').trainInfo+'</div>\
		<div class="departureTimeSort flexRow" style="width:100px;line-height:52px;margin-left:78px;">'+get_lan('siftBody').trainTime+'<div class="departureTimeSortIcon"></div></div>\
		<div style="width:210px;line-height:52px;margin-left:10px;" class="flexRow hide">\
		</div>\
		<div style="width:210px;line-height:52px;margin-left:20px;">'+get_lan('siftBody').station+'</div>\
		<div class="trainUseTimeSort flexRow" style="width:100px;line-height:52px;cursor:pointer;">'+get_lan('siftBody').trainUseTime+'<div class="trainUseTimeSortIcon"></div></div>\
		<div style="width:100px;line-height:52px;margin-left:150px;">'+get_lan('siftBody').trainPrice+'</div>\
        <div style="width:100px;line-height:52px;" class="seats">'+get_lan('siftBody').seats+'</div>\
		')
        ...
        function searchTrain(){
            if($('.searchState option:selected').attr("state")=='1'){
                ...
                var searchTrainInfo = {
                    ...
                    'queryKey':$('#trainDepartureCity').attr('cn')+','+$('#trainArrivalCity').attr('cn')+','+$('#trainDepartureDate').val()+DepartureSelectValue+',',
                ...
                var searchTrainInfo = {
                    ...
                    'queryKey':$('#trainDepartureCity').attr('cn')+','+$('#trainArrivalCity').attr('cn')+','+$('#trainDepartureDate').val()+DepartureSelectValue+',',
                   'queryKeyReturn':$('#trainArrivalCity').attr('cn')+','+$('#trainDepartureCity').attr('cn')+','+$('#trainReturnDate').val()+ReturnSelectValue+',',
				   'domqueryKey':$('#trainDepartureCity').attr('citycode') + ',' + $('#trainArrivalCity').attr('citycode') + ',' + $('#trainDepartureDate').val() + DepartureSelectValue+',ALL',
...
function ticketList(){
	...
	if(searchTrainInfo.type == "oneWay"){
		...
		$(".chooseDate").html('\
		    ...
		    <span class="nextDay" style="margin-right:20px;cursor:pointer">('+nextDayDate.substring(5,nextDayDate.length)+')'+get_lan('ticketList').nextDay+'</span>\
		    ')
		//前一天 后一天
		...
		$.ajax(
		  {
		    ...
		    dataType : 'json',
		    data:{
		        url: $.session.get('obtCompany')+"/QueryService.svc/QueryTrainsNew",
...
function ticketFilter(res){
    ...
    stationList.map(function(item){
		...
		<div class="flexRow trainChooseItem"><div><input type="checkBox" station="'+item.StationStart+'" id="'+item.StationStart+'" class="departStationInput"></div><label for="'+item.StationStart+'">'+item.StationStart+'</label></div>\
        ')
    })
    stationArriveList.map(function(item){
        $(".arrivalStation").append('\
		<div class="flexRow trainChooseItem"><div><input type="checkBox" station="'+item.StationArrive+'" id="'+item.StationArrive+'" class="arrivalStationInput"></div><label for="'+item.StationArrive+'">'+item.StationArrive+'</label></div>\
        ')
    })
	...
	$(".trainTypeInput").change(function(){
		$(this).parent().parent().siblings(".noLimitBtn").removeClass("tabBarColor");
		filterList = [];
		var checkboxList1 = [];
		...
		if(checkboxList1.length == 0){
			$(this).parent().parent().siblings(".noLimitBtn").addClass("tabBarColor");
			filterList = [];
            res.map(function(item){
                filterList.push(item);
            })
	...
	$(".noLimitBtn").unbind("click").click(function(){
		$(this).addClass("tabBarColor");
		if($(this).attr("belongTo") == 'trainModel'){
			$(".trainTypeInput").removeAttr("checked");
			filterList = [];
			res.map(function(item){
				filterList.push(item);
			})
		}else if($(this).attr("belongTo") == 'departStation'){
			$(".departStationInput").removeAttr("checked");
			departTrainList = [];
			res.map(function(item){
				departTrainList.push(item);
			})
		}else if($(this).attr("belongTo") == 'arrivalStation'){
			$(".arrivalStationInput").removeAttr("checked");
			arrivalTrainList = [];
			res.map(function(item){
				arrivalTrainList.push(item);
			})
	...
	//筛选站台
    $(".departStationInput").change(function(){
		var that = this;
		$(this).parent().parent().siblings(".noLimitBtn").removeClass("tabBarColor");
		departTrainList = [];
		var checkboxList1 = [];
		for(var i=0;i<$(".departStationInput").length;i++){
			if($(".departStationInput").eq(i).is(":checked")){
				checkboxList1.push(i);
				res.map(function(item){
					if(item.StationStart==$(".departStationInput").eq(i).attr('station')){
						departTrainList.push(item);
					}
				})
			}
    ...
    $(".arrivalStationInput").change(function(){
		var that = this;
		$(this).parent().parent().siblings(".noLimitBtn").removeClass("tabBarColor");
		arrivalTrainList = [];
		var checkboxList1 = [];
		for(var i=0;i<$(".arrivalStationInput").length;i++){
			if($(".arrivalStationInput").eq(i).is(":checked")){
				checkboxList1.push(i);
				res.map(function(item){
					if(item.StationArrive==$(".arrivalStationInput").eq(i).attr('station')){
						arrivalTrainList.push(item);
					}
				})
			}
...
function siftTrainList(filterList,departTrainList,arrivalTrainList){
    ...
    filterList.map(function(item){
        departTrainList.map(function(dItem){
            if(item.TrainCode == dItem.TrainCode && item.StationStart == dItem.StationStart){
    ...
    chooseTrain.map(function(item){
        arrivalTrainList.map(function(aItem){
            if(item.TrainCode == aItem.TrainCode && item.StationArrive == aItem.StationArrive){
				console.log(item.StationStart);
...
function ticketListInfo(res){
	...
	res.map(function(item,index){
		...
		item.FareList.map(function(dItem,dindex){
			...
			$(".ticketFareList").eq(index).append('\
				<div class="ticketFareLi flexRow">\
                  ...
                  <div class="liSeatType" style="min-width:50px;text-align:right;font-size:14px">'+SeatType+'</div>\
                  <div class="ticketPrice" style="margin-left:10px;width:84px;">'+dItem.Price+'<span style="font-size:12px;">'+ProfileInfo.OfficeCurrency+'</span></div>\
        ...
        if(obtLanguage=="CN"){
            ...
            $(".ticketPrice").css("margin-left",'35px');
        }else if(obtLanguage=="EN"){
			$(".liSeatType").css("min-width",'100px');
            $(".ticketPrice").css("margin-left",'15px');
