...
var train12306accountArray = new Array();
var bookWith12306 = false
...
//中英文对象
var cn = {
    "trainRemind": "请注意：火车的出票时间为05:00 – 23:30",
    "remarkPop": {
    ...
    "seatInfo": {
        ...
        "seatRemind": "如果本次列车剩余席位无法满足您的选座需求，系统将自动为您分配席位",
        "deaprture": "去程",
        "return": "回程",
        "window": "窗",
        "aisle": "过道",
    ...
    "success": "预订成功！",
    "totalAmount": "总金额:",
    "documentRemind": "请确保火车订单姓名与证件姓名一致",
    "use12306Account": "使用12306账号预订",
    "remindTittle":"预订前阅读",
    'priceDetails':"价格详细",
}
var en = {
    "trainRemind": "Please note that Train ticketing is only valid from 05:00 to 23:30hrs.",
    "remarkPop": {
    ...
    "passengerPop": {
        ...
        "train12306account": '12306 account:',
        'company': 'Company:',
        'delete':"Delete",
    },
    "newCustomerPop": {
    ...
    "seatInfo": {
        ...
        "seatRemind": "If the remaining seats of this train cannot meet your seat selection requirements, the system will automatically assign seats to you.",
        "deaprture": "Deaprture",
        "return": "Return",
        "window": "Window",
        "aisle": "Aisle",
    ...
    "totalAmount": "Total:",
    "documentRemind": "Please make sure the name of the train order is the same as the name of the document.",
    "use12306Account": "Booking with 12306 account",
...
function initEmployee12306(customerID, customerName, account, pwd) {
    //12306账号
    if (account && pwd) {
        var customer12306account = new Train12306account(customerID, customerName, account, pwd);
    } else {
        var customer12306account = new Train12306account(customerID, customerName);
        customer12306account.getPassengerInfo(netUserId, obtLanguage);
    }
    train12306accountArray.push(customer12306account);
...
function getPOReminder(){
    ...
    if ( obtLanguage == 'CN' && ProfileInfo.ReminderCnForPObook ){
        POReminder = '<div class="activeFontColor luggageReminder">'+ ProfileInfo.ReminderCnForPObook + '</div>'
    }else if ( ProfileInfo.ReminderEnForPObook ){
        POReminder = '<div class="iactiveFontColor luggageReminder">'+ ProfileInfo.ReminderEnForPObook + '</div>'
    }
    return POReminder
}

function use12306DisplayEffect (profile) {
    var train12306display = {
        use12306Box : true,
        train12306 : true
    }
    if (profile.TongChengTrainSelfBook && !profile.IsShow12306Choose) {
        // 走12306
        train12306display.use12306Box = false
        train12306display.train12306 = true
    } else if ( profile.TongChengTrainSelfBook && profile.IsShow12306Choose) {
        // 用户选择是否走12306
        train12306display.use12306Box = true
        train12306display.train12306 = true
    } else {
        // 不走12306
        train12306display.use12306Box = false
        train12306display.train12306 = false
    }
    switchUse12306Box(train12306display.use12306Box)
    switchTrain12306(train12306display.train12306)
    return train12306display
}
function handleUse12306AccountClick () {
    $('.use12306Account').click(function(){
        var isShow = $(this).prop('checked')
        switchTrain12306(isShow)
    })
}
function switchDisplay (dom, isShow) {
    if ( isShow ) {
        $(dom).show()
    } else {
        $(dom).hide()
...
function switchTrain12306 (isShow) {
    switchDisplay('.train12306', isShow)
    switchDisplay('.customer12306account', isShow)
}
function switchUse12306Box (isShow) {
    switchDisplay('.use12306Box *', isShow)
}
...
function showContent() {
    ...
    $("#main").html('\
        <div class="autoCenter">\
                ...
                <div class="budgetBox">\
                    <div class="budgetItem">\
                        <div class="budgetChartBox">\
                            <div class="budgetChart" id="eChart" style="width:120px;height:120px;"></div>\
			...
			<div class="remindInfo">\
                ...
                '+ POReminder +'\
              <div class="covidReminder"></div><br><br>\
            </div>\
            <div class="use12306Box hide">\
                <input type="checkbox" name="use12306Account" id="use12306Account" checked class="use12306Account"/>\
                <label for="use12306Account">' + get_lan('use12306Account') + '</label>\
            </div>\
            ...
            <div class="fareBody">\
                ...
                <div class="fareBodyDetail"></div>\
                <div class="totalFare">'+get_lan('totalAmount')+'<span class="totalFareText" style="font-size:24px;font-weight:600;color:#FF6720;"></span></div>\
            </div>\
        </div>\
        ')
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".orderDetail").attr("DeparteCityCN", res[0].StationStartSave);
            $(".orderDetail").attr("ArriveCityCN", res[0].StationArriveSave);
            ...
            res.map(function (item) {
                ...
                var station = item.StationStart+' - '+item.StationArrive;
                $(".orderDetail").append('\
	                <div class="orderDetailBody" trainCode="'+ item.TrainCode + '" ticketAmount="' + (parseFloat(item.FareList[0].Price) + parseFloat(item.FareList[0].ServiceFare)) + '">\
                      ...
                      <div class="orderStation">'+station+'<span style="margin-left:40px;">'+item.UseTime+'</span></div>\
	                  <div class="orderTrainCode">'+ item.TrainCode + '</div>\
	                  <div class="orderTimeStart">'+ item.TimeStart + '</div>\
	                  <div class="orderArrowIcon">\
                        <div style="width:4px;height:4px;background-color:#C0CAD8;border-radius:2px;"></div>\
                        <div style="width:1px;height:70px;background-color:#C0CAD8;margin-left:1.5px;"></div>\
                        <div style="width:4px;height:4px;background-color:#C0CAD8;border-radius:2px;"></div>\
                      </div>\
	            ...
	            ')
                $(".fareBodyDetail").append('\
                    <div style="width:296px;height:80px;background:#F5F6FA;margin-bottom:12px;box-sizing:border-box;padding:16px 12px">\
                    <div class="orderFareAmount flexRow"><div style="width:200px;">'+ get_lan('orderDetail').ticketPrice + '</div><span style="color:#111C4E;font-weight:600;">' + item.FareList[0].Price + ProfileInfo.OfficeCurrency+ '</span></div>\
                    <div class="orderServiceFare flexRow '+ showServiceFare + '"><div style="width:200px;">' + get_lan('orderDetail').serviceFare + '</div><span style="color:#111C4E;font-weight:600;">' + item.FareList[0].ServiceFare + ProfileInfo.OfficeCurrency + '</span></div>\
                    </div>\
                ')
            })
            ...
            if (trainTicketInfo.type == "roundTrip") {
                ...
                $(".orderDetailBody").eq(0).css("border-bottom","1px dashed #C0CAD8")
            }
                        ...
                        success: function (data) {
                            ...
                            if (searchTrainInfo.alterTicketInfo && !$.session.get('goOnBookOrderNo')) {
                                ...
                                //无代订权限
                                    ...
                                    oneCustomer();
                                    // 有审批单权限
                                    var city = $('.orderDetail').attr('departecityCN') + ',' + $('.orderDetail').attr('arrivecityCN')
...
function selectPassengers() {
    ...
    function clickPassengerLi() {
        $(".selectPassengerLi").unbind("click").click(function () {
            $('body').mLoading("show");
            $(".selectPassengerList").css("display", "none");use12306DisplayEffect
            if (trainTicketInfo.type == "oneWay") {
    ...
    $.ajax(
        {
            ...
            data: {
                ...
                jsonStr: JSON.stringify(getPassengerInfoAPI())
            },
            success: function (data) {
                $('body').mLoading("hide");
                var res = JSON.parse(data);
    ...
    $.ajax(
        {
            ...
            success: function (data) {
                ...
                train12306accountArray = [];

                // 12.10旧文档
                // <div class="passengerLi" customerId="'+item.ID+'" companyId="'+item.OrderCompanyId+'">
                res.map(function (item, index) {
                    ...
                    <div class="passengerLi" style="width:792px;margin-left:24px;border-bottom: 1px #C0CAD8FF dashed;" Birthday="'+ item.Birthday + '" Gender="' + Gender + '" Nationality="' + Nationality + '" customerId="' + item.ID + '" companyId="' + CompanyId + '">\
						...
						<div style="height:130px;width:100%" class="flexRow">\
							...
							<div style="height:130px;width:140px;font-size:14px;color:#041E5B;position:relative;">\
								<span class="changeRemarkBtn" index="' +index + '" style="position:absolute;top: 40px;right: 15px;cursor:pointer;">' + get_lan('passengerInfo').remarks +'</span>\
							</div>\
						</div>\
                    </div>\
                    \
                    ')
                    var train12306display = use12306DisplayEffect(ProfileInfo)
                    if ( train12306display.train12306 ) {
                        initEmployee12306(res[index].ID, res[index]['Name' + obtLanguage], res[index].ChinaRailwayAccountName, res[index].ChinaRailwayAccountPwd);
                    } 
                    handleUse12306AccountClick();
...
function totalAmount(personNum) {
    ...
    $(".totalFareText").html(total +  ProfileInfo.OfficeCurrency);
}
//订火车票
function clickBookBtn() {
    ...
    $(".bookTicketBtn").unbind("click").click(function () {
        ...
        // 	return false;
        // }
        if (ProfileInfo.TongChengTrainSelfBook && $('.use12306Account').prop('checked')) {
            var trainRemind = false
            $.ajax({
                type: 'post',
                url: $.session.get('ajaxUrl'),
                async: false,
                dataType: 'json',
                data: {
                    url: $.session.get('obtCompany') + "/QueryService.svc/QuerySelectedTrainInfoNew",
                    jsonStr: '{"queryKey":"' + queryKey + '","id":' + netUserId + ',"isTrainAccount":"'+bookWith12306+'"}'
                },
                success: function (data) {
                    var res = JSON.parse(data);
                    console.log(res);
                    if(res[0].trainRemind){
                        trainRemind = confirm(get_lan("trainRemind"));
                    }else {
                        trainRemind = true
                    }
                }
            ...
            // 12306账号
            var has12306account = true;
            $('.customer12306account .beforeEdit').map(function (index) {
                if ($('.customer12306account .beforeEdit').eq(index).attr('data-account') == 'null' || !$('.customer12306account .beforeEdit').eq(index).attr('data-account')) {
                    has12306account = false;
                }
            })

            check12306account(0);
            function check12306account(i) {
                $('body').mLoading("show");
                train12306accountArray[i].check12306account(i, function () {
                    if (i == train12306accountArray.length - 1) {
                        $('body').mLoading("hide");
                        bookTicketBtn();
                    } else {
                        check12306account(++i);
            ...
            // bookTicketBtn();    //测试做加单子用
        } else {
            if (ProfileInfo.IsNeedIdentification) {
    ...
    function bookTicketBtn() {
        ...
        if ($(".passengerLi").length == 0) {
            ...
            if ($.session.get('TAnumber')) {
                ...
                var city = JSON.stringify([$('.orderDetail').attr('departecityCN'), $('.orderDetail').attr('arrivecityCN')]);
                // $('body').mLoading("show");
                $.ajax(
                    {
                        type: 'post',
                        ...
                        dataType: 'json',
                        data: {
                            url: $.session.get('obtCompany') + "/OrderService.svc/CheckTripCompareTANew",
                            jsonStr: '{"request":{"key":' + netUserId + ',"TANo":"' + $.session.get('TAnumber') + '","Language":"' + obtLanguage + '","cityList":' + city + ',"timeList":[' + timeList + '],"tripType":"4","customerList":[' + customerList + ']}}'
                        },
                        success: function (data) {
                            var res = JSON.parse(data);
                            console.log(res);
    ...
    $.ajax(
        {
            ...
            data: {
                ...
                jsonStr: '{"request":{"id":' + netUserId + ',"classLimitation":"' + classLimitation + '","classLimitationBack":"' + classLimitationBack + 
                '","documentNos":"' + documentNos + '","orgTicketInfo":"' + orgTicketInfo + '","queryKey":"' + queryKey + '","trainNoLimitation":"' + trainNoLimitation + 
                '","isDirectTicket":"' + isDirectTicket + '","trainNoLimitationBack":"' + trainNoLimitationBack + '","Language":"' + obtLanguage + 
                '","desCodeTrain":"' + desCodeTrain + '","isGrabGo":"' + isGrabGo + '","isGrabBack":"' + isGrabBack + '","carTaxiInfo":"' + carTaxiInfo + 
                '","standByLimitTimeGo":"' + standByLimitTimeGo + '","standByLimitTimeBack":"' + standByLimitTimeBack + '","use12306Account":"' + bookWith12306 + '"}}'
            },
            success: function (data) {
                $('body').mLoading("hide");
                if (data == '') {
