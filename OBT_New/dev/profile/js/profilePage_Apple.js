...
if (ProfileInfo.NeedUpdatePassword == true||ProfileInfo.appleComplementState) {
	openRemindPop();
	...
	$(".remindPop").html('\
		...
		<div style="text-align:left;font-size:16px;">Please ensure your Chinese and English name, DOB, gender,phone number and nationality in your profile match what you have on the government ID or passport you intend to use during travel.</div>\
		')
}
//中英文对象
var cn = {
...
var en = {
	...
	"personalInfoBody": {
		...
		'phoneRemind':"Phone number must be 11 digits.",
...
function showContent() {
	...
	$("#main").html('\
	    <div class="autoCenter">\
		  ...
		  <div class="pofileHideRemind hide flexRow"> <img src="./images/icon_hotts.png" style="display:block;width:18px;height:18px;margin: 15px 5px 0 0;"> This information comes from your Apple Travel Profile. Update your profile on <a style="color:rgb(48, 131, 251);margin-left: 6px;" href="https://travelweb.apple.com/"> TravelWeb</a></div>\
	    ...
	    ')
	if(ProfileInfo.PofileHideModify){
		$(".pofileHideRemind").removeClass("hide");
	}
	$.ajax({
		type: 'post',
		url: $.session.get('ajaxUrl'),
...
function personalInfoBody(profileInfo) {
	...
	var showModifyPhone = profileInfo.AppleChangePhoneNumber ? "" : "hide";
	$(".personalInfoBody").html('\
		...
		<div class="personalInfoContent flexWrap">\
		  ...
		  <div class="infoFinalContent flexRow">\
			  ...
			  <input class="regionInput" style="width: 60px;height: 27px;position: absolute;left: 1px;top: 2px;border: 0;" readonly>\
			  <img src="./images/dowArrow.png" class="selectRegion">\
			  <div class="regionList autoScrollY hide"></div>\
			</div>\
			<input class="personalInput phoneInput" type="text" value="'+ profileInfo.Phone + '" readonly><span class="editPhone '+showModifyPhone+'">' + get_lan("personalInfoBody").edit + '</span>\
		  </div>\
	...
	$.ajax({
		...
		success: function (data) {
			...
			var res = JSON.parse(data);
			console.log(res);
			res.PhoneRegionCodeList.map(function(item){
				var RegionCode = item.RegionCode;
				$(".regionList").append('\
				<div class="regionLi flexRow" AreaCode="'+item.AreaCode+'" regionCode="'+item.RegionCode+'" class="flewRow"><div class="checkRegion"></div><div style="width:35px;">'+item.AreaCode+'</div> + '+RegionCode+'</div>\
				')
			})
			if(profileInfo.RegionCode!=""){
				var RegionCode = ' + '+profileInfo.RegionCode;
			}else{
				var RegionCode = '';
			}
			$(".regionInput").val(RegionCode);
			$(".regionInput").attr('regionCode',profileInfo.RegionCode);
			$(".regionLi").unbind("click").click(function(){
				var regionCode = $(this).attr("regionCode");
				$(".regionInput").val('+ '+regionCode);
				$(".regionInput").attr('regionCode',regionCode);
				$(".regionList").addClass("hide");
				$(".checkRegion").text('');
				$(this).children(".checkRegion").text('✓');
			})
		}
	});
	if(ProfileInfo.PofileHideModify){
		$(".editNameCn,.editNameEn,.editGender,.editCountry,.editBirth").addClass("hide");
	}
	...
	$(".editPhone").unbind("click").click(function () {
		if (!$(".editPhone").attr("state") || $(".editPhone").attr("state") == "edit") {
			...
			$(".selectRegion").unbind("click").click(function(){
				if($(".regionList").hasClass("hide")){
					$(".regionList").removeClass("hide");
				}else{
					$(".regionList").addClass("hide");
				}
			})
			...
			$(".regionList").addClass("hide");
			var regionCode = $(".regionInput").attr('regionCode');
			if (regionCode=="86"&&!regPhone.test(phoneNum)) {
				alert(get_lan("personalInfoBody").phoneRemind);
				return false;
			} else {
				$(".selectRegion").unbind("click");
				addOrUpdateInfo(profileInfo.ID, "phoneNum", phoneNum+',,'+regionCode);
			}
		}
	})
	...
	$.ajax(
		{
		  ...
		  data:{
			  ...
			  jsonStr:'{"customerId":"'+profileInfo.ID+'","key":'+netUserId+'}'
		  },
		  success : function(data) {
			  var res = JSON.parse(data);
			  console.log(res);
		...
		$.ajax(
			{
				...
				success: function (data) {
					...
					if(ProfileInfo.PofileHideModify){
						$(".delCreditCard,.addCreditCardBtn").addClass("hide");
					}
...
function documentInfoBody(profileInfo) {
	...
	if(ProfileInfo.PofileHideModify){
		$(".addPassportBtn,.updateDocument,.delDocument").addClass("hide");
	}
	//修改证件号
		...
		$.ajax({
			...
			success: function (data) {
				...
				res.Infos.map(function (item, index) {
					...
					if(ProfileInfo.PofileHideModify){
						$(".addPreferenceBtn,.deletePreference").addClass("hide");
					}
...
function membershipBody(profileInfo) {
	...
	if(ProfileInfo.PofileHideModify){
		$(".addMembershipBtn,.deleteMembership").addClass("hide");
	}
	...
	$(".addMembershipBtn").unbind("click").click(function () {
		...
		$(".popContent").html('\
			...
			<div class="popContentLi flexRow">\
			  ...
			  <div class="popContentLiContent">\
			    <input type+"text" class="popInput idNumberInput" onkeyup="this.value=this.value.replace(/[, ]/g,\'\')">\
			  </div>\
			</div>\
		')
		$('body').mLoading("show");
			...
			if (res.code == 200) {
				...
				if (changeType == "phoneNum") {
					if(ProfileInfo.DocumentsDetail.length > 0){
						ProfileInfo.appleComplementState = false;
						$.session.set('ProfileInfo', JSON.stringify(ProfileInfo));
					}
					...
					$.ajax({
						...
						success: function (data) {
							var res = JSON.parse(data);
							console.log(res);
							if(res.DocumentsDetail.length > 0&&res.Phone!=""){
								ProfileInfo.appleComplementState = false;
								$.session.set('ProfileInfo', JSON.stringify(ProfileInfo));
							}
							documentInfoBody(res);
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (res.code == 200) {
					...
					if(ProfileInfo.PofileHideModify){
						$(".addArrangerBtn,.delArranger").addClass("hide");
					}
