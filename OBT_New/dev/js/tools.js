...
(function () {
	...
	* */
	...
	/**
	 * 无title弹窗
	 * tools.alert(content<string>, submit<string>, infoIcon<boolean>, callback<function>)
	 * 样式在common.css
	 *  */
	tools.alert = function (content, submit, infoIcon, callback) {
		if (!callback) {
			callback = function () { }
		}
		var dom = '<div class="toolsCover">\
					<div class="toolsPopupBox">\
						<section class="toolsPopupBody">\
							[INFO]\
							<p>[CONTENT]</p>\
						</section>\
						<footer class="toolsPopupFooter">\
							<button class="toolsPopupBtn toolsPopupSubmit btnBackColor">[SUBMIT]</button>\
						</footer>\
					</div>\
				</div>'
		var infoContent = infoIcon ? '<img src="../images/info.png" alt="" class="toolsPopupInfo">' : ''
		dom = dom.replace('[INFO]', infoContent).replace('[CONTENT]', content).replace('[SUBMIT]', submit)
		$('body').append(dom)
		$('.toolsPopupSubmit').click(function () {
			$('.toolsCover').remove()
			callback()
		})
	}
	...
	* */
	...
	tools.addProfileInfo = function (data, type) {
		...
		if (res.DocumentsDetail.length == 0 || res.Phone == "") {
			var appleComplementState = true;
		} else {
			var appleComplementState = false;
		}
		/*2020-2-3 新增*/
		...
		var ProfileInfoMy = {
			...
			"appleComplementState": appleComplementState,
		...
		$.ajax(
			{
				...
				success: function (data) {
					...
					if ($.session.get('obtLanguage') == "CN") {
						...
						var notMeeting = JSON.parse($.session.get('ProfileInfo')).ChainCode === "Pfizer China" ?
							'个人差旅行程' : '不以会议为目的预订';
					} else {
						...
						var notMeeting = JSON.parse($.session.get('ProfileInfo')).ChainCode === "Pfizer China" ?
							'For individual business travel.' : 'This trip is not for a meeting.';
					}
					...
					<div class="travelRequestPop" style="position:relative;width: 838px;background-color: #fff;z-index: 101;position: fixed;top: 50%;left: 50%;transform: translate(-50%,-50%);border-radius: 10px;padding: 10px;font-family: Sans-serif,Arial,Verdana;">\
						...
						<div style="box-sizing: border-box;padding:10px;font-size: 15px;line-height:24px;">\
							...
							<div class="ApplicationBody travelApplicationBody">\
								...
								<div class="travelApplicationList" style="width:778px;min-height:150px;margin-left:30px;"></div>\
								<div class="applicationLi flexRow '+ showNoApplication + '" style="margin-left: 30px;width:778px;height:50px;line-height:50px;font-size:16px;box-sizing:border-box;position:relative;">\
							...
							<div class="ApplicationBody meetingApplicationBody hide">\
								...
								<div class="meetingApplicationList" style="width:778px;min-height:150px;margin-left:30px;max-height:200px;"></div>\
								<div class="applicationLi flexRow '+ showNoApplication + '" style="margin-left: 30px;width:778px;height:50px;line-height:50px;font-size:16px;box-sizing:border-box;position:relative;">\
					...
					if ($.session.get('obtLanguage') == "CN") {
						...
						<div style="line-height:20px;color:#999;font-size:14px;">\
						  ...
						  <span style="margin:0 5px;color:#2D6BD2;">勾选“'+ notMeeting + '”。</span>\
						</div>\
						');
					} else {
						$(".travelApplicationRemind").html('\
						...
						<div style="line-height:20px;color:#999;font-size:14px;">\
						  ...
						  <span style="margin:0 5px;color:#2D6BD2;">select "'+ notMeeting + '"</span>\
						</div>\
						');
					}
					...
					if ((res.HasMessage && !JSON.parse($.session.get('ProfileInfo')).SelectNoTrOption) || data == '') {
						...
						if ((showNoApplication == "" && res.TravelInfos.length == 0) || (res.TravelInfos.length == 0 && queryKey.split(',')[0] != JSON.parse($.session.get('ProfileInfo')).ID)) {
							$.session.remove("TAnumber");
							$.session.remove("TACustomerId");
							callback();
						}
						var travelApplicationList = [];
						var meetingApplicationList = [];
						res.TravelInfos.map(function (item) {
							var address = item.TravelItinerary[0] ? item.TravelItinerary[0] : "";
							if (item.Type == 1) {
								travelApplicationList.push(item);
								$(".travelApplicationList").append('\
									<div class="applicationLi flexRow" style="width:778px;height:60px;line-height:50px;font-size:16px;box-sizing:border-box;position:relative;">\
									...
									<div style="margin-left:20px;min-width:200px;position:relative;">'+ item.StartDate + '~' + item.EndDate + '<div style="position:absolute;font-size:14px;color:#666;left:0;bottom:-14px;">' + item.TravelRequestName + '</div></div>\
									<div style="margin-left:20px;">'+ address + '</div>\
									</div>\
								')
							}
							if (item.Type == 3) {
								meetingApplicationList.push(item);
						...
						if (travelApplicationList.length > 4) {
							$(".travelApplicationList").addClass("autoScrollY");
						}
						if (meetingApplicationList.length > 4) {
							$(".meetingApplicationList").addClass("autoScrollY");
						}
						/*隐藏差旅审批单*/
