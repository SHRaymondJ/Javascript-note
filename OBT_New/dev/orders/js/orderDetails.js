...
var customer12306account = []

var address = {
	APAC: {
		air: '/index/index.html?search=air&continue=1',
		hotel: '/index/index.html?search=hotel&continue=1',
		rail: '/index/index.html?search=rail&continue=1',
		car: '/index/index.html?search=car&continue=1',
	},
	normal: {
		air: '/search/queryAir.html',
		hotel: '/search/queryHotel.html',
		rail: '/search/queryTrain.html',
		car: '/search/queryCar.html',
	}
}
function continueBookAddress(){
...
var cn = {
	...
	'refundTicketPop': {
		...
		"refundPopRemind": "如因航班取消/延误/疫情影响等情况，需要提交非自愿退票，请联系线下服务组，谢谢！",
	...
	'alterTicketPop': {
		...
		"AlterRemindLine1": "*如因为航班取消/由航空公司延误取消行程，请联系服务小组改签。",
		"AlterRemindLine2": "*如已办理值机，请先取消值机再点击确认，否则将会引起改签费用差异。如果您需要部分更改，请联系服务团队。",
		"AlterRemindLine3": "*改签后原订单无法恢复",
	},
	'approvalPop': {
		"approvalTittle": "审核信息",
		"selectTittle": "审核人选择",
	...
	"selectSeatPop": {
		...
		"ocuppied": "他人已选",
		"selectRemind": "请选择完座位。",
		"reminder": "在线选座仅为预留座位，实际以航司或柜台值机为准。敬请谅解。",
	},
	"combineOrderPop": {
	...
	"otherList": {
		...
		"change": "修改",
		"delete": "删除",
		"deleteOtherRemind": "请确认是否删除所有申请信息?",
	},
}
var en = {
	...
	'refundTicketPop': {
		...
		"refundPopRemind": "For involuntary refund due to epidemic impact, flight cancelation/delay by airlines, please reach out servcie team, thanks!",
	...
	'alterTicketPop': {
		...
		"alterTicketDateTittle": "Change the Search",
		"alterDateTittle": "Change Date:",
		"alterCabinTittle": "Cabin Type:",
		"alterTittle": "Change Apply",
		"alterConfirm": "Apply Change",
		'cabins': {
			'cabin1': 'Unlimited',
			'cabin2': 'Ecomomy',
			'cabin3': 'Business',
		...
		"AlterRemindLine1": "* For involuntary change(it's cancelled/delayed by the airline), please reach out to your service team for the assistance.",
		"AlterRemindLine2": "* If you have already checked in, please cancel the check-in first, otherwise your change request will failed.Please contact the service team if you need change partially.",
		"AlterRemindLine3": "* The booking will be cancelled after it is changed. ",
	},
	'approvalPop': {
	...
	"selectSeatPop": {
		...
		"reminder": "Online seat selection is just reserved and the final on-board seats are subject to the check-in counter at the airport.Please kindly understanding.",
	},
	"combineOrderPop": {
	...
	"otherList": {
		...
		"change": "Edit",
		"delete": "Delete",
		"deleteOtherRemind": "Do you want to delete all request information?",
	},
}
...
function showContent() {
	...
	$("#main").html('\
	    <div class="autoCenter">\
            ...
            <div class="fileListInfo hide">\
				<div class="fileListTittle">' + get_lan('fileListInfo').fileListTittle + '</div>\
				<div class="filesListBar flexRow">\
                  	<div class="filesListBarLi" style="width:180px;">' + get_lan('fileListInfo').uploadFile + '</div>\
                  	<div class="filesListBarLi" style="width:180px;">' + get_lan('fileListInfo').uploadDate + '</div>\
                  	<div class="filesListBarLi delectFileBar" style="width:180px;">' + get_lan('fileListInfo').edit + '</div>\
              	</div>\
            </div>\
            <div class="approvalInfo">\
              	<div class="approvalInfoTittle">' + get_lan('approvalInfo').approvalTittle + '</div>\
              	<div class="passengerBar flexRow">\
                  	<div class="passengerBarLi" style="width:100px;">' + get_lan('approvalInfo').approvalLevel + '</div>\
					<div class="passengerBarLi" style="width:150px;">' + get_lan('approvalInfo').Approver + '</div>\
					<div class="passengerBarLi" style="width:150px;">' + get_lan('approvalInfo').approvalPhone + '</div>\
					<div class="passengerBarLi" style="width:250px;">' + get_lan('approvalInfo').approvalEmail + '</div>\
					<div class="passengerBarLi" style="width:150px;">' + get_lan('approvalInfo').approvalResult + '</div>\
					<div class="passengerBarLi" style="width:230px;">' + get_lan('approvalInfo').approvalTime + '</div>\
					<div class="passengerBarLi" style="width:160px;">' + get_lan('approvalInfo').approvalReason + '</div>\
              	</div>\
	        ...
	        <div class="orderDetailsBody" style="position:relative;">\
				<div class="orderDetailsTabBar flexRow"></div>\
				...
				<div class="orderList otherList"></div>\
	        </div>\
	    </div>\
	'
	)
	$(".refundTicketPop").html('\
        <div class="refundTicketTittle">\
			' + get_lan("refundTicketPop").refundTittle1 + '\
			<span class="orangeColor">' + get_lan("refundTicketPop").refundTittle2 + '</span>\
			' + get_lan("refundTicketPop").refundTittle3 + '\
		</div>\
        <div class="refundTicketBody"></div>\
        <div class="refundTicketFooter flexRow">\
          	<div class="closeRefundTicketBtn">' + get_lan("remarkPop").cancel + '</div>\
          	<div class="sureRefundTicketBtn">' + get_lan("remarkPop").confirm + '</div>\
        </div>\
    ')
	// get_lan("alterTicketPop").alterTicketTittleRemind + '</span><br>'
	if (ProfileInfo.onlineStyle == "APPLE") {
		var grayColor = "activeFontColor"
		var alterTittle = get_lan("alterTicketPop").alterTicket1 + '<span class="orangeColor">' + get_lan("alterTicketPop").alterTicket2 + '</span>' + get_lan("alterTicketPop").alterTicket3 + '</span><br>'
			+ get_lan("alterTicketPop").alterTicketTittleAir
		var alterConfirm = get_lan("remarkPop").confirm

	} else {
		var grayColor = ""
		var alterTittle = get_lan("alterTicketPop").alterTittle
		var alterConfirm = get_lan("alterTicketPop").alterConfirm
	}
	$(".alterPop").html('\
        <div class="alterTicketTittle">\
			<span class="' + grayColor + '">' + alterTittle + '\
		</div>\
        <div class="alterTicketBody"></div>\
        <div class="alterTicketFooter flexRow">\
          	<div class="closeAlterTicketBtn">' + get_lan("remarkPop").cancel + '</div>\
          	<div class="sureAlterTicketBtn">' + alterConfirm + '</div>\
        </div>\
    ')
	// $.ajax(
	//   {
	...
	$(".approvalPop").html('\
        <div class="approvalTittle">' + get_lan("approvalPop").approvalTittle + '</div>\
        <div class="approvalBody">\
          	<div class="selectTittle">' + get_lan("approvalPop").selectTittle + '</div>\
          	<div class="selectBody"></div>\
          	<div class="flexRow">\
            	<div class="popLiTittle">' + get_lan("approvalPop").approvalPrice + '</div>\
            	<div class="approvalPrice"></div>\
        	</div>\
          	<div class="flexRow">\
            	<div class="popLiTittle">' + approvalText + '</div>\
           		<textarea class="commentsBody" maxlength="80"></textarea>\
         	 </div>\
          	<div class="ticketOption">\
            	<div class="selectTittle">' + get_lan("approvalPop").after + '</div>\
            	<div class="flexRow" style="margin:10px 0 10px 0;">\
					<input class="approveReason" type="radio" name="approveReason">\
					<div style="min-height: 20px;width: 500px;margin-left: 10px;">' + get_lan("approvalPop").reason1 + '</div>\
				</div>\
				<div class="flexRow" style="margin-bottom:10px;">\
					<input class="approveReason" type="radio" name="approveReason" checked>\
					<div style="min-height: 20px;width: 500px;margin-left: 10px;">' + get_lan("approvalPop").reason2 + '</div>\
				</div>\
          	</div>\
        </div>\
        <div class="approvalFooter flexRow">\
			<div class="closeApprovalBtn">' + get_lan("remarkPop").cancel + '</div>\
			<div class="sureApprovalBtn">' + get_lan("remarkPop").confirm + '</div>\
        </div>\
    ')
	$(".uploadFilePop").html('\
        <div class="uploadFileTittle">' + get_lan("orderInfo").uploadFile + '</div>\
        <div class="uploadFileContent">\
          	<div class="chooseFileText">' + get_lan("uploadFilePop").chooseFile + '</div>\
          	<div class="chooseFilePath"></div>\
          	<div class="browseBtn"><input id="file" type="file">' + get_lan("uploadFilePop").browse + '</div>\
        </div>\
        <div class="uploadFileFooter flexRow">\
          	<div class="closeUploadFileBtn">' + get_lan("remarkPop").cancel + '</div>\
          	<div class="sureUploadFileBtn">' + get_lan("remarkPop").confirm + '</div>\
        </div>\
    ')
}
// 支付方式
...
function orderDetails() {
	...
	$.ajax({
		...
		success: function (data) {
			...
			var showDownloadFile = ProfileInfo.DownloadItinerary ? "" : "hide";
			var downloadText = ProfileInfo.DownloadBill && (res.State == "完成") ? get_lan("orderInfo").DownloadInvoice : get_lan("orderInfo").DownloadItinerary;
			var downloadType = ProfileInfo.DownloadBill && (res.State == "完成") ? "invoice" : "itinerary";
                ...
                <div class="flexRow">\
                	<div class="orderInfoLeft">\
						<div class="orderDetailInfo">\
							<div class="orderDetailInfoNo" style="font-weight:bold;">' + get_lan('orderInfo').OrderCustomer + ' ' + res.OrderCustomer + '</div>\
							<div class="orderDetailInfoDate">' + get_lan('orderInfo').BookTime + ' ' + res.BookTime.substring(0, 10) + '</div>\
							<div class="orderDetailInfoState ' + showState + '">' + get_lan('orderInfo').State + ' \
								<span class="activeFontColor" style="font-size:20px;">' + res.ApplicationState + '</span>\
							</div>\
							<div class="orderDetailInfoPrice">' + get_lan('orderInfo').OrderFare + ' \
								<span class="activeFontColor" style="font-size:22px;">' + res.PaidAmount + '</span>\
							</div>\
						</div>\
						<div class="orderApproveInfo"></div>\
					</div>\
                    <div class="orderInfoRight">\
                      	<div class="orderInfoRightBtn btnBackColor goOnBookBtn ' + showGoOnBook + '">' + get_lan('orderInfo').goOnBook + '</div>\
                      	<div class="orderInfoRightBtn btnBackColor approvalBtn ' + ShowApproval + '">' + get_lan('orderInfo').approval + '</div>\
                      	<div class="orderInfoRightBtn btnBackColor cancelApprovalBtn ' + ShowCancelApproval + '">' + get_lan('orderInfo').cancelapproval + '</div>\
             			<div class="orderInfoRightBtn btnBackColor ticketBtn ' + showTicket + '">' + get_lan('orderInfo').ticket + '</div>\
                      	<div class="orderInfoRightBtn btnBackColor onlinePayBtn \
							' + showOnlinePay + ' ' + showState + '" orderNo="' + res.OrderNo + '">\
							' + get_lan('orderInfo').onlinePay + '</div>\
                      	<div class="orderInfoRightBtn btnBackColor uploadFileBtn ' + showUpLoadFile + '" \
							orderNo="' + res.OrderNo + '" customerId="' + res.OrderCustomerDetailList[0].CompanyId + '">\
							' + get_lan('orderInfo').uploadFile + '</div>\
						<div class="orderInfoRightBtn btnBackColor downloadFileBtn ' + showDownloadFile + '" \
							downloadType="' + downloadType + '" orderNo="' + res.OrderNo + '" \
							customerId="' + res.OrderCustomerDetailList[0].CompanyId + '">' + downloadText + '</div>\
                    </div>\
                </div>\
			')
			$(".downloadFileBtn").unbind("click").click(function () {
				...
				if (downloadType == "invoice") {
					...
					$.ajax({
						...
						dataType: 'json',
						data: {
							url: $.session.get('obtCompany') + "/orderService.svc/GetItineraryPdfFilePathPost",
							jsonStr: '{"orderInfo":"' + orderNo + '","id":' + netUserId + ',"Language":"' + obtLanguage + '"}'
						},
						success: function (data) {
							if (data == "") {
								alert("生成订单失败")
				...
				} else if (downloadType == "itinerary") {
					$('body').mLoading("show");
					$.ajax({
						type: 'post',
						url: $.session.get('ajaxUrl'),
						dataType: 'json',
						data: {
							url: $.session.get('obtCompany') + "/orderService.svc/GetInvoicePdfFilePathPost",
							jsonStr: '{"orderInfo":"' + orderNo + '","id":' + netUserId + ',"Language":"' + obtLanguage + '"}'
						},
						success: function (data) {
							if (data == "") {
				...
				$(".fileListInfo").append('\
                    <div class="filesList flexRow">\
                        <div class="filesListBarLi" style="width:180px;">\
							<img src="../../orders/images/file.png" style="width:20px;height:24px;margin-top:3px;" />\
						</div>\
                        <div class="filesListBarLi" style="width:180px;">' + res.FileUploadTime + '</div>\
                        <div class="filesListBarLi" style="width:180px;">\
							<span class="delectFile ' + showTicket + '" orderNo="' + res.OrderNo + '">' + get_lan('fileListInfo').delect + '</span>\
						</div>\
                    </div>\
                ')
			}
					...
					$.ajax({
						...
						success: function (data) {
							...
							if (res.length == 0) {
								...
								$(".combineOrderSelect").append('<option value="no">' + get_lan("combineOrderPop").option + '</option>')
							}
				...
				res.ApproveDtailList.map(function (item) {
					$(".approvalList").append('\
                        <div class="approvalLi flexRow">\
                        	<div class="approvalLiDiv" style="width:100px;">' + item.ApproverLevel + '</div>\
                        	<div class="approvalLiDiv" style="width:150px;">' + item.Approver + '</div>\
                        	<div class="approvalLiDiv" style="width:150px;">' + item.ApproverPhone + '\
								<img src="../../orders/images/phoneIcon.png" class="phoneIcon" applicationNo="' + item.ApplicationNo + '" approvor="' + item.ApproverID + '">\
							</div>\
                        	<div class="approvalLiDiv" style="width:250px;">\
								' + hideEmail(ProfileInfo, item.ApproverEmail) + '\
								<img src="../../orders/images/emailIcon.png" class="emailIcon" applicationNo="' + item.ApplicationNo + '" approvor="' + item.ApproverID + '">\
							</div>\
							<div class="approvalLiDiv" style="width:150px;">' + item.ApprovalResult + '</div>\
							<div class="approvalLiDiv" style="width:230px;">' + item.ApprovalDate + '</div>\
							<div class="approvalLiDiv ellipsis" style="width:160px;" title="' + item.ApprovalReason + '"><xmp>' + item.ApprovalReason + '</xmp></div>\
                        </div>\
                    ')
				})
			...
			res.OrderCustomerDetailList.map(function (item, index) {
				$(".passengerList").append('\
                    <div class="passengerLi flexRow" customerId="' + item.Uid + '">\
						...
						<div class="passengerLiDiv passengerLiDocuments" style="width:300px;">' + hideDocument(ProfileInfo, item.DocumentNOs[0], 2) + '</div>\
						<div class="passengerLiDiv" style="width:170px;">' + item.PsgType + '</div>\
						<div class="passengerLiDiv" style="width:100px;"></div>\
						<div class="passengerLiDiv changeRemarkBtn" index="' + index + '"  \
							style="width:130px;text-decoration: underline;color:#1e66ae;cursor:pointer">' + get_lan('orderCustomerInfo').remarks + '</div>\
                    </div>\
                ')
			...
			//其他
			else if (res.TravelRequestNewMiscellList.length != 0) {
				$(".orderList").addClass("hide");
			...
			/*2021-6-22 其他*/
			if (res.TravelRequestNewMiscellList.length != 0) {
				$(".orderDetailsTabBar").append('<div class="orderDetailsTab" name="Other">' + get_lan('orderDetailsTabBar').other +
					'</div>');
				otherList(res);
			}
			// 2020-3-26 下载行程单
			...
			$(".orderDetailsTab").unbind("click").click(function () {
				...
				switch ($(this).attr("name")) {
					...
					case 'Other':
						$(".orderList").addClass("hide");
						$(".otherList").removeClass("hide");
						$('.changeOtherBody').removeClass("hide");
						break;
				}
			})
		},
...
function showDownloadBtn() {
	...
	$(".orderDetailsTabBar").append('\
		<div class="downloadBtnGroup">\
...
function travXpense(orderRes) {
	...
	$.ajax({
		...
		dataType: 'json',
		data: {
			url: $.session.get('obtCompany') + "/orderService.svc/GetTravelRequestDataPost",
		...
		success: function (data) {
			...
			console.log(res);
			$('body').mLoading("hide");
			if (res.TravelRequestNewMiscellBookData.length > 0 && res.TravelRequestNewMiscellBookData[0].MiscellBookList.length != 0) {
	...
	function otherExpensesList(travelRequestRes) {
		...
		$(".otherExpensesList").html('\
            <div class="otherFareTittle">' + get_lan("otherExpensesList").miscell + '</div>\
            <div class="MiscellList"></div>\
        ')
		travelRequestRes.TravelRequestNewMiscellBookData[0].MiscellBookList.map(function (item, index) {
			$(".MiscellList").append('\
                <div class="MiscellLi flexRow">\
                  	<div class="MiscellLiTittle">' + item.ProductCode + '</div>\
                  	<div class="MiscellLiTittle" style="width:60px;">' + get_lan("otherExpensesList").amount + '</div>\
                  	<input type="text" class="miscellLiInput miscellLiInputAmount" value="' + item.Farepaid + '" readonly>\
                  	<div class="activeFontColor" style="margin:0 20px 0 10px;">' + item.Currency + '</div>\
                  	<div class="MiscellLiTittle" style="width:60px;">' + get_lan("otherExpensesList").remark + '</div>\
                  	<input type="text" class="miscellLiInput miscellLiInputRemark" value="' + item.BookRemark + '" readonly>\
                  	<div class="flexRow">\
                    	<input type="checkbox" class="miscellPayCheck">\
                    	' + get_lan("otherExpensesList").companyPay + '\
                  	</div>\
                  	<div class="modifyMiscell" index="' + index + '" state="modify">' + get_lan("otherExpensesList").modify + '</div>\
                </div>\
            ')
...
function onlinePay(orderRes, operationType, payType) {
	...
	if (!ProfileInfo.AliPay && !ProfileInfo.WechatPay && !ProfileInfo.CreditCardYeepay) {
		...
		// 仅有无卡支付
		nocardPay(orderRes, false);
	} else {
		openOnlinePayPop();
		$(".onlinePayPop").html('\
            <div class="onlinePayTittle tittleBackColor">\
				' + get_lan('onlinePayRemind') + '\
				<div class="closeOnlinePayIcon" style="color:#fff;">x</div>\
			</div>\
            <div class="tipsGroup">' + get_lan('tipsText') + '</div>\
			<div class="flexRow onlinePayUl">\
				...
				<div class="onlinePayLi creditCardPayLi">\
					<div class="onlinePayImg creditCardPayImg" name="creditCard"></div>\
					<div class="onlineLiText">' + get_lan('onlinePayPop').creditCardPay + '</div>\
				</div>\
				<div class="onlinePayLi quickLi">\
					<div class="onlinePayImg nocardImg" name="quick"></div>\
					<div class="onlineLiText">' + get_lan('onlinePayPop').quickPayment + '</div>\
				</div>\
			...
			<div class="cancelbtnGroup">\
				<div class="cancelbtn btnBackColor">' + get_lan('cancelbtn') + '</div>\
			</div>\
        ')
		...
		$(".wechatLi").remove();
		if (!ProfileInfo.ICBCPayment) {
			$('.creditCardPayLi').remove()
		}
		...
		var containerWidth = $('.onlinePayUl').width()
		$(".onlinePayLi").css("width", (containerWidth / parseInt($(".onlinePayLi").length)) + "px");
		// $(".onlineLiText").css("width", (containerWidth / parseInt($(".onlinePayLi").length) - 100) + "px");
		...
		$(".onlinePayImg").unbind("click").click(function () {
			if ($(this).attr("name") == "yeePay") {
				...
				nocardPay(orderRes, false)
			} else if ($(this).attr("name") == "creditCard") {
				nocardPay(orderRes, true)
			}
		})
	}
}
			...
			if (res.status == "S") {
				...
				setTimeout(function () {
					...
					$(".onlinePayPop").html('\
						...
						<div class="cancelbtnGroup">\
							<div class="cancelbtn tittleBackColor">' + get_lan('finalbtn') + '</div>\
							<div class="cancelbtn btnBackColor">' + get_lan('cancelbtn') + '</div>\
						</div>\
					')
				...
				alert(res.msg);
				$(".onlinePayPop").html('\
				    <div class="onlinePayTittle tittleBackColor">请在新页面完成付款</div>\
					<div class="flexRow onlinePayUl">\
				       	<p class="" style="width: 100%;display: flex;justify-content: center;align-items: center;font-size:18px">完成付款后请根据情况点击下面按钮：</p>\
				    </div>\
					<div class="cancelbtnGroup">\
						<div class="cancelbtn tittleBackColor refresh">' + get_lan('finalbtn') + '</div>\
						<div class="cancelbtn btnBackColor">' + get_lan('cancelbtn') + '</div>\
					</div>\
				')
...
/*无卡支付*/
function nocardPay(orderRes, ICBC) {
	console.log(orderRes)
	$("#cover").hide();
	...
	$("body").append('\
		<div id="cover2" class="quickPay">\
			<div class="quickGroup">\
				<div class="quickTitle">' + get_lan('noCard').title + '\
	...
	function getcustomerInfo() {
		...
		if (ICBC) {
			$('.CVVline,.overTimeLine,.domline,.phoneLine').hide()
		}
		$.ajax({
			type: 'post',
			url: $.session.get('ajaxUrl'),
					...
					if (res.NeedVerifyCode) {
						...
						$('.quickbody').append('\
							<div class="quickLine  clearfix">\
								...
								<div class="linebtn">' + get_lan('noCard').lineText + '</div>\
							</div>')
	...
	function ICBCpay() {
		...
		var jsonObj = {
			request: {
				id: id,
				orderNo: $(".onlinePayBtn").attr("orderNo"),
				language: obtLanguage,
				payChannel: 4
			}
		}
		$.ajax({
			...
			success: function (data) {
				var res = JSON.parse(data);
				console.log(res);
				var selectedCreditCard = $('.customerCardList').find('option:selected')
				var payJsonObj = {
					request: {
						id: id,
						language: obtLanguage,
						exMechantNO: res.exMechantNO,
						CardId: selectedCreditCard.val(),
						CardNo: selectedCreditCard.text()
					}
				}
				$.ajax({
					type: 'post',
					...
					dataType: 'json',
					data: {
						url: $.session.get('obtCompany') + "/OrderService.svc/ICBC_Pay",
						jsonStr: JSON.stringify(payJsonObj)
					},
					success: function (data) {
						var payRes = JSON.parse(data)
						console.log(payRes)
...
function HotelGKBooking(orderRes, clickType, showMore) {
	$(".GKbookingPop").html('\
        <div class="GKbookingTittle">\
			<div class="closeGKbookingIcon">x</div>\
		</div>\
			...
			if (res.code == "200") {
				if (res.needShow) {
					...
					if (showMore) {
						...
						$(".GKbookingBody").html('\
							<div class="bookhotel">\
							    <p>' + get_lan("GKbookingPop").GKRemindText3 + '</p>\
							    <div class="btngroup">\
									<div class="bookhotelBtn">' + get_lan("remarkPop").confirm2 + '</div>\
								</div>\
							</div>\
							<div class="GKGroup" style="">\
								<div class="GKRemindText2">' + get_lan("GKbookingPop").GKRemindText2 + '</div>\
						...
						$(".GKbookingBody").html('\
							<div class="remindIcon"></div>\
							<div class="GKRemindText">' + get_lan("GKbookingPop").GKRemindText2 + '</div>\
							<select class="GKRemarkSelect"></select>\
						');
					}
			...
			if (!item.CanModify) {
				$(".remarkInfoBody").append('\
                    <div id="liTittle' + item.Index + '" class="remarkLi flexRow">\
                      	...
                      	<div class="liContent" index="' + item.Index + '"><input id="remarkInput' + item.Index + '" CompanyID="' + ProfileInfo.companyId +
					...
					$(".remarkInfoBody").append('\
                        <div class="remarkLi flexRow">\
                          	...
                          	<div class="liContent">\
                            	...
                            	<input class="remarkLiInput" CompanyID="' + ProfileInfo.companyId + '" id="remarkInput' + item.Index + '"  \
									require="' + colorRed + '" value="' + item.Content + '" index="' + item.Index + '"  key="' + item.SubmitContent + '" \
									placeholder="' + get_lan("remarkPop").search + '">\
                          	</div>\
                        </div>\
                    ')
...
function getTAnumber(callback, type) {
	checkGoOnBook(function (data) {
		if (type == 'start') {
			var res = JSON.parse(data)
			if (res.code == "200" && res.data != "") {
				if (isHideGoOnBook(res)) {
					$('.goOnBookBtn').addClass('hide');
				}
				callback();
			}
		} else {
			if (data == '' || data == null) {
				...
				var res = JSON.parse(data)
				if (res.code == "200") {
					$.session.set("TAnumber", res.data.split(',')[0])
					if (isHideGoOnBook(res)) {
						$('.goOnBookBtn').addClass('hide');
					}
				}
...
function checkGoOnBook(successCallback) {
...
function goOnBook(orderRes) {
	...
	$(".GoOnBookPop").html('\
        ...
        <div>\
          	<div class="finishRemind hide">\
			  	<span style="font-size:18px;color:#2577e3;margin-right:30px;">' + get_lan('finishRemind2left') + '</span>\
				' + get_lan('finishRemind2') + '\
			</div>\
          	<div class="finishLi hide">\
            	<div class="finishLiImg approveImg" name="approve"></div>\
            	<div class="finishLiText">' + get_lan('orderInfo').approval + '</div>\
            	<div class="finishLiLine hide"></div>\
        	</div>\
        	...
        	<div class="flexRow continueUl">\
				...
				<div class="continueLi carIconLi '+ showCarRent + '">\
					<div class="continueLiImg carImg" name="car"></div>\
					<div class="continueLiText">' + get_lan('continueUl').car + '</div>\
				</div>\
				<div class="continueLi otherIconLi '+ showOther + '">\
					<div class="continueLiImg otherImg" name="other"></div>\
					<div class="continueLiText">' + get_lan('continueUl').other + '</div>\
				</div>\
			</div>\
        </div>\
		')
	...
	checkGoOnBook(function (data) {
		...
		if (res.code == "200" && res.data != "") {
			if (isHideGoOnBook(res)) {
				$('.continueUl').addClass('hide');
				$('.bookRemind').addClass('hide');
			}
		}
	})
	if (ProfileInfo.onlineStyle == "APPLE") {
			...
			if ($(this).attr("name") == "air") {
				getTAnumber(function () {
					if ($.session.get('TAnumber')) {
						getTAtime(continueBookAddress().air, 1)
						// GetTravelRequestCityInfo由getTAtime方法执行
					} else {
						window.location.href = continueBookAddress().air;
					}
				})
							...
							if (data == '') {
								...
								getTAnumber(function () {
									if ($.session.get('TAnumber')) {
										...
										getTAtime(continueBookAddress().hotel, 2)
									} else {
										window.location.href = continueBookAddress().hotel;
									}
								})
								// HotelGKBooking(orderRes,type,false);
							} else {
				...
				getTAnumber(function () {
					if ($.session.get('TAnumber')) {
						getTAtime(continueBookAddress().rail, 4)
					} else {
						window.location.href = continueBookAddress().rail;
					}
				})
				// window.location.href = '../../search/queryTrain.html';
			} else if ($(this).attr("name") == "car") {
				getTAnumber(function () {
					if ($.session.get('TAnumber')) {
						getTAtime(continueBookAddress().car, 4)
					} else {
						window.location.href = continueBookAddress().car;
					}
				})
				...
				$.session.set('changeOtherInfo', JSON.stringify(changeOtherInfo));
	...
	if ($(".finishRemind").hasClass('hide') && $(".finishLi").hasClass('hide') && $(".bookRemind").hasClass('hide') && $(".continueUl").hasClass('hide')) {
		closeGoOnBookPop();
	}
}
			...
			if (!orderRes.MoreApproversSubmit) {
				...
				res.Approvers.map(function (item, index) {
					$(".selectBody").append('\
                        <div class="flexRow" style="margin-top:5px;">\
                          	<div class="popLiTittle">' + get_lan("approvalPop").Approver1 + (index + 1) + ' ' + get_lan("approvalPop").Approver2 + '</div>\
                          	<div class="ApproverSelectBody"></div>\
                        </div>\
                    ')
					if (item.Value.length > 2) {
						$(".ApproverSelectBody").eq(index).addClass("autoScrollY");
...
function segmentList(res) {
	...
	res.Segment.map(function (sitem, index) {
		...
		var document = get_lan('orderCustomerInfo').popDocuments + hideDocument(ProfileInfo, sitem[0].Passengers[0].DocumentNo, sitem[0].Passengers[0].DocumentType)
			...
			<div class="segmentLi">\
			  	<div class="segmentLiTittle flexWrap">\
			    	...
			    	<div style="margin-left:30px;">' + get_lan('orderDetails').orderState + '\
						<span class="activeFontColor">' + sitem[0].StateDes + '</span>\
					</div>\
                	<div style="margin-left:30px;">' + get_lan('orderDetails').price + '\
						<span class="activeFontColor" title="' + fareAmountDetail + '">' + sitem[0].AirFareAmount + '</span>\
					</div>\
                	<div style="margin-left:30px;" class="' + showTicketDate + '">\
						' + get_lan('orderDetails').TicketDate + '\
						<span class="keypoint">' + sitem[0].TicketDate + '</span>\
					</div>\
                	<div style="margin-left:30px;width:200px;" class="' + showAirlineReference + ' ellipsis">\
						' + get_lan('orderDetails').AirlineReference + '\
						<span class="keypoint" title="' + sitem[0].AirlineReference + '">' + sitem[0].AirlineReference + '</span>\
					</div>\
			  	</div>\
			...
			$(".segmentDetailsList").eq(index).append('\
				<div class="segmentDetailsLi">\
                  	<div style="height:105px;position: relative;">\
						...
						<div class="segmentSelectSeat btnBackColor ' + showBookSeat + '" orderNo="' + res.OrderNo + '" PassengersInfo="' + PassengersInfo + '" \
							flightInfo="' + item.DeparteCity + '-' + item.ArriveCity + '&&' + item.FlightNo + ' ' + item.DepartsDate + ' ' + item.ScheduleDepart + '" \
							airline="' + item.AirlineCode + '" departureTime="' + item.DepartsDate + '" dstCity="' + item.ArriveAirportCode + '" \
							flightNo="' + item.FlightNo.substring(2, item.FlightNo.length) + '" orgCity="' + item.DeparteAirportCode + '" pnrCode="' + item.PnrCode + '">' + get_lan("orderDetails").selectSeat + '\
						</div>\
						<div class="infoGroup">\
							...
							<div class="segmentState ' + showState + '">' + get_lan('orderDetails').State + get_lan('orderDetails').StateInfo + '</div>\
							<div class="segmentMeal">' + item.Meal + ' ' + AirNewInsurancesInfo + '\
								<span class="TicketNoText ' + showTicketNo + '" style="margin-left:20px;">' + get_lan('orderDetails').TicketNo + item.Passengers[0].TicketNo + '</span>\
								<span class="segmentSeat ' + showSeatInfo + ' mainFontColor" title="' + passengersSeatInfo + '">' + passengerSeatInfo + '</span>\
								<sapn class="segmentSeatInfo ' + showPassengerSeat + ' mainFontColor" title="' + passengersSeatInfo + '">' + get_lan('orderDetails').passengerSeatInfo + '</span>\
							</div>\
						</div>\
                	</div>\
                  	<div style="position:relative;">\
                      	<div class="segmentRebooking ' + showRebooking + '">\
							<span class="ruleStyle">' + get_lan('orderDetails').Rebooking + '</span>' + item.Rebooking + '\
						</div>\
                      	<div class="segmentRefund ' + showRebooking + '">\
							<span class="ruleStyle">' + get_lan('orderDetails').Refund + '</span>' + item.Refund + '\
						</div>\
                      	<div class="segmentRes hide">\
							<span class="ruleStyle">' + get_lan('orderDetails').Res + '</span>' + item.Res + '\
						</div>\
                  	</div >\
				</div >\
				')
			$(".segmentBtnList").eq(index).html('\
				<div class="segmentBtn airCheckInBtn ' + showBtn4 + '" AirlineCode="' + sitem[0].AirlineCode + '">\
					' + get_lan('orderDetails').onlineCheckIn + '</div>\
				<div class="segmentBtn cancelBtnAir ' + showBtn3 + '" ItemId="' + sitem[0].AirItemId + '" \
					orderNo="' + sitem[0].PnrCode + '">' + get_lan('orderDetails').orderCancel + '</div>\
				<div class="segmentBtn ChangeTicketBtnAir ' + showBtn2 + '" index="' + index + '" \
					orderId="' + sitem[0].AirItemId + '" IsDomestic="' + sitem[0].IsDomestic + '">' + get_lan('orderDetails').refund + '</div>\
				<div class="segmentBtn alterBtnAir ' + showBtn1 + '" index="' + index + '" \
					changeInfo="' + changeInfo.substring(0, changeInfo.length - 1) + '" \
					alterTicketInfo="' + changeInfo.substring(0, changeInfo.length - 1) + ',' + sitem[0].Passengers[0].DocumentNo + ',' + orderInfo.OrderCustomerDetailList[0].CompanyId + ',' +
	...
	// 保险显示
	var insuranceArray = []
	res.Segment.map(function (segment) {
		segment.map(function (item) {
			item.AirNewInsurances.map(function (insurance) {
				var newArray = insurance
				newArray.FlightNo = item.FlightNo
				insuranceArray.push(newArray)
			})
		})
	})
	insuranceArray = insuranceArray.sort(function (a, b) { return b.InsuranceNameP.localeCompare(a.InsuranceNameP) })
	var insuranceInfo = ''
	var showInsurance = 'style="display:none;"'
	if (insuranceArray.length > 0 && ProfileInfo.onlineStyle !== 'APPLE') {
		insuranceArray.map(function (insurance) {
			var insAmount = insurance.AirInsuranceAmount + ProfileInfo.OfficeCurrency
			insuranceInfo += '<p class="insurance_detail">' + insurance.FlightNo + ' - ' + insurance.InsuranceNameP + ' ' + insurance.AirInsuranceName + ': ' + '<span class="insurance_price">' + insAmount + '</span></p>'
		})
		showInsurance = ''
	}
	$('.segmentDetailsList').eq(res.Segment.length - 1).append('<div class="insurance_wrapper" ' + showInsurance + '>\
										<span class="ruleStyle">'+ get_lan('orderDetails').insurance + '</span>\
										<div class="insurance_info">' + insuranceInfo + '</div>\
										<img class="insurance_icon" src="../../images/icon_ord_insu.png"/>\
									</div >')
	//机票在线选座
			...
			success: function (data) {
				...
				if (res.length == 0) {
					...
					$(".selectSeatPop").html('\
							...
							<div class="seatContent">\
								<div class="seatContentTittle flexRow">\
									...
									<div style="width:120px;text-align:center;">\
										<div class="seatIcon1" style="margin: 0 auto;margin-top: 15px;"></div>\
										' + get_lan("selectSeatPop").available + '\
									</div>\
									<div style="width:120px;text-align:center;">\
										<div class="seatIcon2" style="margin: 0 auto;margin-top: 15px;"></div>\
										' + get_lan("selectSeatPop").blocked + '\
									</div>\
									<div style="width:120px;text-align:center;">\
										<div class="seatIcon3" style="margin: 0 auto;margin-top: 15px;"></div>\
										' + get_lan("selectSeatPop").select + '\
									</div>\
									<div style="width:120px;text-align:center;">\
										<div class="seatIcon4" style="margin: 0 auto;margin-top: 15px;"></div>\
						...
						<div class="flexRow">\
							<p class="seatContentReminder">'+ get_lan("selectSeatPop").reminder + '<p>\
						</div>\
                        <div class="flexRow">\
					...
					PassengersList.map(function (item) {
						$(".seatCustomerList").append('\
                            <div class="seatCustomerLi ellipsis" customerId="' + item.split('_')[0] + '">\
								' + item.split('_')[1] + '\
								<div class="seatCustomerLiText" style="position:absolute;left:220px;top:0;"></div>\
							</div>\
                        ')
					})
				...
				if (ticket.code == "502") {
					...
					res.Segment[index].map(function (item) {
						...
						<div class="refundTicketInfo">\
						 	<div class="refundFlightNo">' + item.FlightNo + '</div>\
						 	<div class="refundDepartureTime">' + item.ScheduleDepart + '</div>\
						 	<div class="refundArriveTime">' + item.ScheduleArrive + '</div>\
						 	<div class="refundArrow"></div>\
						 	<div class="refundDepartureCity">' + item.DeparteCity + '</div>\
						 	<div class="refundArriveCity">' + item.ArriveCity + '</div>\
						</div>'
					})
					// "refundBrokerage":"预计航空公司手续费",
					// "refundMoney":"预计退款金额",
					$(".refundTicketBody").html('\
						<div class="refundTicketPassenger">\
							' + get_lan("refundTicketPop").refundTicketPassenger + ' ' + res.Segment[index][0].Passengers[0].PassengerName + '\
						</div>\
						<div class="autoScrollY" style="min-height:85px;max-height:170px;">\
							' + refundTicketInfoBody + '\
						</div>\
						<p class="refundBrokerage hide" style="padding-top: 20px;">\
							'+ get_lan("refundTicketPop").refundBrokerage + '\
							<span class="money" style="color:#ED8322;float:right">-</span>\
						</p>\
						<p class="refundMoney hide" style="padding-top: 20px;">\
							'+ get_lan("refundTicketPop").refundMoney + '\
							<span class="money" style="color:#ED8322;font-size:18px;float:right">-</span>\
						</p>\
						<div class="refundTicketLimit">\
							<div class="refundTicketLimitTittle">' + get_lan("refundTicketPop").refundTicketLimit + '</div>\
							<div class="refundTicketLimitBody">' + res.Segment[index][res.Segment[index].length - 1].Refund + '</div>\
						</div>\
						<div class="refundTicketReason">\
						  	<div class="refundTicketReasonTittle">' + get_lan("refundTicketPop").refundTicketReason + '</div>\
						  	<div class="refundTicketReasonBody">\
						  		<div class="flexRow" style="margin-bottom:10px;">\
								  	<input class="refundReason" type="radio" name="refundReason" checked>\
									<div style="height:20px;margin-left:10px;">' + get_lan("refundTicketPop").refundTicketReason1 + '</div>\
								</div>\
						  		<div class="flexRow">\
								  	<input class="refundReason" type="radio" name="refundReason">\
									<div style="height:20px;margin-left:10px;">' + get_lan("refundTicketPop").refundTicketReason2 + '</div>\
								</div>\
						  	</div>\
						</div>\
					')
					...
					$(".refundTicketReason").append('<span style="color:#3083fb;margin-top:8px;">' + get_lan("refundTicketPop")
						.refundPopRemind2 + '</span>');
					if (JSON.parse($.session.get('ProfileInfo')).IsBCD) {
						$(".refundTicketReason").html("<span>" + get_lan("refundTicketPop").refundPopRemind + "</span>");
						$(".refundTicketReason").append('<span style="color:#3083fb;margin-top:8px;">' + get_lan("refundTicketPop")
							.refundPopRemind2 + '</span>');
	...
	$(".alterBtnAir").unbind("click").click(function () {
		...
		res.Segment[index].map(function (item) {
			alterTicketInfoBody += '\
            	<div class="alterTicketInfo">\
             		<div class="alterFlightNo">' + item.FlightNo + '</div>\
             		<div class="alterDepartureTime">' + item.ScheduleDepart + '</div>\
             		<div class="alterArriveTime">' + item.ScheduleArrive + '</div>\
             		<div class="refundArrow"></div>\
             		<div class="alterDepartureCity">' + item.DeparteCity + '</div>\
             		<div class="alterArriveCity">' + item.ArriveCity + '</div>\
            	</div>'
		})
		$(".alterTicketBody").html('\
            <div class="alterTicketPassenger">' + get_lan("alterTicketPop").alterTicketPassenger + ' ' + res.Segment[index][0].Passengers[0].PassengerName + '</div>\
            <div class="autoScrollY" style="min-height:85px;max-height:170px;" id="alterTicketInfo">' + alterTicketInfoBody + '</div>\
            <div class="alterTicketDate">\
              	<div class="alterTicketDateBody">\
                	<div class="alterTicketDateTittle">' + get_lan("alterTicketPop").alterTicketDateTittle + '</div>\
              	</div>\
            </div>\
            <div class="alterTicketLimit">\
              	<div class="alterTicketLimitTittle">' + get_lan("alterTicketPop").alterTicketLimit + '</div>\
              	<div class="alterTicketLimitBody">' + res.Segment[index][res.Segment[index].length - 1].Rebooking + '\
				  	<span class="domAlterRemindText hide"><br>' + get_lan("alterTicketPop").domAlterRemind + '</span>\
				</div>\
            </div>\
			<p style="font-size: 12px;font-weight: 400;color: #E2251B;line-height: 17px;margin-bottom:10px;">'+ get_lan("alterTicketPop").AlterRemindLine1 + '</p>\
			<p style="font-size: 12px;font-weight: 400;color: #2D6BD2;line-height: 17px;margin-bottom:10px;">'+ get_lan("alterTicketPop").AlterRemindLine2 + '</p>\
			<p style="font-size: 12px;font-weight: 400;color: #333333;line-height: 17px;margin-bottom:10px;">'+ get_lan("alterTicketPop").AlterRemindLine3 + '</p>\
        ')
		if (ProfileInfo.onlineStyle != 'APPLE') {
			$('#alterTicketInfo').after($('.alterTicketDateTittle'))
		}
		var hoursOptions =
		...
		if (IsDomestic == 'true') {
			var alterInputBody = '\
				<div class="alterInputBody flexRow">\
					...
					<select id="alterTimeInput">\
						<option value="all" class="domAllDay">' + get_lan("alterTicketPop").allDay + '</option>\
						'+ hoursOptions + '\
					</select>\
				</div>\
            '
			...
			$(".alterTicketDateBody").append('\
                <div class="alterDateTittle">' + get_lan("alterTicketPop").alterDateTittle + '</div>\
                <div class="alterCabinTittle">' + get_lan("alterTicketPop").alterCabinTittle + '</div>\
                ' + alterInputBody + '\
                <select id="alterCabinSelect">\
                  	<option berthType="0">' + get_lan('alterTicketPop').cabins.cabin1 + '</option>\
					<option berthType="1">' + get_lan('alterTicketPop').cabins.cabin2 + '</option>\
					<option berthType="2">' + get_lan('alterTicketPop').cabins.cabin3 + '</option>\
					<option berthType="3">' + get_lan('alterTicketPop').cabins.cabin4 + '</option>\
					<option berthType="23">' + get_lan('alterTicketPop').cabins.cabin5 + '</option>\
                </select>\
                ')
...
function hotelList(res) {
	...
	res.Hotel.map(function (hitem, index) {
		...
		var totalPrice = ''
		var priceText = get_lan('orderDetails').price
		if (ProfileInfo.onlineStyle !== 'APPLE') {
			// 总金额
			totalPrice =
				hitem.HotelServiceFee !== '0CNY'	// 没金额后台返回的好像都是<string>0CNY
					? '<div style="margin-left:50px;">' + get_lan('orderDetails').serviceFee +
					'<span class="activeFontColor">' + hitem.HotelServiceFee +
					'</span></div>'
					: ''
		}
		$(".hotelList").append('\
			<div class="hotelLi">\
			  	<div class="hotelLiTittle flexRow">\
			    	...
			    	<div style="margin-left:50px;">' + priceText + '<span class="activeFontColor">' + hitem.HotelFareAmount + '</span></div>\
					' + totalPrice + '\
				</div>\
		...
		if (ProfileInfo.onlineStyle !== 'APPLE') {
			// 房费
			serviceFee =
				hitem.HotelDailyFare !== '0CNY'	// 没金额后台返回的好像都是<string>0CNY
					? '<div class="hotelServiceFee flexRow">\
						<div style="width:220px">'+ get_lan('orderDetails').roomRate + hitem.HotelDailyFare + '</div>\
					</div>'
					: ''
		}
		...
		$(".hotelDetailsList").eq(index).append('\
	    	<div class="hotelDetailsLi">\
              	...
              	<div class="hotelLastTime flexRow ' + showLastTime + '">\
                	<div class="flexRow" style="width:220px;"><div class="lastTimeIcon">!</div>' + get_lan('orderDetails').CancelDeadLine + '</div>\
	    	  	...
	    	  	<div class="hotelAddress flexRow">\
				  	<div style="width:220px;">' + get_lan('orderDetails').address + '</div>\
					<span class="mainFontColor">' + hitem.HotelAddress + '</span>\
				</div>\
	    	  	<div class="hotelRoomName flexRow">\
				  	<div style="width:220px;">' + get_lan('orderDetails').RoomName + '</div>\
					' + hitem.RoomName + '\
				</div>\
	    	  	<div class="hotelBedName flexRow">\
				  	<div style="width:220px;">' + get_lan('orderDetails').BedName + '</div>\
					' + hitem.BedName + '\
				</div>\
	    	  	<div class="hotelBreakfast flexRow">\
				  	<div style="width:220px;">' + get_lan('orderDetails').Breakfast + '</div>\
					' + hitem.Breakfast + '\
				</div>\
              	<div class="hotelPayType flexRow">\
					<div style="width:220px;">' + get_lan('orderDetails').PayType + '</div>\
					' + hitem.PayType + '\
				</div>\
              	<div class="hotelLastArriveTime flexRow">\
			  		<div style="width:220px;">' + get_lan('orderDetails').LastTime + '</div>\
					' + hitem.LastTime + '\
				</div>\
				...
				<div class="hotelBusInfo flexRow ' + showBus + '" style="min-height:0px;">\
              	...
              	<div class="CabinCustomerName flexRow ' + showCabinCustomer + '">\
				  	<div style="width:220px;">' + get_lan('orderDetails').CabinCustomerName + '</div>\
					' + hitem.CabinCustomerName + '\
				</div>\
	    	</div>\
		')
...
function trainList(res) {
	...
	res.Train.map(function (titem, index) {
		...
		$(".trainBtnList").eq(index).html('\
			...
			'</div>\
	    	<div class="trainBtn ChangeTicketBtnTrain ' + showBtn2 + '" index="' + index + '" trainItemId="' + titem.TrainItemId + '" \
				searchTime="' + titem.TrainDeparteDate + '" isSelfBook="' + titem.IsSelfBook + '" customerID="' + titem.customerID + '" \
				customerDocment="'+ titem.customerDocment + '" customerDocmentType="' + titem.customerDocmentType + '">' + get_lan('orderDetails').refund +
			'</div>\
	...
	//退票
	function add12306Account(accountInfo12306) {
		$('.frequentCardsText').mLoading("show");
		$.ajax({
			type: 'post',
			url: $.session.get('ajaxUrl'),
			dataType: 'json',
			data: {
				url: $.session.get('obtCompany') + "/SystemService.svc/ShowMatchedPassengersNewPost",
				jsonStr: '{"goAirline":"null","backAirline":"null","newOrder":"0","key":' + netUserId + ',"Language":"' + obtLanguage + '","reginType":"T","dstCode":"TRAIN"}'
			},
			success: function (data) {
				$('.frequentCardsText').mLoading("hide");
				var matchedPassenger = JSON.parse(data)
				console.log(matchedPassenger)
				try {
					var account = '', pwd = ''
					matchedPassenger.map(function (passenger) {

						if (passenger.ID === accountInfo12306.customerID) {
							account = passenger.ChinaRailwayAccountName
							pwd = passenger.ChinaRailwayAccountPwd
						}
					})
					if (account != '' && pwd != '') {
						customer12306account = new Train12306account(accountInfo12306.customerID, accountInfo12306.customerName, account, pwd);
					} else {
						customer12306account = new Train12306account(accountInfo12306.customerID, accountInfo12306.customerName);
					}
					customer12306account.insertAccount('.frequentCardsText');
	...
	$(".ChangeTicketBtnTrain").unbind("click").click(function () {
		var index = parseInt($(this).attr("index"));
		var accountInfo12306 = {
			customerID: $(this).attr('customerID'),
			customerDocment: $(this).attr('customerDocment'),
			customerDocmentType: $(this).attr('customerDocmentType'),
			customerName: res.Train[index].nameP
		}
		var IsSelfBook = $(this).attr('IsSelfBook')
		if (obtLanguage == "CN") {
		...
		$(".refundTicketBody").html('\
            <div class="refundTicketPassenger">' + get_lan("refundTicketPop").refundTicketPassenger + ' ' + res.Train[index].nameP + '</div>\
            <div class="refundTicketInfo">\
             	<div class="refundDepartureTime">' + res.Train[index].TrainDeparteTime + '</div>\
             	<div class="refundArrow"></div>\
             	<div class="refundArriveTime">' + res.Train[index].TrainArriveTime + '</div>\
             	<div class="refundDepartureCity">' + res.Train[index].TrainDeparte + '</div>\
             	<div class="refundArriveCity">' + res.Train[index].TrainArrive + '</div>\
            </div>\
            ...
            <div class="passengerLi" customerid="'+ accountInfo12306.customerID + '">\
				<div class="frequentCardsText">\
				</div>\
			</div>\
			<div class="refundTicketLimit">\
				<div class="refundTicketLimitTittle">' + get_lan("refundTicketPop").refundTicketLimit + '</div>\
              	<div class="refundTicketLimitBody">' + refundTicketLimitBodyText + '</div>\
            </div>\
            ')
		//火车票退票费 icon_hotts
		if (IsSelfBook == 'true') {
			add12306Account(accountInfo12306)
		}
		...
		$(".sureRefundTicketBtn").unbind("click").click(function () {
			if (IsSelfBook == 'true') {
				$('body').mLoading("show");
				customer12306account.check12306account(0, function () {
					$('body').mLoading("hide");
					trainRefundAPI()
				})
			} else {
	...
	$(".alterBtnTrain").unbind("click").click(function () {
		...
		$(".alterTicketBody").html('\
            <div class="alterTicketPassenger">\
				' + get_lan("alterTicketPop").alterTicketPassenger + ' ' + res.Train[index].nameP + '\
			</div>\
            <div class="alterTicketInfo">\
             	<div class="alterDepartureTime">' + res.Train[index].TrainDeparteTime + '</div>\
             	<div class="refundArrow"></div>\
             	<div class="alterArriveTime">' + res.Train[index].TrainArriveTime + '</div>\
             	<div class="alterDepartureCity">' + res.Train[index].TrainDeparte + '</div>\
            	 <div class="alterArriveCity">' + res.Train[index].TrainArrive + '</div>\
            </div>\
            <div class="alterTicketDate">\
              	<div class="alterTicketDateBody">\
                	<div class="alterTicketDateTittle">' + get_lan("alterTicketPop").alterTicketDateTittle + '</div>\
            ...
            <div class="alterTicketLimit">\
				<div class="alterTicketLimitTittle">' + get_lan("alterTicketPop").alterTicketLimit + '</div>\
				<div class="alterTicketLimitBody">' + get_lan("alterTicketPop").alterTicketRemark + '</div>\
            </div>\
        ')
		// $(".alterTicketLimit").hide();
		$(".alterTicketDateBody").append('\
            <div class="alterDateTittle">' + get_lan("alterTicketPop").alterDateTittle + '</div>\
            <div class="alterCabinTittle">' + get_lan("alterTicketPop").alterTrainNumber + '</div>\
            <input id="alterDateInputTrain" readonly>\
            <input id="alterCabinSelect" placeholder="' + get_lan("alterTicketPop").optional + '">\
        ')
		if (ProfileInfo.onlineStyle != 'APPLE') {
			$('.alterTicketInfo').after($('.alterTicketDateTittle'))
		}
							...
							$.ajax({
								...
								success: function (data) {
									...
									var searchTrainInfo = {
										...
										'queryKey': orderInfo.Train[index].TrainDeparteCN + ',' + orderInfo.Train[index].TrainArriveCN + ',' +
	...
	res.Car.map(function (cItem, cIndex) {
		...
		$(".carList").append('\
			<div class="carLi">\
			  	<div class="carLiTittle flexRow">\
                	<div style="margin-left:27px;">' + orderInfo.OrderCustomer + '</div>\
			    	<div style="margin-left:50px;">' + cItem.CarDepAddress + ' - ' + cItem.CarArrAddress + '</div>\
			    	<div style="margin-left:50px;">\
						' + get_lan('orderDetails').orderState + '\
						<span class="activeFontColor">' + cItem.CarState + '</span>\
					</div>\
			    	<div style="margin-left:50px;">' + get_lan('orderDetails').price + '\
						<span class="activeFontColor">' + cItem.CarFareAmount + '</span>\
					</div>\
			  	</div>\
		...
		$(".carDetailsList").eq(cIndex).append('\
			<div class="carDetailsLi">\
			  	<div class="carCompany">' + carCompany + '</div>\
			  	<div class="carType">' + carType + '</div>\
			  	<div class="carPickUp">\
				  	<span style="color:#041e58;">' + get_lan("carBody").pickUp + '</span> ' + cItem.PickTime.split(" ")[0] + '\
				</div>\
			  	<div class="carPickOff">\
			  		<span style="color:#041e58;">' + get_lan("carBody").pickOff + '</span> ' + cItem.ReturnTime.split(" ")[0] + '\
				</div>\
			  	<div class="carPickUpTime">' + cItem.PickTime.split(" ")[1] + '</div>\
				<div class="carPickOffTime">' + cItem.ReturnTime.split(" ")[1] + '</div>\
				<div class="carPickUpAddress">' + cItem.CarDepAddress + '</div>\
				<div class="carPickOffAddress">' + cItem.CarArrAddress + '</div>\
		...
		$(".carBtnList").eq(cIndex).html('\
	    	<div class="carBtn cancelBtnCar ' + showBtn + '" ItemId="' + cItem.CarItemId + '" orderNo="' + cItem.orderNo + '">' + get_lan('orderDetails').orderCancel + '</div>\
	    	')
	})
	//取消订单
...
function otherList(res) {
	...
	orderPassenger = orderPassenger.substring(0, orderPassenger.length - 1);
	$(".otherList").html('\
	<div class="otherHeader">'+ orderPassenger + '\
		<div style="width:200px;text-align:right;position:absolute;right:30px;top:0;">'+ get_lan("orderInfo").OrderFare + '<span style="color:#FF7A00;" class="otherAmount"></span></div>\
	</div>\
	')
	if (res.TravelRequestNewMiscellList.length > 0) {
		$(".orderDetailsBody").append('\
		<div class="changeOtherBody hide"><span class="changeOther" orderNo="'+ res.OrderNo + '" style="cursor:pointer;">' + get_lan("otherList").change + '</span><span class="deleteOther" orderNo="' + res.OrderNo + '" style="cursor:pointer;margin-left:20px;">' + get_lan("otherList").delete + '</span></div>\
		')
		$(".changeOther").unbind("click").click(function () {
			var changeOtherInfo = {
				'orderNo': $(this).attr("orderNo"),
			}
			$.session.set('changeOtherInfo', JSON.stringify(changeOtherInfo));
			window.location.href = '../../others/others.html?state=change';
		})
		if (res.Segment.length == 0 && res.Hotel.length == 0 && res.Train.length == 0 && res.Car.length == 0) {
			$('.changeOtherBody').removeClass("hide");
		}
		if (!res.ShowGoOnBook) {
			$('.changeOtherBody').remove();
		}
		$('body').mLoading("show");
		var otherAmount = 0;
		$.ajax({
			...
			data: {
				...
				jsonStr: '{"orderNo":"' + orderInfo.OrderNo + '","id":' + netUserId + ',"companyId":"' + ProfileInfo.companyId + '","Language":"' + obtLanguage + '"}'
			},
			success: function (data) {
				var res = JSON.parse(data);
				console.log(res);
				$('body').mLoading("hide");
				res.map(function (item) {
					otherAmount += item.FarePaid;
					if (item.miscellDetails.length > 0) {
						var city = item.miscellDetails[0].DstCityName == "" ? item.miscellDetails[0].DstCityName : item.miscellDetails[0].OrgCityName + '-' + item.miscellDetails[0].DstCityName;
						var date = item.miscellDetails[0].DepTime.split(" ")[0] + '-' + item.miscellDetails[0].ArrTime.split(" ")[0];
					}
					if (item.hotelDetails.length > 0) {
						var city = item.hotelDetails[0].CityName;
						var date = item.hotelDetails[0].CheckIn.split(" ")[0] + '-' + item.hotelDetails[0].CheckOut.split(" ")[0];
					}
					$(".otherList").append('\
					<div class="otherLi flexRow">\
						<div style="width:100px;">'+ item.ProductName + '</div>\
						<div style="width:280px;">'+ get_lan("otherList").city + ':' + city + '</div>\
						<div style="width:250px;">'+ date + '</div>\
						<div style="width:400px;overflow:hidden;text-overflow:ellipsis;" title="'+ item.BookRemark + '"><span style="color:#666;">' + get_lan("otherList").remark + ':</span>' + item.BookRemark + '</div>\
						<div style="width:200px;text-align:right;position:absolute;right:0;top:0;"><span style="color:#666;">'+ get_lan("otherList").fare + ':</span><span style="color:#FF7A00;">' + item.FarePaid + item.Currency + '</span></div>\
					</div>\
					')
				})
				$(".otherLi").eq($(".otherLi").length - 1).css("border", 0);
				$(".otherAmount").text(otherAmount + res[0].Currency);
				//删除
				$(".deleteOther").unbind("click").click(function () {
					var r = confirm(get_lan('otherList').deleteOtherRemind);
					if (r == true) {
						res.map(function (item) {
							if (item.miscellDetails.length > 0) {
								var ajaxUrl = $.session.get('obtCompany') + "/OrderService.svc/SaveTravelRequestMiscellInfoPost";
								var request = '{"request":{"BCN":"' + orderInfo.OrderNo + '","Rid":' + item.miscellDetails[0].Rid + ',"id":' + netUserId + ',"ProductCodeID":"","FlightNo":"","Cabin":"","TrainNo":"","TrainSeat":"","fullRate":"","Currency":"","StartTime":"","EndTime":"","OrgCity":"","DstCity":"","OrgCityName":"","DstCityName":"","Remark":"","Language ":"' + obtLanguage + '","Operate":"DEL"}}'
							}
							if (item.hotelDetails.length > 0) {
								var ajaxUrl = $.session.get('obtCompany') + "/OrderService.svc/SaveTravelRequestMiscellHPost";
								var request = '{"request":{"BCN":"' + orderInfo.OrderNo + '","Rid":' + item.hotelDetails[0].Rid + ',"id":' + netUserId + ',"ProductCodeID":"","HotelName":"","RoomType":"","LanType":"","Breakfast":"","fullRate":"","Currency":"","CheckIn":"","CheckOut":"","City":"","CityName":"","Remark":"","Language ":"' + obtLanguage + '","Operate":"DEL"}}'
							}
							$('body').mLoading("show");
							$.ajax({
								type: 'post',
