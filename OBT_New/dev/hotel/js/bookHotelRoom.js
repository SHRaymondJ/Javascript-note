...
var usedBudget = 8800;
var serviceFee = 0, passengerNumber = 0
//中英文对象
var cn = {
	"closeText": "确定",
	"currentText": "当月",
	...
	"myPreference": {
		...
		"otherRequirement": "入住要求:",
		'checkOutRequirements': "离店要求:",
		"NoRequirement": "无要求",
		"KingBedRoom": "尽量大床房",
		"DoubleBedRoom": "尽量双床房",
		"NoSmokingRoom": "尽量无烟房",
	...
	"totalFare": "总金额:",
	"serviceFee": "服务费:",
	"serviceFeeReminder": "该订单需要收取服务费[SERVICEFEE]",
	"continueBook": "继续预订",
	"return": "返回",
...
var en = {
	...
	"myPreference": {
		...
		"otherRequirement": "Check-in Requirements:",
		'checkOutRequirements': "Check-out Requirements:",
	...
	"serviceFeeReminder": "There will be [SERVICEFEE] additional service charge for this booking.",
	"continueBook": "Continue",
	"return": "Return",
...
function initChart(orderAmount) {
	...
	$('.budgetRemain').html(remainAmount + 'CNY');
}
...
function showContent() {
	...
	$("#main").html(
		...
		'</div>\
	            <div class="passengerBody">\
    	          ...
    	          <div class="passengerBar flexRow">\
    	              <div class="passengerBarLi passengerLiName" style="width:250px;box-sizing:border-box;padding-left:45px;">' +
		...
		'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 120px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="bedTypeRequirement" value="no requirement" checked></div>' +
		get_lan('myPreference').NoRequirement +
		'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="bedTypeRequirement" value="大床"></div>' +
		get_lan('myPreference').KingBedRoom +
		'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="bedTypeRequirement" value="双床"></div>' +
		...
		'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 120px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="roomTypeRequirement" value="no requirement"checked ></div>' +
		get_lan('myPreference').NoRequirement +
		'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="roomTypeRequirement" value="无烟"></div>' +
		get_lan('myPreference').NoSmokingRoom +
		'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="roomTypeRequirement" value="吸烟"></div>' +
		get_lan('myPreference').SmokingRoom +
		'</div>\
                </div>\
                <div class="otherRequirement flexRow">\
                  <div class="requirementTittle">' + get_lan('myPreference').otherRequirement + '</div>\
                  <div class="flexRow" style="margin-right:40px;width: 120px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="otherRequirement" value="no requirement" checked></div>' + get_lan('myPreference').NoRequirement + '</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="otherRequirement" value="stay1"></div>' + get_lan('myPreference').StayLate + '</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="otherRequirement" value="stay2"></div>' + get_lan('myPreference').early + '</div>\
                </div>\
				<div class="checkOutRequirement flexRow">\
                  <div class="requirementTittle">' + get_lan('myPreference').checkOutRequirements + '</div>\
            ...
            <div class="creditCardInfo hide">\
              ...
              <div class="creditCardBody flexWrap">\
			  	...
			  	<div class="flexWrap">\
					...
					<div class="creditCardPolicy flexRow hide">\
              ...
              <div class="budgetBody">\
                ...
                <div class="budgetBox">\
                    <div class="budgetItem">\
                        ...
                        <p class="budgetAmount">'+ totalBudget + 'CNY</p>\
                    </div>\
                    <div class="budgetItem">\
                        <p class="budgetDesc">'+ get_lan('budget').budgetUsed + '</p>\
                        <p class="budgetAmount">'+ usedBudget + 'CNY</p>\
                    </div>\
                </div>\
              </div>\
            </div>\
			...
			<div class="hotelTips activeFontColor hide">'+get_lan('hotelTipsWithResourceTypeNine')+'</div>\
			<br><br>\
			<div class="totalFareContainer">\
    ...
    ')
	$('.creditCardDocumentTypeSelect').unbind('change').bind('change',function(){
		if($(this).val() == 1){
			$('.creditCardDocumentNumberInput').val(customerIDNumber)
		}else{
			$('.creditCardDocumentNumberInput').val('')
		}
	})
	...
	$.ajax({
		...
		success: function(data) {
                ...
                <div class="orderDetailBody"">\
                  ...
                  <div class="hotelDetailBody">\
					...
					<div class="orderHotelDailyRate">\
						...
						<span class="hotelAveragePerNight hide">' + get_lan("averagePerNight") + '</span>\
					</div>\
			...
			ratePlans.LocalCurrency = res.RoomInfos[0].RatePlans[0].LocalCurrency
			ratePlans.LocalTotalFare = res.RoomInfos[0].RatePlans[0].LocalTotalFare
			ratePlans.TotalFare = res.RoomInfos[0].RatePlans[0].TotalFare
			ratePlans.Currency = res.RoomInfos[0].RatePlans[0].Currency
			var totalFareText = ratePlans.LocalCurrency ? '<span class="TotalFare">'+ ratePlans.LocalTotalFare +'</span>'+
				'<span style="font-size:12px;margin-left:5px;">' + ratePlans.LocalCurrency + '</span>' : '<span class="TotalFare">'+ratePlans.TotalFare +'</span>'+
				'<span style="font-size:12px;margin-left:5px;">' + ratePlans.Currency + '</span>'
			$(".totalFareBody").html('\
                <span style="font-size:13px;color:#000;margin-right:20px;">' + get_lan('totalFare') + '</span>' + totalFareText + '\
            ')
...
/**
 * 更新服务费
 * @param {serviceFee, passengerNumber} obj
 */
function updateServiceFee(obj){
	if (obj.serviceFee) {
		serviceFee = Number(obj.serviceFee)
	}
	if (obj.passengerNumber) {
		passengerNumber = Number(obj.passengerNumber)
	}
	var total = serviceFee * passengerNumber
	var serviceFeeDom = ''
	var currency = ProfileInfo.OfficeCurrency
	if(total) {
		serviceFeeDom = '<p class="serviceFee"><span class="serviceFeeTitle">'+get_lan('serviceFee')+'</span><span>' + total + currency + '</span></p>'
	}
	$('.serviceFeeBody').html(serviceFeeDom)
}
/*需不需要信用卡*/
		...
		success: function(data) {
			...
			if (!res.IsCentralFapiao) {
				...
				if(hotelChooseInfo.FuXunPayType == "公司支付" || hotelChooseInfo.FuXunPayType == "Central Pay"){
					...
					// $(".totalFareBody").css({"position":"relative","margin":"40px 0 0 0px","text-align":"right","width":"100%"});
					if(hotelChooseInfo.HotelResourceType != '9') {
						$('.hotelTips').text(get_lan('hotelTipsWithoutResourceTypeNine')) 
					}
				}else{
					// $(".totalFareBody").css("margin", "40px 0 0 950px");
				}
			} else {
				$(".centralFapiao").removeClass("hide");
			}
			updateServiceFee({serviceFee: res.ServiceFee})
				...
				if(res.AcceptedCreditCards!=null && res.AcceptedCreditCards.length>0 && ProfileInfo.onlineStyle!="APPLE"){
					...
					res.AcceptedCreditCards.map(function(item){
						if(format(item) ==format("AMERICAN EXPRESS") || format(item)==format("Diners") || format(item)==format("Union") || format(item)==format("JCB") || format(item)==format("Master") || format(item)==format("VISA") || format(item)==format("DISCOVER CARD")|| format(item)==format("WORLD BANK")){
							creditCardImgList += '<img src="./images/CreditCardType/'+format(item)+'.png">'
						}
					})
					if(creditCardImgList != ''){
						$('.creditCardTypeBody').html(creditCardImgList)
					}
				}
...
function surePassengerInfo() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(passengerJson.AbacusPriceNotContainTax && !ProfileInfo.ShowHigherHotelDailyPrice){
				$('.hotelAveragePerNight').removeClass('hide');	//平均每日价
			}
	...
	$.ajax({
		...
		success: function(data) {
			...
			res.DocumentTypeList.map(function(item) {
				...
				$(".creditCardDocumentTypeSelect").append('<option value="' + item.Rid + '">' + item.Name + '</option>');
			})
		},
		error: function() {
			// alert('fail');
...
function passengersInOrder(customerState) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			customerIDNumber = ''
			$(".passengerList").html('');
			updateServiceFee({passengerNumber: res.length})
			res.map(function(item, index) {
				if(index == 0) {
					// 添加身份证, 用的都是第一个人的
					item.Documents.map(function(doc){
						if(doc.Rid == '1'){
							customerIDNumber = doc.DocumentNumber
						}
					})
				}
				...
				$(".passengerList").append('\
					...
					'">\
                    <div class="passengerLiDiv passengerLiName" style="width:250px;text-align:left;padding-left:45px;box-sizing:border-box;"><span class="PassengerNameText">' +
					item.NameCN + '</span><span class="changePassengerInfo" index="' + index +
					'" customerId="' + item.ID + '">' + changePassengerInfo +
					'</span></div>\
			...
			//价格
			if(ratePlans.LocalCurrency != "null" && ratePlans.LocalCurrency){
				var allPrice = ratePlans.LocalTotalFare*res.length
			}else{
				var allPrice = ratePlans.TotalFare*res.length
			}
			if(ProfileInfo.onlineStyle != "APPLE"){
				$('.passengerLiName').css('width','235px');
			}
			...
			if (!ProfileInfo.HideMeberShip) {
				if (ProfileInfo.onlineStyle == "APPLE") {
					...
					if (res[0].HotelCards && res[0].HotelCards.length > 0) {
...
function reserveHotel(res, payRes) {
	...
	var preference = $('input[name="bedTypeRequirement"]:checked').val() + "," + $('input[name="roomTypeRequirement"]:checked').val() + ","
	 + $('input[name="otherRequirement"]:checked').val() + ',' + $('input[name="checkOutRequirement"]:checked').val();
	if ($(".lastestTimeSelect option:selected").val() == "24:00") {
		var s1 = '2201';
		var s2 = '-0041';
	} else {
	...
	if (res.NeedPay) {} else {
		...
		var finalQueryKey = queryKeySelect.split(',')[0] + ',' + queryKeySelect.split(',')[1] + ',' + s1 + ',' + s2 + ',' +
			...
			hotelChooseInfo.GuestType + ',' + ratePlans.TotalFare + ',' + reasoncode + ',,' + changeHotelId + ',false';
		...
		if(serviceFee * passengerNumber > 0) {
			var currency = ProfileInfo.OfficeCurrency
			var msg = '<img src="/images/info.png"/>' + get_lan('serviceFeeReminder').replace('[SERVICEFEE]', (serviceFee*passengerNumber+currency))
			var continueBook = eTravelConfirm(msg, get_lan('continueBook'), get_lan('return'))
			continueBook(ajax_book)
		}else{
			ajax_book()
		}
