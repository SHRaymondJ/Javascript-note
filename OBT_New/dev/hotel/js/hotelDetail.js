...
var cn = {
	...
	"faPiaoRemind":"CENTRAL FAPIAO – this hotel sends the fapiao to Apple. You should not collect the fapiao when checking out.",
	priceType: {
		1: '协议价格',
		2: '第三方价格',
		3: '第三方价格',
...
function showContent(res){
	...
	var showFaPiao = res.IsCentralFapiao?"":"hide";
	$("#main").html('\
		<div class="hotelImgPop hide">\
          ...
          <div class="pic_pop">\
			...
			<div style="float: left;min-height: 350px;width: 312px;">\
		...
		<div class="autoCenter">\
		  ...
		  <div style="position:relative;margin-top:28px;">\
		    ...
		    <div id="hotelMap"></div>\
			<div id="mapboxMap" style="height:380px;width:306px;display:block;position:relative;margin-left:7px;"></div>\
		    </div>\
		  </div>\
		</div>\
	')
	...
	var hotelInfoPrice = res.LocalCurrency==null?res.LowestPrice+'<span style="font-size:18px;margin:0 5px 0 5px">'+currency+'</span>':res.LocalLowestPrice+'<span style="font-size:18px;margin:0 5px 0 5px">'+res.LocalCurrency+'</span>';
	...
	$(".hotelInfoBody").html('\
		...
		<div style="position:relatove;">\
			...
			<div class="hotelInfoBusRemind flexRow '+showFaPiao+'" style="font-weight:600;">\
				<img src="./images/fapiao.png" style="display:block;width:26px;height:24px;margin:-2px 15px 0 0;">\
				'+get_lan("faPiaoRemind")+'\
			</div>\
		</div>\
...
function hotelRoomList(res){
	...
	res.RoomTypes.map(function(item,index){
		...
		$(".hotelRoomList").append('\
			<div class="hotelRoomLi">\
			  '+outLiRoom+'\
			  <div class="flexRow">\
			    <div class="roomImg '+roomImgHidden+'" style="background-image:url('+roomImg+')" index="'+index+'"></div>\
		...
		item.RateInfos.map(function(bedItem,bedIndex){
			...
			if(ProfileInfo.onlineStyle!="APPLE" ){
				if(bedItem.CancelPolicy){
					cancelPolicy = '<div class="cancelPolicy cancelPolicy'+index+'">'+bedItem.CancelPolicy+'</div>';
					cancelTips = ' tips';
				}else if(bedItem.Policy){
					cancelPolicy = '<div class="cancelPolicy cancelPolicy'+index+'">'+bedItem.Policy+'</div>';
					cancelTips = ' tips';
				}
			}
			...
			console.log(bedItem)
			var currency = bedItem.Currency || 'CNY'
			$(".bedList").eq(index).append('\
				<div class="bedLi flexRow ">\
					'+inLiRoom+'\
				  <div class="bedLiText" style="width: 80px;margin-left: 16px;">'+item.BedType+'</div>\
				  <div class="bedLiText bedBreakfast" style="width: 85px;margin-left: 35px;">'+bedItem.BreakFast+'</div>\
				  <div class="bedLiText bedWLAN" style="width: 60px;margin-left: 19px;">'+item.WLAN+'</div>\
				  <div style="position:relative;width: 105px;margin-left: 5px;font-size: 18px;text-align: center;color: #'+DailyRateColor+';" class="'+shakeHands+'">\
				  	<span class="hotelDailyRateText '+rateUnderline+'" roomInfo="'+item.HotelRoomType+','+bedItem.RatePlanCode+'">'+hotelDailyRate+'</span>\
					<span style="font-size:14px;margin-left:3px;">'+hotelDailyCurrency+'</span>\
				  ...
				  <div class="bedLiPolicyRule '+PolicyRuleShow+' '+policyUnderline+' '+ruleMarginLeft+cancelTips+'" Policy="'+bedItem.Policy+'">'+bedItem.PolicyRule + cancelPolicy + '</div>\
				  <div class="bedLiPolicyRule '+freeCancel+' '+policyUnderline+' '+ruleMarginLeft+cancelTips+'" Policy="'+bedItem.Policy+'">'+cancelText+ cancelPolicy + '</div>\
				  <div class="bedLiBookBody '+showBookBtn+'">' + priceTypeDom +
			...
			bedDom.css('top',(-18-bedDom.height()+'px'));
	...
	if(res.AbacusPriceNotContainTax && !ProfileInfo.ShowHigherHotelDailyPrice){
		if(obtLanguage=="CN"){
	...
	if(ProfileInfo.onlineStyle != "APPLE"){
		if(!res.ManualHotelReferenceDisplay || !$('.agreementHotel')[0] ){
			$('.hotelTabBody').append('<div class="lineType2"><img src="../../train/images/icon_hotts_bcd.svg">'+get_lan('hotelRoomList').agreementTips+'</div>');
		}
	}
