if (JSON.parse(sessionStorage.getItem('ProfileInfo')).onlineStyle == 'APAC') {
    (function ($) {
        var hotelCityString = '';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (!isJsonString(data) && TAnumber && TAnumberIndex) {
                    ...
                    console.log(JSON.parse(data))
                    if (TAnumber && $.session.get('TAOneCity') != 1) {
                        cityFilter("2", hotelCityJson, hotelCity, "intel")
                    } else {
                        cityData(hotelCityJson, hotelCity);
                    }
                }
        ...
        KuCity.prototype = {
            ...
            creatItem: function () {
                ...
                $(".kucity").remove();
                var template = '<div class="kucity"><div class="citybox"><div class="kucity_body"></div></div><ul class="result"></ul></div>';
                if (TAnumber && $.session.get('TAOneCity') != 1) {
                    ...
                    for (var i = 0, iLen = itemKey.length; i < iLen; i++) {
                        ...
                        for (var j = 0, jLen = city[group][itemKey[i]].length; j < jLen; j++) {
                            ...
                            switch ($.session.get('obtLanguage')) {
                                case 'CN':
                                    str += '<span code="' + code + '" cn="' + city[group][itemKey[i]][j].NameCN + '" en="' + city[group][itemKey[i]][j].NameEN + '">' + city[group][itemKey[i]][j].NameCN + '</span>'
                                    break;
                                case 'EN':
                                    str += '<span code="' + code + '" cn="' + city[group][itemKey[i]][j].NameCN + '" en="' + city[group][itemKey[i]][j].NameEN + '">' + city[group][itemKey[i]][j].NameEN + '</span>'
                                    break;
                            }
                        }
                ...
                if (localStorage.getItem('recentSearch') && JSON.parse(localStorage.getItem('recentSearch')).hotel) {
            ...
            creatResult: function (city, value) {
                ...
                if (!!len) {
                    for (var i = 0; i < len; i++) {
                        ...
                        str += '<li><span class="name" code="' + searchCode + '" cn="' + allCity[i].NameCN + '" en="' + allCity[i].NameEN + '">' + allCity[i].NameCN + '</span><span class="letter">' + allCity[i].NameEN + '</span></li>'
                    }
            ...
            citySelect: function () {
                ...
                $('.kucity_item dd').on('click', 'span', function (e) {
                    ...
                    self.target.attr("cityCode", (($(e.target).attr('cityCode')) || ($(e.target).attr('code'))));
                    self.target.attr("cn", ($(e.target).attr('cn')));
                    self.target.attr("en", ($(e.target).attr('en')));
                    if ($(e.target).attr('departureDate') && $(e.target).attr('returnDate')) {
                        $("#hotelDepartureDate").val($(e.target).attr('departureDate'));
                        $("#hotelReturnDate").val($(e.target).attr('returnDate'));
                    }
                    self.container.hide();
                })
            },
                    ...
                    if (current !== -1) {
                        switch (e.keyCode) {
                            ...
                            case 13:
                                ...
                                self.target.attr("cityCode", (self.resultct.find('.active .name').attr('cityCode') || self.resultct.find('.active .name').attr('code')));
                                self.target.attr("cn", self.resultct.find('.active .name').attr("cn"));
                                self.target.attr("en", self.resultct.find('.active .name').attr("en"));
        ...
        $.fn.kuCity = function (options) {
            ...
            target.on('focus', function (e) {
                ...
                kucity.resultct.on('click', 'li', function () {
                    ...
                    kucity.target.attr("cityCode", ($(this).find('.name').attr('cityCode') || $(this).find('.name').attr('code')));
                    kucity.target.attr("cn", $(this).find('.name').attr('cn'));
                    kucity.target.attr("en", $(this).find('.name').attr('en'));
                    kucity.triggleShow('all');
                    /*酒店*/
                    if (kucity.target.attr("id") == "hotelCity") {
                        $('body').mLoading("show");
                        $.ajax({
                            type: 'post',
                            url: $.session.get('ajaxUrl'),
