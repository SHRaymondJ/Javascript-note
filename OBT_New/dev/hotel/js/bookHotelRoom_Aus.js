...
var cn = {
	...
	"passengerInfo": {
		...
		"remarks": "账单信息",
		"changePassengerInfoApple": "[修改信息]",
		"changePassengerInfoBCD": "修改信息",
	...
	"myPreference": {
		...
		"tipsApple": "( 您的需求会尽量转达给酒店，具体安排以酒店为准。)",
		"tipsBCD": "您的需求会尽量转达给酒店，具体安排以酒店为准。",
		"bedTypeRequirement": "床型要求:",
		"roomTypeRequirement": "房型要求:",
		"otherRequirement": "时间要求:",
		"checkInRequirement":"入住时间要求",
		"checkOutRequirement":"离店时间要求",
		"NoRequirement": "无要求",
		"KingBedRoom": "尽量大床房",
		"DoubleBedRoom": "尽量双床房",
		"NoSmokingRoom": "尽量无烟房",
	...
	"creditCardInfo": {
		...
		"creditCardtipsBCD":"您的预订将使用信用卡担保。担保后，酒店或BCD代理商可能会扣除您首晚房费作为担保费用。",
		"creditCardtipsOther":"您的预订将使用信用卡担保。担保后，酒店或代理商可能会扣除您首晚房费作为担保费用。",
		...
		'creditCardResourceReminder': '请提供卡号前两位数字是“62”的银联信用卡进行担保'
	},
	"budget": {
		"budgetTittle": "预算信息",
		"budgetChartSubtittle": "剩余",
		"budgetRemain": "剩余预算",
		"budgetCurrentYear": "本年度总预算",
		"budgetUsed": "已用预算"
	},
	"hotelReason": "原因：",
	...
	"totalFare": "总金额:",
	"hotelTipsWithResourceTypeNine":"请注意：您选择的是公司支付酒店，房费将由公司支付，其它费用请自行与酒店结算。预订成功后将自动担保，若晚于取消时限取消预订，酒店将扣除首晚房费。",
	"hotelTipsWithoutResourceTypeNine":"请注意：您选择的是公司支付酒店，房费将由公司支付，其它费用请自行与酒店结算。您做的信用卡担保，将会在您入住后自动退还。若晚于取消时限取消预订，酒店将扣除首晚房费。",
	"bookTicket": "预定",
...
var en = {
	...
	"orderDetail": {
		...
		"reselectionApple": "[Modify]",
		"reselectionBCD": "Modify",
		"checkIn": "Check In:",
		"checkOut": "Check Out:",
	...
	"passengerInfo": {
		...
		"changePassengerInfoApple": "[Modify]",
		"changePassengerInfoBCD": "Modify",
	...
	"myPreference": {
		...
		"tipsApple": "( Kindly notice your special request(s) cannot be guaranteed, the property will do its best to meet your needs upon your check-in. )",
		"tipsBCD": "Kindly notice your special request(s) cannot be guaranteed, the property will do its best to meet your needs upon your check-in.",
		"bedTypeRequirement": "Bed Type Requirement:",
		"roomTypeRequirement": "Room Requirement:",
		"otherRequirement": "Other Requirements:",
		"checkInRequirement":"Check-in Requirements",
		"checkOutRequirement":"Check-out Requirements",
	...
	"creditCardInfo": {
		...
		"creditCardtipsBCD":"Please note your hotel booking will be guaranteed by credit card.",
		"creditCardtipsOther":"Please note your hotel booking will be guaranteed by credit card.",
		...
		'creditCardResourceReminder': 'Please use Union Pay credit card start with “62” to do the guarantee'
	},
	"budget": {
		"budgetTittle": "Budget Information",
		"budgetChartSubtittle": "Remainded",
		"budgetRemain": "Remainded Budget",
		"budgetCurrentYear": "Current Year",
		"budgetUsed": "Used"
	},
	...
	"hotelTipsWithResourceTypeNine":"Friendly reminder: You’ve selected a centrally paid room rate and payment will be made by your company. For other miscellaneous expenses, please settle directly with the hotel upon check-out. Once your reservation is confirmed, your room is guaranteed automatically and cancellation policy applies. You’ll incur a cancellation penalty equivalent to one room night should you decide to cancel your reservation after the cancellation deadline.",
	"hotelTipsWithoutResourceTypeNine":"Friendly reminder: You’ve selected a centrally paid room rate and payment will be made by your company. For other miscellaneous expenses, please settle directly with the hotel upon check-out. Your credit card guarantee will be returned to you automatically once you’ve checked in. You’ll incur a cancellation penalty equivalent to one room night should you decide to cancel your reservation after the cancellation deadline.",
	"bookTicket": "Book",
	...
	"centralFapiao": "I acknowledge that the hotel will send a centralized fapiao to Apple. I do not need to request a fapiao from the hotel.",
	...
	"OTReason" : "Please enter the reason",
    'averagePerNight':'average per night',
	'priceDetails':"Price Details",
	"remindTittle":"Read Before Booking",
}
// 控制苹果和其他环境文本
var creditCardtips = '',tips = '', reselection = '', changePassengerInfo = '';
if(ProfileInfo.onlineStyle == 'APPLE'){
	creditCardtips = ''
	tips = get_lan('myPreference').tipsApple
	selection = get_lan('orderDetail').reselectionApple
	changePassengerInfo = get_lan('passengerInfo').changePassengerInfoApple
}else{
	if(ProfileInfo.onlineStyle === 'BCD'){
		creditCardtips = get_lan('creditCardInfo').creditCardtipsBCD
	}else{
		creditCardtips = get_lan('creditCardInfo').creditCardtipsOther
	}
	tips = get_lan('myPreference').tipsBCD
	selection = get_lan('orderDetail').reselectionBCD
	changePassengerInfo = get_lan('passengerInfo').changePassengerInfoBCD
} 
$(function() {
	showContent(); //内容展示
...
function addRemark(res){
	...
	$('body').append('<div class="fixed">'+
		'<div class="ReasonGroup">'+
			...
			'<div class="ReasonOption">'+
				...
				'</select>'+
			'</div>'+
			'<textarea placeholder="' + get_lan('OTReason') + '" class="popOTReason"></textarea>' + 
	...
	$(".Reasonbtn").unbind("click").click(function() {
		if($('.popReasonSelect').val()==null){
			...
			$('.hotelOTReason').val($('.popOTReason').val())
			$('.fixed').hide()
		}
	})
...
function initChart(orderAmount) {
	// 基于准备好的dom，初始化echarts实例
	var myChart = echarts.init(document.getElementById("eChart"));
	var remainAmount = totalBudget - usedBudget - orderAmount,
		totalAmount = totalBudget;
	remainAmount = remainAmount <= 0 ? 0 : remainAmount;
	var remainPercentage = parseInt(parseInt(remainAmount) / parseInt(totalAmount) * 100);
	var subText = remainPercentage + "%";
	var color = remainPercentage < 20 ? "#F6AA25" : "#8DB15C"
	// 指定图表的配置项和数据
	option = {
		title: {
			text: " ",
		...
		series: [{
			type: "bar",
			data: [{
				name: get_lan("budget").budgetChartSubtittle,
				value: remainAmount,
				itemStyle: {
					normal: {
						color: color
	...
	$('.budgetRemain').html(remainAmount + ProfileInfo.OfficeCurrency);
}
//内容展示
function showContent() {
	...
	$("#main").html(
		...
		'</div>\
            ...
            <div class="myPreference" style="width:840px;background-color: #fff;">\
              <div class="preferenceTittle">\
			  	<span>' + get_lan('myPreference').myPreferenceTittle + '</span>\
				<span class="preferenceSubTittle">' + tips + '</span>\
			  </div>\
              <div class="preferenceBody">\
			  	<div class="bedTypeRequirement flexRow">\
                  <div class="requirementTittle">' +get_lan('myPreference').bedTypeRequirement +'</div>\
                </div>\
                <div class="bedTypeRequirement flexRow">\
                  <div class="flexRow" style="margin-right:40px;width: 120px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="bedTypeRequirement" value="no requirement" checked></div>' +get_lan('myPreference').NoRequirement +'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="bedTypeRequirement" value="大床"></div>' +get_lan('myPreference').KingBedRoom +'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="bedTypeRequirement" value="双床"></div>' +get_lan('myPreference').DoubleBedRoom +'</div>\
                </div>\
				<div class="roomTypeRequirement flexRow">\
                  <div class="requirementTittle">' +get_lan('myPreference').roomTypeRequirement +'</div>\
                </div>\
                <div class="roomTypeRequirement flexRow">\
                  <div class="flexRow" style="margin-right:40px;width: 120px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="roomTypeRequirement" value="no requirement"checked ></div>' +get_lan('myPreference').NoRequirement +'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="roomTypeRequirement" value="无烟"></div>' +get_lan('myPreference').NoSmokingRoom +'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="roomTypeRequirement" value="吸烟"></div>' +get_lan('myPreference').SmokingRoom +'</div>\
                </div>\
				<div class="checkInRequirement flexRow">\
                  <div class="requirementTittle">' +get_lan('myPreference').checkInRequirement +'</div>\
                </div>\
                <div class="checkInRequirement flexRow">\
                  <div class="flexRow" style="margin-right:40px;width: 120px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="checkInRequirement" value="no requirement" checked></div>' +get_lan('myPreference').NoRequirement +'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="checkInRequirement" value="stay1"></div>' +get_lan('myPreference').StayLate +'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="checkInRequirement" value="stay2"></div>' +get_lan('myPreference').early +'</div>\
                </div>\
				<div class="checkOutRequirement flexRow">\
                  <div class="requirementTittle">' +get_lan('myPreference').checkOutRequirement +'</div>\
                </div>\
                <div class="checkOutRequirement flexRow">\
                  <div class="flexRow" style="margin-right:40px;width: 120px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="checkOutRequirement" value="no requirement" checked></div>' +get_lan('myPreference').NoRequirement +'</div>\
                  <div class="flexRow" style="margin-right:40px;width: 110px;"><div style="margin-top:1px;"><input type="radio" class="requirementRadio" name="checkOutRequirement" value="stay3"></div>' +get_lan('myPreference').LateCheckout +'</div>\
                </div>\
              </div>\
            </div>\
			<div class="creditCardPolicy hide">\
            ...
            <div class="creditCardInfo hide">\
			  ...
			  '<span style="display: inline-block;min-width: 140px;">'+get_lan('creditCardInfo').creditCardTittle +'</span>'+'<span class="creditCardtips hide">'+creditCardtips+'</span>'+'</div>\
              <div class="creditCardBody flexWrap">\
			  	<p class="creditCard-resourceReminder"></p>\
			  	<div class="flexWrap">\
				  	<div>\
			...
			<div class="lastestTime hide">\
              <span class="lastestTimeDes">' +get_lan('lastestTime') +'</span>\
			...
			<div class="budgetInfo">\
              <div class="budgetTittle">'+ get_lan('budget').budgetTittle + '</div>\
              <div class="budgetBody">\
                <div class="budgetBox">\
                    <div class="budgetItem">\
                        <div class="budgetChartBox">\
                            <div class="budgetChart" id="eChart"></div>\
                            <p class="budgetChartSubtittle">'+ get_lan('budget').budgetChartSubtittle + '</p>\
                        </div>\
                    </div>\
                    <div class="budgetItem">\
                        <p class="budgetDesc">'+ get_lan('budget').budgetRemain + '</p>\
                        <p class="budgetAmount"><b class="budgetRemain"></b></p>\
                    </div>\
                </div>\
                <div class="budgetBox">\
                    <div class="budgetItem">\
                        <p class="budgetDesc">'+ get_lan('budget').budgetCurrentYear + '</p>\
                        <p class="budgetAmount">'+ totalBudget + ProfileInfo.OfficeCurrency+'</p>\
                    </div>\
                    <div class="budgetItem">\
                        <p class="budgetDesc">'+ get_lan('budget').budgetUsed + '</p>\
                        <p class="budgetAmount">'+ usedBudget + ProfileInfo.OfficeCurrency+'</p>\
                    </div>\
                </div>\
              </div>\
            </div>\
            <div class="hotelReason hide">\
              ...
              <select class="hotelReasonSelect">\
				...
				<textarea placeholder="' + get_lan('OTReason') + '" class="hotelOTReason"></textarea>\
			</div>\
			<div class="remindInfo">\
              <div class="remindTittle">'+get_lan("remindTittle")+'</div>\
              <div class="covidReminder"></div>\
			  <div class="hotelTips hide">·'+get_lan('hotelTipsWithResourceTypeNine')+'</div>\
            </div>\
			<div class="fareBody">\
				<div class="fareBodyTittle">'+get_lan("priceDetails")+'</div>\
				<div class="dailyRate">'+get_lan('orderDetail').DailyRate+'<span class="dailyRateText" style="color:#111C4E;font-weight:600;"></span></div>\
				<div class="totalFare">'+get_lan('totalFare')+'<span class="totalFareText" style="font-size:24px;font-weight:600;color:#FF6720;"></span></div>\
			</div>\
	...
	if(!ProfileInfo.ShowBudgetBook){
        $('.budgetInfo').hide();
    }
	// 苹果时不显示
	if (ProfileInfo.onlineStyle != "APPLE") {
		$(".creditCardtips").removeClass("hide")
		$('.hotelReason').after($('.creditCardPolicy'))	//非苹果样式不同
		$('.creditCardBody').css('min-height','0px'); //非苹果样式不同
		$('.passengerLiName').css('width','235px');
    }else{
		$('.covidReminder').hide();
	}
    if(!ProfileInfo.ReminderCnForCOVID && !ProfileInfo.ReminderEnForCOVID){
        $('.covidReminder').hide();
    }else{
        var covidReminder = obtLanguage == 'CN' ? ProfileInfo.ReminderCnForCOVID : ProfileInfo.ReminderEnForCOVID;
        $('.covidReminder').text('·'+covidReminder);
    }
	...
	$(".progressBar").html(
		'\
	    <div class="progressLine progressActiveBack"></div><span class="progressActiveFont">' +
		get_lan('progressBar').search +
		'</span>\
	    <div class="progressLine progressActiveBack"></div><span class="progressActiveFont">' +
		get_lan('progressBar').book +
		'</span>\
	    <div class="progressLine progressBackColor"></div>' +
	...
	$.ajax({
		...
		success: function(data) {
			...
			var rateUnderline = res.RoomInfos[0].RatePlans[0].Rates.length > 1 ? "rateUnderline" : "";
			var dailyRateBodyString = '';
			if (res.RoomInfos[0].RatePlans[0].Rates.length>1) {
				res.RoomInfos[0].RatePlans[0].Rates.map(function(rItem) {
					// var displayRate = rItem.MemberRate;
					var displayRate = (rItem.LocalCurrency && rItem.LocalCurrency != "") ? rItem.LocalMemberRate : rItem.MemberRate;	//新
					if (rItem.Date != null) {
						dailyRateBodyString += '<div class="dailyRateBodyLi"><div class="dailyRateBodyLiTittle">' + rItem.Date.substring(
							5, 10) + '</div><div class="dailyRateBodyLiContent">' + displayRate + '</div></div>'
					}
				})
			}
			if(ProfileInfo.onlineStyle=="APPLE"){
			...
			// 	'<span class="currencyFont">' + hotelChooseInfo.LocalCurrency + '</span>' : hotelChooseInfo.DailyRate +
			// 	'<span class="currencyFont">' + hotelChooseInfo.currency +'</span>'
			var dailyRateText = hotelChooseInfo.LocalCurrency != "null"?hotelChooseInfo.LocalDailyRate+hotelChooseInfo.LocalCurrency:hotelChooseInfo.DailyRate+hotelChooseInfo.currency;
			$(".dailyRateText").text(dailyRateText);
			var showBedType = res.RoomInfos[0].BedType == null ? "hide" : "";
			var selectionColor = ProfileInfo.onlineStyle == 'APPLE' ? 'specificFontColor' : '';
			$(".orderDetail").append(
				'\
                <div class="orderDetailBody"">\
                  <div class="orderHotelImg" style="background-image:url(' + ImageUrl + ')"></div>\
                  <div class="hotelDetailBody">\
					<div class="orderHotelName">\
						<span>' + res.Name + '</span>\
						<div class="reselection ' + selectionColor + '">' + reselection + '</div>\
					</div>\
					<div class="orderHotelAddress">' + res.Address + '</div>\
					<div class="orderHotelInfo">\
						<span class="breakfast">' + breakfast +'</span>\
						<span style="margin-left:20px;">Wi-Fi</span>\
						<span style="margin-left:20px;" class="' + showBedType + '">' + res.RoomInfos[0].BedType + '</span>\
						<span style="margin-left:20px;">' + res.RoomInfos[0].RoomName + '</span>\
					</div>\
					<div class="orderCheckInDate flexRow">\
						<div class="orderDateDes">' + get_lan('orderDetail').checkIn + '</div>\
						<div class="orderDate">' + queryKeySelect.split(',')[0] + ' ' + getWeek(queryKeySelect.split(',')[0]) + '</div>\
					</div>\
					<div class="orderCheckOutDate flexRow">\
						<div class="orderDateDes">' + get_lan('orderDetail').checkOut + '</div>\
						<div class="orderDate">' + queryKeySelect.split(',')[1] + ' ' + getWeek(queryKeySelect.split(',')[1]) + '</div>\
					</div>\
					\
                  </div>\
			...
			')
			// <div class="orderHotelDailyRate">\
			// 	<div class="hotelDailyRate">\
			// 		<span style="font-size:14px;">' + get_lan('orderDetail').DailyRate + '</span>\
			// 		<span class="hotelDailyRateText ' + rateUnderline + '">' + dailyRateText + '</span>\
			// 	</div>\
			// 	<span class="hotelAveragePerNight">' + get_lan("averagePerNight") + '</span>\
			// </div>\
			...
			// 	'</div>\
			// 	<div class="dailyRateBodyContent flexWrap">' + dailyRateBodyString + '</div>\
			// </div>\
			if (res.RoomInfos[0].HotelType === '1B') {
				var jsonObj = {
					queryKey: queryKeySelect,
				...
				$.ajax({
					...
					success: function(data) {
						var res = JSON.parse(data)
						if(res.Code == '200') {
							$('.breakfast').text(res.NewBreakfast)
						} else {
							alert(res.ErrInfo)
						}
					}
			...
			var totalFareText = hotelChooseInfo.LocalCurrency != "null"?hotelChooseInfo.LocalTotalFare+hotelChooseInfo.LocalCurrency:hotelChooseInfo.TotalFare+'CNY';
			$(".totalFareText").html(totalFareText);
			// $(".totalFareBody").html('\
            //     <span style="font-size:13px;color:#000;margin-right:20px;">' +
			// 	get_lan('totalFare') + '</span>' + totalFareText + '\
            //   ')
			initChart(parseInt($('.TotalFare').text()));
			/*班车信息*/
			$.ajax({
				type: 'post',
				url: $.session.get('ajaxUrl'),
						...
						if (res.infos.length > 0) {
							...
							$(".selectMorning").change(function() {
								...
								if($(this).children('option:selected').val()==""){
									$(".selectMorningText").eq(index).text('');
								}
							})
							$(".selectEvening").change(function() {
								...
								if($(this).children('option:selected').val()==""){
									$(".selectEveningText").eq(index).text('');
								}
							})
...
function needCreditCard(queryKeySelect) {
		...
		success: function(data) {
			...
			if (!res.IsCentralFapiao) {
				...
				if(hotelChooseInfo.FuXunPayType == "公司支付" || hotelChooseInfo.FuXunPayType == "Central Pay"){
					...
					if(hotelChooseInfo.HotelResourceType != '9') {
						$('.hotelTips').text(get_lan('hotelTipsWithoutResourceTypeNine')) 
					}
				}else{
			...
			if (ProfileInfo.onlineStyle == "APPLE") {
				...
				var canPolicyColor = "#FF7A00"
			}
			if(hotelChooseInfo.HotelResourceType == '2') {
				$('.creditCard-resourceReminder').text(get_lan('creditCardInfo').creditCardResourceReminder).css('color',canPolicyColor)
			}
			if (res.CanPolicy != "" && res.CanPolicy != null) {
				$(".creditCardPolicyBody").html('<span style="color:' + canPolicyColor + ';display:block;margin-bottom:12px;">' + res.CanPolicy + '</span>');
			}
			if(ProfileInfo.onlineStyle != "APPLE"){
				if (res.Policy != "" && res.Policy != null) {
					// $(".creditCardPolicyBody").append('<span style="color:#4a4a4a">' + res.Policy + '</span><br/>');
					$(".creditCardPolicyBody").append('<span style="color:' + canPolicyColor + ';display:block;margin-bottom:12px;">' + res.Policy + '</span>');
				}
				if (res.IsShowLastArriveTime || res.IsShowLastArriveTime == 'true') {
					$(".lastestTime").removeClass('hide');
					$(".preferenceBody").append($('.lastestTime'));
				}
			}
			// $(".creditCardPolicyBody").append('<span>' + get_lan("cancelDateRemind") + '</span>');
			if( res.GaranteeReminder ){
				$(".creditCardPolicyBody").append('<span>' + res.GaranteeReminder + '</span>');
			}
			if ( res.CanPolicy || res.Policy || res.GaranteeReminder ) {
				$('.creditCardPolicy').removeClass('hide')
			}
			if (res.IsNeedCreditCard) {
				...
				$.ajax({
					...
					success: function(data) {
						...
						var format = Y + M
						if (CreditCardNumber != "") {
							fillCCIInput(CreditCardNumber,format);
							$('.creditCardList').html('');
							res.customerCCIs.map(function(CCI){
								var CCInumber = tools.Decrypt(CCI.CreditCardNumber);
								var CCIhideNo = CCInumber.substring(0,4) + "************" + CCInumber.substring(CCInumber.length -
									4, CCInumber.length);
								var CCIexpire = CCI.CreditCardExpire;
								var CCIdate = new Date(CCIexpire);
								Y = CCIdate.getFullYear() + '-';
								M = (CCIdate.getMonth() + 1 < 10 ? '0' + (CCIdate.getMonth() + 1) : CCIdate.getMonth() + 1);
								var CCIformat = Y + M
								$('.creditCardList').append('<p expire="'+CCIformat+'" CreditCardNumber="'+CCInumber+'">'+CCIhideNo+'</p>');
							})
							$('.creditCardNumberInput').click(function(e){
								$('.creditCardList').css('display','block');
								stopBubble(e)
							})
							$('.creditCardList p').click(function(){
								console.log('123');
								var CCInumber = $(this).attr('CreditCardNumber'),
								    CCIexpire = $(this).attr('expire');
								fillCCIInput(CCInumber,CCIexpire);
								$('.creditCardList').css('display','none');
							})
							$(document).click(function(e){
								if(!$(e.target).hasClass('creditCardList')){
									$('.creditCardList').css('display','none');
								}
							})
						}
				...
				function fillCCIInput(number, expire){
					$(".creditCardNumberInput").attr("CreditCardNumber", number);
					$(".creditCardNumberInput").val("****************" + number.substring(number.length -
						4, number.length));
					$(".creditCardDateInput").val(hideDocDate(ProfileInfo,expire));
					$(".creditCardDateInput").attr("hideNo",expire);
				}
			...
			if (res.ReasonCodes.length > 1 || reasonOT) {
    ...
    //                     ')
				...
				function toggleOTReason(dom){
					if($(dom).val()=='OT'){
						$('.popOTReason,.hotelOTReason').show();
					}else{
						$('.popOTReason,.hotelOTReason').hide();
					}
				}
				$('.popReasonSelect').bind('change',function(){
					toggleOTReason(this);
				})
				$('.hotelReasonSelect').bind('change',function(){
					toggleOTReason(this);
				})
			} else {
			...
			if (res.ReasonCodes.length == 1 && res.ReasonCodes[0].Code != 'OT') {
...
function surePassengerInfo() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(!passengerJson.AbacusPriceNotContainTax){
				$('.hotelAveragePerNight').hide();	//平均每日价
			}
...
function newCustomerPop(CompanyId) {
	$(".newCustomerPop").html('\
		...
		'</div>\
		<div class="passengerPopList">\
			<div class="newCustomerLi require flexRow hide newCustomerCompany">\
                <div style="width:105px;">'+get_lan('passengerPop').company+'</div>\
                <select class="newCompanySelect"></select>\
            </div>\
	...
	/*新旅客公司*/
    if(ProfileInfo.AddCustomerChangeCompany){
        $(".newCustomerCompany").removeClass("hide");
        $.ajax(
            {
              type:'post',
              url : $.session.get('ajaxUrl'), 
              ...
              success : function(data) {
                  ...
                  $('body').mLoading("hide");
                  if(res.length==0){
                    $(".newCustomerCompany").addClass("hide");
                  }
                  res.map(function(item){
                      if(item.CompanyId==JSON.parse($.session.get('ProfileInfo')).companyId){
                          $(".newCompanySelect").append('\
	...
	$(".newCustomerPopBtn").unbind("click").click(function() {
		...
		if (!regPhone.test($(".newCustomerInputPhone").val())) {
			...
			var companyId = '';
            if(ProfileInfo.AddCustomerChangeCompany){
				companyId = $(".newCompanySelect").val();
				CompanyId = companyId;
            }
			$('body').mLoading("show");
			$.ajax({
				...
				dataType: 'json',
				data: {
					url: $.session.get('obtCompany') + "/SystemService.svc/AddNewCustomerChangeCompanyPost",
					jsonStr: '{"request":{"language":"' + obtLanguage + '","id":' + netUserId + ',"nameCn":"' + NameCN + '","nameEn":"' +
						NameEN + '","nickname":"' + Nick + '","gender":"' + sex + '","nation":"' + nationality + '","birthday":"' +
						birthday + '","NameS":"' + NameCN + '","email":"' + email + '","phoneNumber":"' + phone + '","groupIDs":"0","companyId":"'+companyId+'"}}'
				},
				success: function(data) {
					var res = JSON.parse(data);
					console.log(res);
	...
	$.ajax({
		...
		success: function(data) {
			...
			res.map(function(item, index) {
				...
				$(".passengerList").append('\
                    <div class="passengerLi" customerId="' + item.ID +'" companyId="' + item.OrderCompanyId +'" style="width:792px;margin-left:24px;border-bottom: 1px #C0CAD8FF dashed;">\
						<div class="delIcon" customerId="' +item.ID + '">x<span style="margin-left:10px;">'+get_lan('passengerPop').delete+'</span></div>\
						<div style="height:130px;width:100%" class="flexRow">\
							<div style="height:130px;width:676px;box-sizing:border-box;font-size:14px;" class="flexWrap">\
								<div class="passengerLiName" style="width:300px;height:60px;">\
									<div style="color:#5D667C;height:30px;line-height:30px;">' +get_lan('passengerPop').ticketName +'</div>\
									<div style="color:#041E5B;height:30px;line-height:30px;"><span class="PassengerNameText">' +passengerName + '</span><span class="changePassengerInfo" index="' + index +'" customerId="' + item.ID + '">' + changePassengerInfo +'</span></div>\
								</div>\
								<div class="passengerLiDocuments" style="width:300px;height:60px;">\
									<div style="color:#5D667C;height:30px;line-height:30px;">' +get_lan('passengerPop').popDocuments +'</div>\
									<div style="color:#041E5B;height:30px;line-height:30px;"><select class="documentsSelect"></select></div>\
								</div>\
								<div class="passengerPhone" style="width:300px;height:60px;" hideNo="'+item.Phones+'">\
									<div style="color:#5D667C;height:30px;line-height:30px;">' +get_lan('passengerPop').popPhone +'</div>\
									<div style="color:#041E5B;height:30px;line-height:30px;">' +profilePhone +'</div>\
								</div>\
								<div class="passengerEmail" style="width:300px;height:60px;">\
									<div style="color:#5D667C;height:30px;line-height:30px;">' +get_lan('passengerPop').popMail +'</div>\
									<div style="color:#041E5B;height:30px;line-height:30px;">' +hideEmail(ProfileInfo,item.Email) +'</div>\
								</div>\
							</div>\
							<div style="height:130px;width:140px;font-size:14px;color:#041E5B;position:relative;">\
								<span class="changeRemarkBtn" index="' +index + '" style="position:absolute;top: 40px;right: 15px;cursor:pointer;text-decoration: underline;">' + get_lan('passengerInfo').remarks +'</span>\
							</div>\
						</div>\
                    </div>\
                    ')
			...
			$(".passengerLi").eq($(".passengerLi").length-1).css("border",0);
			//价格
...
function checkCreditCardInfo (payRes) {
	let mandatoryItem = ''
	if ($("#creditCardSurname").val() == "" || $("#creditCardGivenName").val() == "") {
		mandatoryItem = obtLanguage == "CN" ? '持卡人姓名, ' : 'Cardholder, ' 
	} 
	if ($('.creditCardNumberInput').val() == "") {
		mandatoryItem += obtLanguage == "CN" ? '信用卡卡号, ' : 'Card Number, ' 
	} 
	if ($(".creditCardDateInput").val() == "") {
		mandatoryItem += obtLanguage == "CN" ? '信用卡有效期, ' : 'Expiry Date, ' 
	} 
	if (payRes.IsNeedCvv && $(".creditCardCvvInput").val() == "") {
		mandatoryItem += obtLanguage == "CN" ? '信用卡CVV, ' : 'CVV, ' 
	} 
	if (payRes.IsNeedDocumentNum && $(".creditCardDocumentNumberInput").val() == "") {
		mandatoryItem += obtLanguage == "CN" ? '证件号码, ' : 'Document Number, ' 
	}
	if(mandatoryItem !== '') {
		if (obtLanguage == "CN") {
			tools.alert('请输入' + mandatoryItem.substr(0, mandatoryItem.length-2), '确定', false);
		} else {
			tools.alert('Please enter the ' + mandatoryItem.substr(0, mandatoryItem.length-2), 'Confirm', false);
		}
		return false
	} 
	return true
...
function clickReserveBtn(payRes) {
	$(".bookHotelBtn").unbind("click").click(function() {
		if(payRes.IsNeedCreditCard) {
			if(!checkCreditCardInfo(payRes)) {
				return false
			}
		}
...
function CheckHotelNeedRemaind(payRes) {
	...
	if ($(".hotelReasonSelect").length > 0 && ProfileInfo.IsHideHotelReason!=true) {
		if ($(".hotelReasonSelect option:selected").val() == "" || ($(".hotelReasonSelect option:selected").val() == "OT" && $(".hotelOTReason").val() == "")) {
			$('body').mLoading("hide");
...
function reserveHotel(res, payRes) {
	...
	if ($(".bookHotelBtn").attr("reasoncode")) {
		...
		//防注入
		var otherReason = $(".hotelOTReason").val().trim().split("").map(function(item){ return item.replace(/'|"|=|;|>|<|%/i,'')}).join("");
	...
	var preference = $('input[name="bedTypeRequirement"]:checked').val() + "," + $(
		'input[name="roomTypeRequirement"]:checked').val() + "," + $('input[name="checkInRequirement"]:checked').val()+ "," + $('input[name="checkOutRequirement"]:checked').val();
	if ($(".lastestTimeSelect option:selected").val() == "24:00") {
		var s1 = '2201';
		var s2 = '-0041';
	} else {
	...
	if (res.NeedPay) {} else {
		...
		if ($(".frequentCardsSelect").length > 0 && !$(".frequentCardsInfo").hasClass("hide") && $(".frequentCardsSelect").val() !=
			...
			for (var i = 0; i < $(".frequentCardsSelect").length; i++) {
				if ($(".frequentCardsSelect").eq(i).find("option:selected").attr("cardnumbers") != "不使用会员卡" && $(
		...
		$.ajax({
			...
			data: {
				...
				jsonStr: '{"request":{"queryKey":"' + finalQueryKey + '","preference":"' + preference + '","creditCardKey":"' +
					...
					',"Language":"' + obtLanguage + '","busInfo":"' + busInfo + '","memberShipInfo":' + JSON.stringify(memberShipInfo.split(',')) + 
					',"swPolicyCode":"'+swPolicyCode+'","otherReason":"' + otherReason + '"}}'
			},
			success: function(data) {
				$('body').mLoading("hide");
				var res = JSON.parse(data);
...
function changeNewUid(orderNo) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			console.log(message);
			if (code == 200) {
				$.cookie('closeParentTab', true, { expires: 1, path: '/' })	// 控制酒店列表不关闭子窗口
