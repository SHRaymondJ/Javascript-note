        ...
        $.ajax({
            ...
            success: function (data) {
                if (!isJsonString(data) && TAnumber && TAnumberIndex) {
                    ...
                    if (TAnumber && $.session.get('TAOneCity') != 1) {
                        cityFilter("1", JSON.parse(data), airCity, "intel")
                    } else {
                        cityData(JSON.parse(data), airCity)
                    }
            ...
            creatItem: function () {
                ...
                for (var group in city) {
                    ...
                    for (var i = 0, iLen = itemKey.length; i < iLen; i++) {
                        ...
                        var dl = $('<dl>'),
                            dt = '<dt>' + (itemKey[i] == 'hot' ? popular : itemKey[i]) + '</dt>',
                            dd = $('<dd>'),
                            str = '';
                ...
                if (localStorage.getItem('recentSearch') && JSON.parse(localStorage.getItem('recentSearch')).air) {
                    var recentAir = JSON.parse(localStorage.getItem('recentSearch')).air
                    var dl = $('<dl>'),
                        dt = '<dt>' + ($.session.get('obtLanguage') === 'CN' ? '最近搜索' : 'Recent Searches') + '</dt>',
                        dd = $('<dd>'),
                        str = '';
                    recentAir.map(function (air) {
                        switch ($.session.get('obtLanguage')) {
                            case 'CN':
                ...
                $('.kucity_item dd').on('click', 'span', function (e) {
                    if ($(e.target).attr('type')) {
                        ...
                        var departureCode = $(e.target).attr('departureCode'),
                            ...
                            departureDate = $(e.target).attr('departureDate'),
                        ...
                        $('#intlDepartureDate').val(departureDate)
                        if(returnDate){
                            $('#intlReturnDate').val(returnDate)
                        }
                        switch(type){
