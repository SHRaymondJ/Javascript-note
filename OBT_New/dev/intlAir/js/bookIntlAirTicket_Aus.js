...
function initChart(orderAmount) {
    ...
    $('.budgetRemain').html(remainAmount + curreny);
}
function getPOReminder(){
    var POReminder = ''

    if ( obtLanguage == 'CN' && ProfileInfo.ReminderCnForPObook ){
        POReminder = ProfileInfo.ReminderCnForPObook
    }else if ( ProfileInfo.ReminderEnForPObook ){
        POReminder = ProfileInfo.ReminderEnForPObook
    }
    return '<div class="activeFontColor intlNotice1">'+ POReminder + '</div>'
}
//内容展示
    ...
    $("#main").html('\
        <div class="autoCenter">\
              ...
              <div class="budgetBody">\
                ...
                <div class="budgetBox">\
                    <div class="budgetItem">\
                        ...
                        <p class="budgetAmount totalBudget">' + curreny + totalBudget + '</p>\
                    </div>\
                    <div class="budgetItem">\
                        <p class="budgetDesc">'+ get_lan('budget').budgetUsed + '</p>\
                        <p class="budgetAmount usedBudget">' + curreny + usedBudget + '</p>\
                    </div>\
                </div>\
              </div>\
            </div>\
            <div class="remindInfo">\
               ...
               <div class="activeFontColor intlNotice1">·'+ get_lan("intlNotice1") + '</div><br>'
              + POReminder +
              '<div class="intlNotice2 mainFontColor">·'+ get_lan("intlNotice2") + '</div><br>\
              <div class="covidReminder"></div><br><br>\
            </div>\
            <div class="intlReasonBody">\
              <div class="intlReasonTittle">'+ get_lan('intlReason') + '</div>\
              <div class="intlReason flexRow">\
                <select class="intlReasonSelect">\
                </select>\
                <input class="reasonText hide" placeholder="'+ get_lan('reasonText') + '">\
              </div>\
              <div class="lowestPriceBody"></div>\
            </div>\
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.ajax({
                ...
                data: {
                    url: $.session.get('obtCompany') + "/QueryService.svc/InterAirRecommendNew",
                    jsonStr: '{"request":{"segmentKey":"' + intlTicketInfo.segmentKey + '","goAirline":"' + intlTicketInfo.AirLineCode + '","searchType":"' + searchType + '","includeTax":"true","id":' + netUserId + ',"Language":"CN","BCN":"' + BCN + '"}}'
                },
                success: function (data) {
                    var res = JSON.parse(data);
                    var citys = ''
                    ...
                    $(".orderDetail").attr("citys", citys.substring(0, citys.length - 1));
                }
            })
            ...
            res.map(function (item, index) {
                ...
                initChart(parseInt($(".orderDetail").attr('totalPrice')));
                $(".totalFareText").html($(".orderDetail").attr('totalPrice') + '<span style="font-size:16px;">' + item.Curreny + '</span>');
                var showServiceFare = item.ServiceValue == 0 ? "hide" : "";
                $(".fareBodyDetail").html('\
                    <div class="orderFareAmount flexRow"><div style="width:240px;">'+ get_lan('orderDetail').ticketPrice + '</div><span style="color:#111C4E;font-weight:600;">' + item.FaceValue + item.Curreny+ '</span></div>\
                    <div class="orderTotalTax flexRow"><div style="width:240px;">'+ get_lan('orderDetail').tax + '</div><span style="color:#111C4E;font-weight:600;">' + item.TaxValue + item.Curreny + '</span></div>\
                    <div class="orderServiceFare flexRow '+ showServiceFare + '"><div style="width:240px;">' + get_lan('orderDetail').serviceFare + '</div><span style="color:#111C4E;font-weight:600;">' + item.ServiceValue + item.Curreny + '</span></div>\
                    ')
                item.InterSegments.map(function (sItem, sIndex) {
                    ...
                    var airport = sIndex==0?item.InterSegments[0].AirportDeparte+'-'+ item.InterSegments[item.InterSegments.length-1].AirportArrive:"";
                    $(".orderDetailLeft").eq(index).append('\
                		<div class="orderDetailLeftBody">\
                          <div class="orderStayTime '+ showStay + '">\
                            '+ get_lan('orderDetail').stay + '<span style="color:#041E5B;margin-left:38px;">' + sItem.StayTime + '<span>\
                          </div>\
                          <div style="position:relative;height: 215px;">\
                            <div class="orderDateStart">'+ sItem.DateBegin + '</div>\
                            <div class="orderAirport ellipsis">'+ airport + '</div>\
                            ...
                            <div class="orderTimeStart">'+ sItem.TimeStart + '</div>\
                            ...
                            <div class="orderTimeArrive">'+ sItem.TimeArrive + '</div>\
                            ...
                            <div class="orderCabin">'+ get_lan('orderDetail').cabin + ':<span style="color:#333;"> ' + sItem.Cabin + '</span></div>\
                            <div class="orderPlaneType">'+ get_lan('orderDetail').planeType + ':<span style="color:#333;">' + sItem.PlaneType + '</span></div>\
                          </div>\
                		</div>\
                		')
                })
                // 预算
                $('.totalBudget').text(totalBudget + curreny);
                $('.usedBudget').text(usedBudget + curreny);
            })
            ...
            if (!res[0].isShowReason && !ProfileInfo.NeedSpecialPolicy) {//方糖权限NeedSpecialPolicy 为true显示违反政策
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        console.log(res);
                        if (res.length > 0) {
                            $(".lowestPriceBody").html('\
                                <div class="intlReasonPrice"><span class="intlReasonPriceTittle" style="margin-bottom: 0;">'+ get_lan("lowestPrice") + '</span><span class="minPrice">' + (parseInt(res[0].Fare) + parseInt(res[0].TotalTax)) + res[0].Curreny + '</span></div>\
                            ')
                            res.map(function (item) {
                                item.InterSegments.map(function (sItem) {
                                    var DepTerm = sItem.DepTerm == null || sItem.DepTerm == "" ? "" : '(' + sItem.DepTerm + ')';
                                    var ArrTerm = sItem.ArrTerm == null || sItem.ArrTerm == "" ? "" : '(' + sItem.ArrTerm + ')';
                                    $(".lowestPriceBody").append('\
                                        <div class="intlReasonLi flexWrap">\
                                            <div style="width:230px;">'+ sItem.DateBegin +' '+sItem.TimeStart+'~'+sItem.TimeArrive+ '</div>\
                                            <div style="width:500px;margin-left:20px;">'+ sItem.FlightNo + '</div>\
                                            <div style="width:230px;">'+ sItem.Marketing + '</div>\
                                            <div style="width:500px;margin-left:20px;">' + sItem.AirportDeparte + DepTerm+'-'+sItem.AirportArrive+ArrTerm+'</div>\
                                        </div>\
                                    ')
                                })
                            })
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (searchIntlInfo.alterTicketInfo && !$.session.get('goOnBookOrderNo')) {
                            ...
                            //有代订权限//&&!$.session.get('TACustomerId')
                            if (res.length > 1) {
...
function passengersInOrder(customerState) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".passengerList").html('');
            if (res.length > 0) {
                $(".orderDetail").attr("CompanyID", res[0].OrderCompanyId);
            }
            var changePassengerInfo = ProfileInfo.onlineStyle === 'APPLE' ? get_lan('passengerInfo').changePassengerInfoApple : get_lan('passengerInfo').changePassengerInfo
            ...
            /*有审批单*/
            if (($(".passengerLi").length == 1 && $.session.get('TAnumber')) || searchIntlInfo.alterTicketInfo) {
...
function bookTicket(queryKey) {
    ...
    $.ajax({
        ...
        data: {
            ...
            jsonStr: '{"request":{"id":' + netUserId + ',"queryKey":"' + queryKey + '","otherKey":"' + otherKey + '","frequentCards":"' + frequentCards + '","remarkKey":"' + remarkKey + '","includeTax":"true","orgTicketInfo":"' + orgTicketInfo + '","isDirectTicket":"' + isDirectTicket + '","Language":"' + language + '","phones":"","orgOrderNo":"","orgPNR":"","remarkList":' + remarkList + '}}'
        },
        success: function (data) {
            $('body').mLoading("hide");
            if (data == '') {
