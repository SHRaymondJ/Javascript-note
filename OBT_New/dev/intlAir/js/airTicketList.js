...
function newFilter(segmentList){
	...
	initFilterSort();
	//优化搜索
	$('.filter__closeBox__items input').click(function(event){
		console.log(123);
	})
	//优化搜索 - 初始化 - 筛选框
	function initFilterSort(){
		...
		$('.ticketBody').html('\
			<div class="filter">\
				...
				<div class="filter__closeBox__container"></div>\
				<div class="filter__openBox"></div>\
				<div class="reset__filter"><span>' + get_lan('refineSearch').reset + '</span></div>\
				<div class="sort">\
					<h3 class="sort__title">' + get_lan('refineSearch').sortTitle + '</h3>\
					<div class="sort__buttons">\
						<div class="sort__button departureTimeSortNew">' + departureSortDefault + '</div>\
						<div class="sort__button durationSortNew">' + durationSortDefault + '</div>\
						<div class="sort__button priceSortNew">' + priceSortDefault + '</div>\
					</div>\
				</div>\
			</div>\
		')
		...
		//初始化 - 标题
		var resultCount = '('+ get_lan('refineSearch').show +  showingCounts + get_lan('refineSearch').of + Summary + ')';
		$('.filter__bar').html('\
			<h2 class="filter__bar__title">'+get_lan('refineSearch').title+'</h2>\
			<span class="filter__bar__resultCount">' + resultCount + '</span>\
			<hr>\
			<span class="filter__bar__button open">' + get_lan('refineSearch').collapse + '</span>\
		')
		
		//初始化 - 小筛选
		$('.filter__closeBox__container').html('\
			<div class="filter__closeBox">\
				<div class="filter__closeBox__fixedPart">\
				</div>\
			</div>\
			<div class="filter__airCompanyPart">\
				<div class="left"><div class="left__button"></div></div>\
				<div class="filter__airCompanys">\
					<div class="filter__airCompanys__box">\
					</div>\
				</div>\
				<div class="right"><div class="right__button"></div></div>\
			</div>\
		');
		...
		filters.map(function(filterOption,filterIndex){
			...
			if(filterOption.child && filterOption.child.length >= 1){		
				var openSelectorBox = '', closeSelectorBox = '';
				multiple = 'multiple';
				filterOption.child.map(function(cardChild,index){
					var options = ''
					cardChild.values.map(function(option){
						options += '\
							<div class="openBox__card__itemBox" value="' + option.value + '" selector="' + cardChild.name + '">\
								<div class="filter__selector selected"></div>\
								<div class="filter__selector__desc">' + option.text + '</div>\
							</div>'
					})
					openSelectorBox += '\
						<div class="filter__closeBox__items">\
							<p>' + cardChild.title + '</p>\
							<input type="text" value="' + cardChild.default + '" name="' + cardChild.name + '" id="" readonly>\
							<div class="openBox__selectorBox__options" index="' + index + '">' + options + '</div>\
						</div>'
					closeSelectorBox += '\
						<div class="filter__closeBox__items-air">\
							<p>' + cardChild.title + '</p>\
							<div class="openBox__selectorBox__options-air" index="' + index + '">' + options + '</div>\
						</div>'
				})
				openCardBody = '<div class="openBox__card__selectorBox">' + openSelectorBox + '</div>';
				closeCardBody = '<div class="openBox__card__selectorBox-air">' + closeSelectorBox + '</div>';
			}else{
				var options = ''
				filterOption.values.map(function(option){
					options += '\
						<div class="openBox__card__itemBox" value="' + option.value + '" selector="' + filterOption.name + '">\
							<div class="filter__selector selected"></div>\
							<div class="filter__selector__desc">' + option.text + '</div>\
						</div>'
					
				})
				openCardBody = '<div class="openBox__card__selectorBox">' + options + '</div>';
				closeCardBody = '<div class="openBox__card__selectorBox">' + options + '</div>';
			}
			//2. 大筛选条件
			$('.filter__openBox').append('\
				<div class="openBox__card">\
					<div class="openBox__card__title">' + filterOption.title + '</div>\
					<div class="openBox__card__body" index="'+filterIndex+'">' + openCardBody +'</div>\
				</div>');
			//3. 小筛选条件
			$('.filter__closeBox__fixedPart').append('\
				<div class="filter__closeBox__items">\
					<p>' + filterOption.title + '</p>\
					<input class="filter__closeBox__input" type="text" name="' + filterOption.name + '" value="' + filterOption.default + '" id="" readonly>\
					<div class="openBox__card__body ' + multiple + '" index="'+filterIndex+'">' + closeCardBody +'</div>\
				</div>')
			//4. 公司遍历
			if(filterOption.name == 'airlines'){
				filterOption.values.map(function(airlines){
					if(airlines.value == 'all'){ return false; };
					var priceText = airlines.lowestPrice + ' ' + airlines.currency
					var lowestPrice = obtLanguage == 'CN' ? 
									priceText + '起' :
									'From '+ priceText;
					$('.filter__airCompanys__box').append('\
						<div class="filter__airCompany" value="' + airlines.value + '" selector="' + filterOption.name + '">\
							<div class="filter__selector selected"></div>\
							<div class="filter__airCompany__itemBox">\
								<p class="filter__airCompany__name">' + airlines.text + '</p>\
								<p class="filter__airCompany__price">' + lowestPrice + '</p>\
							</div>\
						</div>')
				})
				$('.filter__airCompanys__box').append('<div style="clear:both"></div>');
			}
		})
		if(isReturn == 1){
		...
		$('.reset__filter span').click(function(){
			//还原搜索条件
	...
	window.onscroll = function(){
		//339 relative时的高度
		// 187 fixed时的高度
		if(339 <= window.scrollY && 
			$('.ticketList').height() - window.innerHeight > 200){
			$('.ticketBody,.sort,.ticketList').addClass('fixed');
			if($('.filter__bar__button').hasClass('open')){
				switchFilterItems();
			}
			hiddenBox('.filter__airCompanyPart');
			$('.sort').hide();
		}else{
			$('.ticketBody,.sort,.ticketList').removeClass('fixed');
			showBox('.filter__airCompanyPart');
			$('.sort').show();
		}
	}
	...
	//展开收起逻辑
	function switchFilterItems(){
		if($('.filter__bar__button').hasClass('open')){
			//收起
			$('.filter__bar__button').text(get_lan('refineSearch').expand);
			showBox('.filter__closeBox__container');
			hiddenBox('.filter__openBox');
		}else{
			//展开
			$('.filter__bar__button').text(get_lan('refineSearch').collapse);
			hiddenBox('.filter__closeBox__container');
			showBox('.filter__openBox');

		}
		$('.filter__bar__button').toggleClass('open')
	}
	function showBox(dom){
		$(dom).removeClass('hide');
	}
	function hiddenBox(dom){
		$(dom).addClass('hide');
	}
	//阻止冒泡函数
	...
	//优化搜索 -- 弹框及防止冒泡
	$('.openBox__card__selectorBox input').click(function(e){
		var option = $(this).siblings('.openBox__selectorBox__options');
		var optionIndex = option.attr('index');
		$('.openBox__selectorBox__options[index!='+optionIndex+']').removeClass('show');
		option.toggleClass('show');
		stopBubble(e);
	})
	$('.filter__closeBox__container .filter__closeBox__input').click(function(e){
		var option = $(this).siblings('.openBox__card__body');
		var optionIndex = option.attr('index');
		$('.filter__closeBox__container .openBox__card__body[index!='+optionIndex+']').removeClass('show');
		option.toggleClass('show');
		stopBubble(e);
	})
	$('.openBox__selectorBox__options,.filter__closeBox__container .openBox__card__body').click(function(e){
		stopBubble(e);
	})
	$(document).click(function(e){
		if(!$(e.target).hasClass('openBox__selectorBox__options')){
			$('.openBox__selectorBox__options').removeClass('show');
		}
		if(!$(e.target).hasClass('openBox__card__body')){
			$('.filter__closeBox__container .openBox__card__body').removeClass('show');
		}
	})
	//优化搜索 -- 航空公司轮播
	var lineWidth = '1036',
		airCompanyCount = $('.filter__airCompany').length;
	var airCompanysBoxWidth = 207.2 * airCompanyCount;
	var maxPage = airCompanyCount%5 == 0 ? parseInt(airCompanyCount/5) : parseInt(airCompanyCount/5) + 1;
	var airCompanyPage = 1;

	checkAirCompanyPage()
	$('.filter__airCompanys__box').css('width', airCompanysBoxWidth + 'px');

	var i = 5;
	while(i < airCompanyCount){
		$('.filter__airCompany').eq(i).css('border','none');
		i += 5;
	}
	$('.filter__airCompanyPart .right').click(function(){
		if($(this).hasClass('active')){
			var translateX = airCompanyPage * ( - lineWidth ) + 'px';
			$('.filter__airCompanys__box').css('transform','translateX('+translateX+')');
			airCompanyPage ++;
			checkAirCompanyPage();
		}
	})
	$('.filter__airCompanyPart .left').click(function(){
		if($(this).hasClass('active')){
			airCompanyPage --;
			var translateX = (airCompanyPage - 1) * ( - lineWidth ) + 'px';
			$('.filter__airCompanys__box').css('transform','translateX('+translateX+')');
			checkAirCompanyPage();
		}
	})
	
	$('.filter__bar__button').click(function(){
		switchFilterItems()
	});
	
	function checkAirCompanyPage(){
		console.log(maxPage,airCompanyPage);
		//左
		if(maxPage > airCompanyPage){
			$('.filter__airCompanyPart .right').addClass('active');
		}else{
			$('.filter__airCompanyPart .right').removeClass('active');
		}
		//右
		if(airCompanyPage > 1){
			$('.filter__airCompanyPart .left').addClass('active');
		}else{
			$('.filter__airCompanyPart .left').removeClass('active');
		}
	}
	//优化搜索 -- 公司轮播 -- 动态控制高度
	var boxHeight = 0;
	$('.filter__airCompany').map(function(index){
		var heightPX = $('.filter__airCompany').eq(index).css('height')
		var height = heightPX.slice(0,heightPX.length-2);
		if(parseInt(height) > boxHeight){
			boxHeight = height;
		}
	})
	console.log(boxHeight);
	var companyHeight = $('.filter__airCompanyPart').css('height').slice(0,$('.filter__airCompanyPart').css('height').length-2);
	$('.filter__airCompany').css('height',boxHeight+'px');
	$('.left,.right').css('top',((companyHeight-70)/2)+'px');
	$('.filter__closeBox__container').addClass('hide');
}
function resetSorts(){
	...
	$("#main").html(
        ...
        <div class="autoCenter">\
            ...
            <div class="ticketBody">\
                <div class="listTittle hide">' +get_lan('ticketList').tittle +'</div>\
                <div class="chooseDate specificFontColor hide"></div>\
                <div class="listRemind flexRow hide">\
                   <div style="width:90%;font-weight:bold;padding-left:40px" class="roundTittle">' +get_lan('searchBody').oneWay +'</div>\
                </div>\
                <div class="siftBody hide"></div>\
			</div>\
...
function sortTicketInfo(res) {
	...
	$(".departureTimeSortNew").unbind("click").click(function() {
		...
		if ($(".departureTimeSortNew").attr("sortType") == "asc") {
			ticketListInfo(bubbleSortDes(timeSortDes));
			$(".departureTimeSortNew").attr("sortType", "desc").text(get_lan('refineSearch').departureSortDesc);
		} else if (!$(".departureTimeSortNew").attr("sortType") || $(".departureTimeSortNew").attr("sortType") == "desc") {
			ticketListInfo(bubbleSortAsc(timeSortAsc));
			$(".departureTimeSortNew").attr("sortType", "asc").text(get_lan('refineSearch').departureSortAsc);
		}
		// scrollToTop()
	})
	$(".durationSortNew").unbind("click").click(function() {
		$(this).siblings().removeClass('active');
		$('.departureTimeSortNew').text(departureSortDefault);
		$('.priceSortNew').text(priceSortDefault);
		if(!$(this).hasClass('active')){
		...
		if ($(".durationSortNew").attr("sortType") == "asc") {
			ticketListInfo(usetimebubbleSortDes(useTimeSortDes));
			$(".durationSortNew").attr("sortType", "desc").text(get_lan('refineSearch').durationSortDesc);
		} else if (!$(".durationSortNew").attr("sortType") || $(".durationSortNew").attr("sortType") == "desc") {
			ticketListInfo(usetimebubbleSortAsc(useTimeSortAsc));
			$(".durationSortNew").attr("sortType", "asc").text(get_lan('refineSearch').durationSortAsc);
		}
		// scrollToTop()
	})
	$(".priceSortNew").unbind("click").click(function() {
		$(this).siblings().removeClass('active');
		$('.departureTimeSortNew').text(departureSortDefault);
		$('.durationSortNew').text(durationSortDefault);
		if(!$(this).hasClass('active')){
		...
		if ($(".priceSortNew").attr("sortType") == "asc") {
			ticketListInfo(pricebubbleSortDes(priceSortDes));
			$(".priceSortNew").attr("sortType", "desc").text(get_lan('refineSearch').priceSortDesc);
		} else if (!$(".priceSortNew").attr("sortType") || $(".priceSortNew").attr("sortType") == "desc") {
			ticketListInfo(pricebubbleSortAsc(priceSortAsc));
			$(".priceSortNew").attr("sortType", "asc").text(get_lan('refineSearch').priceSortAsc);
		}
		// scrollToTop()
	})
}
