...
var en = {
	...
	"budget": {
		...
		"budgetUsed": "Used Budget",
		"budgetText": "Other Budget"
	},
...
function initChart(costCenter) {
	// 基于准备好的dom，初始化echarts实例
	var myChart = echarts.init(document.getElementById("eChart"));
	var remainAmount = costCenter.leftBudget,
		totalAmount = costCenter.budgetAmount;
	// var remainPercentage = costCenter.leftBudgetRate;
	var subText = costCenter.subText;
	var color = costCenter.color
	// 指定图表的配置项和数据
	option = {
		title: {
			text: " ",
...
function showContent() {
	    ...
	    <div class="autoCenter">\
			...
			<div class="budgetInfo">\
              ...
              <div class="budgetBody">\
			  	<table border="0" class="budgetTable">\
				  	<tr>\
						<th class="tableCostCenter"><div class="tableCostCenterSelector"><select></select></div></th>\
						<th class="thBorder"></th>\
						<th class="thBorder">'+get_lan('budget').budgetRemain+'</th>\
						<th class="thBorder">'+get_lan('budget').budgetTotal+'</th>\
						<th class="thBorder">'+get_lan('budget').budgetUsed+'</th>\
					</tr>\
				</table>\
              </div>\
            </div>\
	...
	')
	if(!ProfileInfo.ShowBudgetApprove){
        $('.budgetInfo').hide();
    }
...
function orderDetails() {
	...
	$.ajax({
		...
		dataType: 'json',
		data    : {
			url    : $.session.get('obtCompany') + '/QueryService.svc/QueryCompanyBudget',
			jsonStr: JSON.stringify(budgetJson)
		},
		success: function (data) {
			if ( data ) {
				var res = JSON.parse(data);
				if( res.code === '200' && res.budgetInfos.length >= 1){
					var budgetInfos = res.budgetInfos
					var budgetArray = []
					budgetInfos.map(function(budget, index){
						var costCenter = {
							costCenterName : budget.remarks.length>0 ? budget.remarks[0].remarkName : get_lan('budget').budgetText,
							costCenterValue : ' ' + budget.remarks.length>0 ? budget.remarks[0].remarkValue : '',
							subText : budget.leftBudgetRate + '%',
							budgetAmount : budget.budgetAmount,
							usedBudget : budget.usedBudget,
							leftBudget : budget.leftBudget,
							leftBudgetRate : budget.leftBudgetRate,
							color : budget.leftBudgetRate < 20 ? "#F6AA25" : "#8DB15C"
						}
						budgetArray.push(costCenter)
						// 初始添加图表
						if ( index === 0) {
							var eChart = 
								'<td rowspan="' + index + '" class="tableChartBox">\
							...
							var trActive = '<tr class="active" key="'+index+'">'
						} else {
							var eChart = ""
							var trActive = '<tr key="'+index+'">'
						}
						// 添加列表
						$('.budgetTable').append(
							trActive + eChart +
								'<td>\
									<p class="budgetAmountTittle">' + costCenter.costCenterName + costCenter.costCenterValue +':</p>\
								</td>\
								<td>\
									<p class="budgetAmount"><b class="budgetRemain">' + costCenter.leftBudget + '</b></p>\
								</td>\
								<td>\
									<p class="budgetAmount">' + costCenter.budgetAmount + 'CNY</p>\
								</td>\
								<td>\
									<p class="budgetAmount">' + costCenter.usedBudget + 'CNY</p>\
								</td>\
							</tr>\
						')
						$('.tableCostCenterSelector select').append('<option value="'+index+'">' + costCenter.costCenterName + ' ' + costCenter.costCenterValue +'</option>')
						// 初始渲染
						if ( index === 0) {
							initChart(costCenter)
						}
					})
					// $.session.set('costCenterBudget', JSON.stringify(budgetArray))
					$('.tableCostCenterSelector select').change(function(){
						var index = $(this).val()
						console.log(budgetArray)
						initChart(budgetArray[index])
						$('tr').removeClass('active')
						$('tr[key="'+index+'"]').addClass('active')
					})
				}
			}
		}
			...
			success: function (data) {
				...
				<div class="orderDetailInfo">\
					<div class="lineInfo lineInfoBetween">\
						<div class="orderDetailInfoNo flexRow" style="font-weight:bold;">'+ get_lan('orderInfo').OrderCustomer + ' ' + Passenger + '</div>\
						<div class="orderDetailInfoPrice">'+ get_lan('orderInfo').OrderFare + ' <span style="font-size:22px;color:#F58A00">' + res.Price + 'CNY</span></div>\
					</div>\
...
function otherList(res){
	...
	$.ajax({
		...
		data: {
			...
			jsonStr: '{"orderNo":"' + res.OrderNo + '","id":' + netUserId + ',"companyId":"' + ProfileInfo.companyId + '","Language":"'+obtLanguage+'"}'
		},
		success: function (data) {
			var res = JSON.parse(data);
			console.log(res);
